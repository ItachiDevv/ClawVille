...
- Also repaired in-branch: the 3 pre-existing `partner-hatcher-p5-handler.test.ts` failures (fabricated rows missing `createdAt/updatedAt`; table-blind db stub letting post-commit wallet inserts clobber the agent row → PATCH 404). All 16 tests across watcher-gate + both partner p5 suites green.
