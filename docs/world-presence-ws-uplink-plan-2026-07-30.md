...
- **Pong-timeout reap closes with private 4408, not 1001 (found by live-staging smoke S7, 2026-07-31):** Bun rewrites `close(1001)` to 1000 on the wire (proven by a minimal in-container repro; the reason string survives, 4xxx codes pass through intact). 1000 would make reaps indistinguishable from voluntary `/leave` closes for both the client and the close-code-distribution monitoring, so the reap uses `PONG_TIMEOUT: 4408`. Client behavior is unchanged — unlisted 4xxx codes already route to the generic bare-reopen ladder.
