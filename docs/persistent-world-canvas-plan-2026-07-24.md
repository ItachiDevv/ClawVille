...
| P1b — cove joins the stage; returns become fades | IMPLEMENTED LOCALLY — serial build/typecheck/probe evidence + reviewer manual drive pending | `docs/world-stage-p1b-{brief,notes}.md`; `/cove` page-only joins `(world)`; real-route 30-round-trip Puppeteer lane added; no completion/sign-off claimed |
