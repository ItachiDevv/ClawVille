...
- **P5 directive preemption (2026-07-16; implemented in local diff, not deployed):** a live staging repro posted a directive during a walk to Boating School, then observed the agent finish that walk (+36s), complete the teacher turn (+68s), and remain in the 60-second talk linger without acting by the +114s end of the probe. A fresh directive now preempts walking/arrival/talking into an immediate decide, including one bounded follow-up cycle when it lands during an in-flight drive; after a successful chat-bar POST, the web invalidates `['autonomy-status']` immediately while preserving the 4-second fallback poll. Expected visible new-decision latency is approximately 2–5 seconds. The durable DB directive remains the retry source after a failed decide; no action-whitelist, Hatcher wire, env, route, or `PROTOCOL_VERSION 18` change.
