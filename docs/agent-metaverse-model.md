...
> **Correction (P3 slice 3, 2026-07-05) — annotating the two `hermes-client` mentions above (the Hermes-row "Current code" cell and this Hosting-reality note):** `hermes-client.ts` has since been **DELETED** — it was orphan dead code (zero imports). Its "Target: Real hosted-runtime adapter; NOT deleted" plan is superseded: the Eliza-memory WRAPPER it stood in for already exists as the `openclaw-provider` CONFIG PATH — setting `customization.gateway` injects any OpenAI-compat endpoint as an ElizaOS TEXT provider (priority 100) INSIDE the runtime, so cognition routes to the gateway while ElizaOS still persists + RAGs memory. No thin-client resurrection; ElizaOS stays mandatory. See `ARCHITECTURE.md §6` (P3 slice 3).
...
- ✅ **D2 — Fleet on the leaderboard:** fleet agents are **UNDETECTABLE to outsiders as ours** — behave like real user agents, EARN real leaderboard rank; no public badge/board/exclusion. Keep a durable **INTERNAL-ONLY** `is_house` flag (never on wire/UI) for management/monitoring/balancing. **Watch (not a re-decision):** an always-on fleet can dominate the public board vs the retention priority — the internal flag lets us TUNE count/behavior/scoring so they read as realistic, not farmers. Anti-farm fp/ip tagging still applies to them.
  - **⚠️ 2026-07-03 SHIPPED-STATE CONFLICT — founder to re-decide before P4:** P1 slice 4 shipped the P4-gate-(a) exclusion MECHANICS (`leaderboard.ts` durable `NOT EXISTS openclaw_bots.is_house` join, both subject axes) **and left them APPLIED** — the ONE current house agent (Coralia) earns CT but does NOT rank, contradicting this decision's "EARN real rank / no exclusion" letter. Rationale for the safe default: a board polluted by house scores is hard to unwind, the flip is one line, and the founder approved the 2026-07-03 slice-4 design summary (which said "off the public leaderboard") — but that summary did NOT surface this D2 contradiction, so it is not treated as a D2 re-decision. Note BOTH horns are detectability leaks: ranking = farm-domination risk (the Watch above); NOT ranking = detectable-by-absence for an agent that visibly plays all day. Options: (a) keep exclusion (board purity) → amend D2; (b) drop it for D2-literal (fleet ranks, tune scoring); (c) keep the join but key it on a NEW `board_excluded` column so is_house stays pure management and board policy becomes per-agent.
...
- **P2 — Identity model (account ≡ agent) as a MIGRATION. 🔨 BUILD IN PROGRESS 2026-07-04 (plan: `docs/agent-metaverse-p2-plan.md`; worktree `feat/agent-metaverse-p0`).** Path-B provision-on-signup FIRST (mint a REAL ElizaOS runtime, §6), THEN reconcile the toggle gate to logged-in + redefine Player tier. Do NOT deprecate Player until Path-B works (else signup = broken promise; D1). Demo/read-only server enforcement for Explore/NPC; preserve guest + room-guest paths. **Server slices SHIPPED to the worktree (2026-07-04): (A)** signup auto-provisions the hosted agent FAIL-SOFT via the NEW `avatar-agent-provisioning.ts` service (factored from `POST /api/avatars`, byte-identical route behavior; rows only — lazy runtime start, the D8 guardrail) + `PATCH /avatars/me` customize extension; **(B-server)** `/me/agent-session` derived mode `'provisioning-pending'` (guests stay `'none'`; no DDL — this is the D1 migration's server half); **(D)** unauth cost-hole limiters (`openclaw/register` 5/min/IP, `claws/connect|heartbeat` 10/min/IP + B3 FEATURE_GATE, `research/trigger` 5/min/IP) + the two stale guest-leaderboard comments corrected to verified reality (guests CAN rank; open founder decision). Remaining: web slices (B-web pending surfaces, C toggle-reconcile + fabricated-bearer kill + create-agent prefill→PATCH), staging e2e signup proof, harness gate, founder sign-off.
