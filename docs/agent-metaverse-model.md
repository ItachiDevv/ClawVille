...
- **P0 — Lifecycle truth (the real smallest first diff). 🔁 RE-FOUNDED on v7 2026-07-01 (`feat/agent-metaverse-p0`, base `origin/staging` @ `7ff0d793`; the prior `feat/agent-metaverse-build` v3 attempt was 818 commits stale → discarded). BUILD IN PROGRESS — gates pending, see below.** ONE lifecycle authority: `session-status` (now RAM+DB, `agent-gateway.ts`), bearer gate (RAM+DB TTL, `require-auth-or-agent.ts:91`), boot-rehydration of the body registry + session map from live-TTL `openclaw_bots` (NEW `agent-session-rehydrator.ts`, awaited at boot), `/disconnect`, and the sweeper (`openclaw-session-sweeper.ts`, now removes the in-world body) ALL agree — fixes restart-desync at the source. Rehydrated sessions mint a fresh server-side sessionId (bearer never persisted) + are fail-closed NON-LEDGER (real-CT only after `/reconnect`); `/connect` eviction widened to unconditional so a reconnect cleanly replaces a rehydrated body (no double body); TTL-live-but-RAM-absent AND NON-restorable (real-gateway openclaw/custom) → 410 `session_not_live`; restorable rows (hatcher-proxy/nanoclaw/milady/anonymous) self-heal their exact bearer via lazy `restoreAgentSessionFromRow` at the gate → stay `connected:true`, NO reconnect (reuses 410, no `PROTOCOL_VERSION` bump). The boot rehydrator restores the BODY (provisional); lazy restore recovers the remote BEARER — two complementary halves. No autonomy engine yet. Scoped constants (D-4): `movement.ts:15` autonomous-avatar clamp 5120→**22528** (v7 canonical world dim — the design-doc's 18432 target was STALE; this is a LIVE latent-bug fix, NOT dead-path: `stepMovement` runs every bridge tick and its clamp pinned autonomous avatars to [16,5104] while `findPath` routes them across the real 22528 world, so town-center buildings at ~11264 game-px were unreachable); `avatars.ts` heartbeat cap ALREADY correct in v7 (uses `WORLD_PX_WIDTH`=22528 — no change needed); pathfinding grid grown 360→**704** (full 22528px world, derived drift-proof from `WORLD_COLLIDER_MAP_HALF*2/TILE`) — a REAL bug fix, NOT the cosmetic deferral the design doc assumed: it flips the regress-auditor's D3 harness baseline (enter_building on messaging-channels, whose center is tile 482 / game-px 15200 — entirely outside the old 360 grid → findPath clamped to the edge → null) to PASS, and covers every building + parcel. The design-doc's "360→576" was itself stale (the world grew 576→704 on 2026-06-24). **REMAINING GATES: mock-Hatcher harness green before/after, Codex adversarial review, restart-survival live proof.**
