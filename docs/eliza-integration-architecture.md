...
| Evaluators | Not used |
| Autonomy self-loop | Custom stub in `SimulationRuntime` — LLM picks from 4 pet-sim actions, not the standard ElizaOS loop |
...
### SimulationRuntime (Autonomous Pets)

`packages/agent-runtime/src/simulation/simulation-runtime.ts` is a specialized wrapper that hosts autonomous pet behavior. It uses ElizaOS Actions and Providers internally, but only for the simulation planning loop — not for chat.

- **4 Actions:** `PET_MOVE_TO_BUILDING`, `PET_VISIT_BUILDING`, `PET_RETURN_HOME`, `PET_SLEEP`
- **1 Provider:** `PET_WORLD_STATE` — formats per-pet position, nearby buildings, visit count
- **Planning:** Bridge calls `planPetNextAction(userId)` → Provider injects context → LLM picks action as structured JSON → handler mutates `PetStateStore`
...
Actions dynamically import `@clawville/database` tables (`pets`, `petInventory`, `bazaarListings`, `quests`, `bounties`, etc.) and use the injected `state.services.db` Drizzle instance to execute queries. This avoids a static import of the database package at the module level.
...
- Existing call sites in `chat.ts`, `pets.ts`, `agent-gateway.ts`, and `openclaw.ts` continue to pass `dynamicContext` and work unchanged. They will be migrated to pass `state` objects in a follow-up cleanup.
...
|---|---|---|
| `apps/api/src/routes/chat.ts` | ~30 lines (visitor pet, shop items, theme, collab) | `PetStateProvider`, `WorldStateProvider`, `InventoryProvider` |
| `apps/api/src/routes/pets.ts` | ~25 lines (token balance, knowledge count, NPC state) | `PetStateProvider`, `KnowledgeProvider`, `WorldStateProvider` |
...
Learned skills go into `characterConfig.knowledge[]` — a flat string array in a JSONB column on the `pets` table. The entire array is prompt-stuffed into every turn via `convertToElizaCharacter()`. This works for 5-10 skills but degrades at 30+ as the context window fills with irrelevant knowledge entries.
...
- **P3:** Skills bought on the bazaar become genuinely useful — the pet applies relevant knowledge per-conversation-turn instead of dumping everything
...
Pet autonomous mode is a thin custom stub. The `SimulationRuntime` handles movement/sleep via 4 hardcoded actions, but the pet's goals are random. There is no goal planning, no spending strategy, no learning progression.

### Solution

Enable ElizaOS's built-in autonomy self-loop on the pet runtime. The loop wakes the pet on a configurable interval and uses the full Provider + Action stack from Phase 1.

### Autonomy Cycle

```
Wake (every 60s, configurable per pet)
  ...
  → Handler executes: pet moves to building, enters
...
- **P4:** Gamified loop where pets autonomously explore, learn, earn, trade — visible in the 3D world
...
External agents connecting via `/api/agent/connect` will get the same Action set as pets. Combined with SKILL.md knowledge from `/api/skills/*` (11 buildings, served from the `building_skills` table), a Milady agent could run this sequence autonomously:
...
| `packages/agent-runtime/src/simulation/simulation-runtime.ts` | Autonomous pet simulation wrapper with 4 actions + 1 provider |
| `packages/agent-runtime/src/simulation/pet-state-store.ts` | In-memory pet state for simulation |
| `packages/agent-runtime/src/simulation/movement.ts` | Pure movement/pathfinding helpers |
| `packages/agent-runtime/src/simulation/actions/pet-move-to-building.ts` | Simulation action: compute path to building |
| `packages/agent-runtime/src/simulation/actions/pet-visit-building.ts` | Simulation action: enter building, set activity |
| `packages/agent-runtime/src/simulation/actions/pet-return-home.ts` | Simulation action: return to spawn |
| `packages/agent-runtime/src/simulation/actions/pet-sleep.ts` | Simulation action: enter sleep state |
| `packages/agent-runtime/src/simulation/providers/pet-world-state.ts` | Simulation provider: per-pet context for LLM planner |
...
| `apps/api/src/routes/pets.ts` | Pet chat (processMessage call site 2) |
...
| `packages/agent-runtime/src/providers/pet-state.ts` | Provider: pet name, species, level, tokens, stats |
...
| `apps/api/src/routes/pets.ts` | Autonomy state endpoint, budget config |
