...
## Public onboarding appearance rejection, 2026-09-23

Staging 13d02c5a passed public identity connect, join, reconnect, and owned skills, then failed appearance with HTTP 403. The coordinator's exact-fixture SELECT found an active non-guest avatar and non-guest owner, with no platform-agent link. Public onboarding supports that nullable link. The new appearance service incorrectly required a hosted platform row for every agent.

The repair accepts exactly null platform linkage. Present links still require an existing same-owner row. The UPDATE preserves its null/link snapshot predicate and all owner, guest, active-avatar, harness, session, and projection guards. No onboarding provisioning or bearer behavior changed. A new public-avatar HTTP regression failed with 403 before the patch. The corrected route supports model, color, gender, and exact live projection. A concurrent null-to-linked change fails the pinned UPDATE. Existing dangling/foreign links remain rejected.

The smoke probe now distinguishes tools discovery HTTP errors from appearance PATCH HTTP errors without printing response bodies or credentials. Local focused tests passed 28 tests and 106 assertions on Bun 1.3.11. The independent reviewer passed 33 tests and 144 assertions across appearance, smoke, and executor suites. API typecheck passed. These tests mock the database; live acceptance remains the coordinator's new-SHA gate. Protocol 69 already advertises bound-avatar appearance, so this restores its stated contract without new verbs, parameters, or world-scope knowledge. PARITY: human appearance behavior remains; public connected and hosted agents use the same exact-avatar service.
