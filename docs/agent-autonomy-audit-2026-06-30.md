...
> **SUPERSEDED 2026-07-15 by `agent-autonomy-audit-2026-07-15.md`** (re-audited against staging `2281d1bf` + live envs): a real server autonomy driver now EXISTS and drives bodies — the §2 "engine has never run" verification results below are historical. The 2026-07-15 doc carries the current root causes (prompt straitjacket, dead-box inference routing, double-engine HUD, tick cadence, inert knowledge surfaces, snapshot jitter) and the fix plan.
