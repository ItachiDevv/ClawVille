...
> **Current local protocol: `PROTOCOL_VERSION 41` (2026-07-30).** Version 41
> adds the self-reported `at-kelp` co-presence convention and its "at the Kelp
> Forest" client tag alongside version 40's `at-cove` convention, without
> changing the world wire. Version 39 added
...
*`PROTOCOL_VERSION 38->39->40->41` added 2026-07-24–30: v39 documents the
...
including `at-cove` and its "at the Cove" client tag. v41 adds `at-kelp` and its
"at the Kelp Forest" client tag. The public
...
only the manual version/hash advances. The v41 staging partner harness passed
on `bfbd7b16` (identity-bound mock client ALL-PASS; contract probe 7/7); the test
signer was then removed and the signer-free API redeployed.*
