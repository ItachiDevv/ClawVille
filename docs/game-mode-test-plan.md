...
- **Prerequisite**: Logged-in user with a created avatar (e.g. TokenTestBot)
...
| Avatar status bar | Hidden | |
| Avatar chat bar | Hidden | |
...
6. Verify bottom-left has NO avatar status bar or avatar chat bar
...
| Feature | Expected | Status |
|---------|----------|--------|
| Player lobster visible | YES — avatar lobster with user's color tint | |
| Camera | FPSFollowCamera follows avatar | |
| WASD keys | Move avatar (screen-relative) | |
| Joystick | Move avatar (screen-relative) | |
| E key near building | Enters building | |
| Click-to-move | Active — click ground to pathfind | |
| Avatar status bar | Visible | |
| Avatar chat bar | Visible | |
...
3. Verify avatar lobster appears in 3D scene
4. Press WASD — avatar moves, camera follows
5. Click on ground — path dots appear, avatar walks to destination
...
| Feature | Expected | Status |
|---------|----------|--------|
| Player lobster visible | YES — same avatar, but autonomy engine drives it | |
| Camera | FPSFollowCamera follows avatar | |
| WASD keys | Disabled (autonomy drives movement) | |
| Avatar navigates independently | YES — visits buildings, learns skills | |
...
2. Verify avatar begins moving on its own
3. Verify Autonomy HUD appears with agent status
4. Verify WASD has no effect on avatar movement
5. Watch avatar navigate to a building and enter it
...
| Connect agent while in Explore | Switches to Play mode, avatar appears |
| Connect agent while in NPC | Switches to Play mode, NPC removed, avatar appears |
| Disconnect agent in Play | Switches to Explore mode, avatar hidden |
...
1. **Avatar UI in Explore mode** — AvatarStatusBar/AvatarChatBar were visible in explore mode. Fixed: gated on `controlMode !== 'explore'` in `game/page.tsx`.
