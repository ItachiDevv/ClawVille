...
### §2.2 — Auto-generated Solana wallets for pets

Following Milady's pattern (`generateWalletKeys()` in `packages/agent/src/api/wallet.ts`), ClawVille should auto-generate a Solana keypair for every connecting agent. The Phase 4 treasury infrastructure is already wired up — we just need to add a **pet custody wallet** pattern alongside the merchant wallet.

**Schema changes:**
- `pets.walletAddress varchar(64) UNIQUE` — base58 public key
- `pet_wallets` table (new, parallel to `treasury_wallets`):
  ```ts
  id uuid PK
  ...
  ```
- OR alternately, reuse `treasury_wallets` with a new `purpose: 'pet-custody'` enum value and a nullable `pet_id` column. **I lean toward a separate `pet_wallets` table** because the access patterns are different (pets query by their own id, treasury queries by purpose) and it keeps the treasury table focused on merchant/fee-collector/escrow purposes.
...
1. **On pet creation** (via the existing `POST /api/pets` endpoint for human players)
...
3. **Lazy backfill** — a migration script that generates wallets for existing pets that don't have one yet
...
**Schema addition:**
- `pets.avatarType enum('glb', 'vrm') default 'glb'`
- `pets.avatarUrl text` — URL to the model file (currently implicit based on species + color lookup)
- `pets.vrmMetadata jsonb` — Expression map, animation names, bone mapping

Rendering work (Phase 5, probably needs the 3da subagent):
- `World3DCanvas.tsx` checks `pet.avatarType` and dispatches to either the existing lobster GLB or a new VRM renderer
...
| `apps/api/src/services/milady-identity.ts` | NEW — `resolveMiladyAgent()`: takes the inbound metadata, creates or looks up a stable pet identity, returns the canonical `agentId`. No external API call needed (ClawVille is the identity provider, per answer 5). | 100 |
...
#### Phase C2 — Auto-generated Solana pet wallets (~300 lines)

| File | Change | ~Lines |
|---|---|---:|
| `packages/database/src/schema/pet-wallets.ts` | NEW — `pet_wallets` table (mirrors `treasury_wallets` shape, keyed on pet_id) | 40 |
| `packages/database/src/schema/pets.ts` | Add `walletAddress varchar(64) UNIQUE` column to `pets` table | +2 |
| `packages/database/src/schema/index.ts` | Register new `petWallets` export + relation | +3 |
| `apps/api/src/services/pet-wallet-service.ts` | NEW — `generatePetWallet(petId)`: calls `Keypair.generate()`, encrypts secret via `keypair-vault`, inserts into `pet_wallets`, updates `pets.walletAddress` | 80 |
| `apps/api/src/routes/pets.ts` | Call `generatePetWallet()` on pet creation if the pet has no wallet | +20 |
...
| `scripts/backfill-pet-wallets.ts` | NEW — one-time script that finds all pets without a wallet and generates one for each | 60 |
| `apps/api/src/routes/pets.ts` | Add `GET /api/pets/:id/wallet` — returns pubkey + balance (not the secret) | +40 |
| `bun run db:push` | Apply schema change | — |
...
| File | Change | ~Lines |
|---|---|---:|
| `packages/database/src/schema/pets.ts` | Add `avatarType varchar enum('glb','vrm')`, `avatarUrl text`, `vrmMetadata jsonb` columns | +5 |
| `packages/shared/src/types/pet.ts` | Add fields to the `Pet` type + a `PetAvatarConfig` helper type | +20 |
| `apps/api/src/routes/pets.ts` | Accept `avatarUrl` in the create/update endpoints | +10 |
...
| File | Change | ~Lines |
|---|---|---:|
| `scripts/test-milady-plugin-flow.ts` | NEW — mimics what the Milady plugin's `resolveLaunchSession` will do. POSTs to `/api/agent/connect` with `identityType: 'milady'`, asserts a pet wallet was generated, calls `/move` and `/visit-building`, disconnects. | 80 |
...
- **C2 (pet wallets) is independent of Milady.** Any pet gets a wallet; Milady agents just happen to get one via the new identity branch.
...
2. **Legal review on custodial pet wallets** — do you have any existing legal guidance on holding user keys, or is this a net-new concern I should flag publicly in the code as "custodial — do not store real value until legal review"?
...
- **Phase C2 coding** (pet wallet service + schema). Mechanical, reuses existing `keypair-vault.ts`.
