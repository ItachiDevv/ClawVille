...
**Last edit:** 2026-06-15 (perf/round2-steady-state): **O(1) terrain heightfield** replacing all per-frame NPC + player terrain raycasts — the #1 JS CPU consumer on prod (~57% of JS CPU in a real prod trace: `intersectTriangle + _computeIntersections + getX/Y/Z + Vector3 dot/cross`). New module `terrain-heightfield.ts`: `initTerrainHeightfield(geo)` snaps the 121×121 displaced vertex heights from `PlaneGeometry` into a `Float32Array` singleton after `createSandGeometry()` finishes; `getTerrainHeightAt(worldX, worldZ)` returns world Y in O(1) via bilinear interpolation (4 array reads + 7 multiplications + 3 additions, zero allocations). Coordinate derivation: `localX=worldX`, `localY=-worldZ` (−90°X rotation), `u=(localX+halfW)/(halfW*2)*(cols-1)`, `v=(halfH-localY)/(halfH*2)*(rows-1)`, `worldY=bilinear(localZ)-2` (plane `position={[0,-2,0]}`). Verified: all 11 exact-vertex test points show diff=0.000e+0 vs formula reference; 7 typical NPC positions all `in_range=OK`. Files changed: `terrain-heightfield.ts` (new), `arena-terrain.tsx` (init call), `arena-npcs.tsx` (removed `_raycaster/_rayOrigin/_rayDir` + `findTerrainMesh()`), `arena-location-npcs.tsx` (removed `_locRaycaster/_locRayOrigin/_locRayDir` + `findLocTerrainMesh()`), `player-avatar.tsx` (removed `_avatarRaycaster/_avatarRayOrigin/_avatarRayDir/_cachedAvatarTerrainMesh` + `findAvatarTerrainMesh()` + unused `TERRAIN_LAYER` import). §5c allocation table updated (raycaster entries removed); §5d throttle table updated; §7 terrain section updated.

**Prior Last edit:** 2026-06-15 (perf/round2-steady-state): `arena-buildings.tsx` `EditableBuilding` useFrame: added settle-once guard (`settled` ref). Before: `intersectObjects(threeScene.children, true)` ran every frame per edit-mode building (~345k triangle-ray tests/frame across 10–12 buildings × 28,800-tri terrain). After: raycast runs only until first valid terrain hit, then `settled.current=true` skips all future raycasts. Position update still runs every frame so dragging stays responsive. `3dStructure.md §5d` throttle table updated. Pattern mirrors NPC single-mesh terrain-snap optimization. No visual change; startup/drag behavior identical.
...
- `arena-npcs.tsx` — raycaster scratch objects REMOVED 2026-06-15; terrain height now via `getTerrainHeightAt()` (O(1) heightfield, zero allocation)
- `arena-buildings.tsx:37-40` — `_buildRaycaster, _buildRayOrigin, _buildRayDir` (used by `EditableBuilding` settle-once snap only)
- `arena-location-npcs.tsx` — raycaster scratch objects REMOVED 2026-06-15; terrain height now via `getTerrainHeightAt()`
- `player-avatar.tsx` — `_avatarRaycaster, _avatarRayOrigin, _avatarRayDir` REMOVED 2026-06-15; terrain height now via `getTerrainHeightAt()`
...
| Terrain height (NPC + player grounding) | **REPLACED by O(1) bilinear heightfield** (2026-06-15, perf/round2-steady-state). Old path: downward raycast against the 28,800-tri terrain mesh every 3rd frame per NPC — no BVH, linear triangle scan — confirmed ~57% of JS CPU in a prod trace. New path: `getTerrainHeightAt(worldX, worldZ)` from `terrain-heightfield.ts` — 4 array reads + bilinear, zero allocations, zero triangle tests. `initTerrainHeightfield(geo)` runs once in `createSandGeometry()` after the `pos.setZ()` loop completes. **Edit-mode buildings (`EditableBuilding`):** still use a ONE-TIME raycast via the settle-once guard (`settled` ref) — fires only until first valid terrain hit, then stops forever. Cost ≈ 0 at steady-state. | `terrain-heightfield.ts` (new), `arena-npcs.tsx`, `arena-location-npcs.tsx`, `player-avatar.tsx`, `arena-buildings.tsx:EditableBuilding` |
...
**Terrain height (NPC + player grounding) — O(1) heightfield since 2026-06-15:** `terrain-heightfield.ts` exports `initTerrainHeightfield(geo)` (call once at end of `createSandGeometry()`) and `getTerrainHeightAt(worldX, worldZ)` (O(1) bilinear lookup). The heightfield reads the same `pos.getZ(i)` displaced vertex data that Three.js rasterises, so its output matches the old per-NPC downward raycast to floating-point precision. Confirmed via 11 exact-vertex diff=0.000 comparisons + 7 typical NPC positions all `in_range=OK`. The terrain mesh still sets `layers = TERRAIN_LAYER (=1)` for the `EditableBuilding` settle-once raycast (buildings are static; one raycast per building at first mount, never repeated).
