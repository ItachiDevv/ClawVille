...
**Cache-bust on GLB swap:** every replacement of the served interior GLB MUST bump `?v=N` in the `INTERIOR_GLB` URL (currently `/models/cove/cove-interior-cleaned-v1-mo-ktx.glb?v=2`; the landing `CoveVignette.tsx` serves `cove-interior-cleaned-v1.glb?v=6`). Cloudflare edge cache TTL is 7 days, deploy token has no `cache_purge` scope, and the service worker caches assets cache-first by full URL. See §6f rule 9.
