...
- 2026-06-10 (later) — `AdaptiveRendererDpr` DELETED: root cause of the staging blue-until-resize bug. Direct `gl.setPixelRatio()` ~0.5s post-first-paint reconfigured the WebGPU swapchain outside R3F's resize path. Dead defensive code (tier-4 DPR clamp it guarded against was already removed). New invariant: never call gl.setPixelRatio/setSize directly on the WebGPU backend after first paint.
