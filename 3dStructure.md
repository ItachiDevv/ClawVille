...
- 2026-06-06 — Perf asset wave 2 (building + pavilion compression, `compress-glb-targeted.ts` slot-aware): 6 live `-opt1` buildings recompressed (color→lossy WebP q92, normal/MR→**lossless** WebP, meshopt geometry) — **arcade 4.20MB→734K (-83%)**, chum 1.83→1.48, krusty 1.50→0.98, squidward 1.24→1.04, patricks 1.22→0.95, cove 0.38→0.12 (~5MB total); `quest-bounty-pavilion.glb` geometry meshopt **4.68MB→2.11MB (-55%)** (textures already WebP, untouched). `?v=` bumped on all refs (`arena-buildings.tsx` + `asset-preload-manifest.ts` + `sw.js` precache); SW v7→**v8**. Each structurally re-validated (decode + mesh/tex counts intact; meshopt skips UV>1 tiling coords). **Pixel-verify on staging pending.**
