...
**Prior Last edit:** 2026-06-19 (town-ux: cove discoverability beacon + proximity prompt, directional fingerpost, spawn scatter, cove entrance animation). See §15.1–15.4 below.
...
## 14. Land FOR-SALE sign system (2026-06-18)

**Files:** `packages/shared/src/constants/land-signage.ts` (NEW) · `packages/shared/src/index.ts` (updated) · `apps/web/src/lib/three/land-parcels.tsx` (reworked) · `apps/web/src/lib/three/land-showroom.tsx` (simplified)

**Sign model — 3 categories:**

| Category | Applies to | Plank W×H (wu) | Plank D | Post H | Canvas | Texture design |
|---|---|---|---|---|---|---|
| `regular` | b / c / starter tiers | 68 × 28 | 2.5 | 52 | 256×64 | Dark `#0a1520` bg, `#c9b48a` single border, "FOR SALE" white mono |
| `premium` | founder + a tiers | 92 × 38 | 3.0 | 64 | 256×64 | Gold `#ffd54a` double border + corner ticks, "FOR SALE" white + "PREMIUM" gold subtitle |
| `premium-partner` | 6 curated partner lots | 116 × 48 | 3.5 | 76 | 256×80 | Cyan `#9fe9ff` ornate: double border + filled topper bar (10px) + dots, "FOR SALE" white + "PARTNER" cyan subtitle |
...
**Coverage:** signs render on ALL 180 plots including showroom lots. The old `SHOWROOM_PARCEL_IDS` guard (which suppressed signs on ~22 showroom lots to avoid overlap with the old `ShowroomSign` component) is REMOVED. The `ShowroomSign` component, its textures, and its material ref-count singleton (`_signMats` / `_signTextures` / `_signMatRefCount`) have been deleted from `land-showroom.tsx`.

**Draw-call budget:** 5 body (pad+posts+rails per tier) + 3 plank (per category) + 3 post (per category) = **11 total** (was 7). Net +4 from splitting 1 post+1 plank into 3 post+3 plank.

**Disposal:** all 3 CanvasTextures + MeshBasicMaterials + MeshStandardMaterial signPostMat + 5 tier MeshStandardMaterials disposed in the `useEffect` cleanup. Geometries disposed after `mergeGeometries` builds them.

**Iris Xe / WebGPU constraints:** no drei `<Text>`/`<Billboard>`, no `InstancedMesh+ShaderMaterial`, no per-frame allocations — all scratch Matrix4 objects are module-scope; signs are static, NO `useFrame` in `land-parcels.tsx`.
