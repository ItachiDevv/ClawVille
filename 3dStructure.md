...
**Last edit:** 2026-06-10 (`/game` mobile jump input): `jump-state.ts` now exposes `setJumpPressed(pressed)` so `<MobileControls>` can feed the same charge/release/quicksink state machine as keyboard SPACE. No physics constants changed; only the input source expanded.
