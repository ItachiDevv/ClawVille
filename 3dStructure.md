...
**Last edit:** 2026-06-18 (Land showroom — decorative FOR RENT model buildings on ~15 outer starter lots). See §13 for detail.
...
**Prior Last edit:** 2026-06-17 (jump animation BUG 1+2 fix + magnitude correction). **BUG 1 (midair squat):** `'squat'` removed from `IN_PLACE_CLIPS` — hip-Y descent track now survives retarget. **MAGNITUDE CORRECTION:** the retargeter keeps ABSOLUTE hip-Y (~0.89 VRM-m for Milady at standing frame), not zeroed; correct formula is `(restHipY − currentHipY) × fitScale` in world units. `_squatRestHipY` captured at squat-clip lazy-load time. `getSquatGroundLift(t, fitScale)` updated: `fitScale` = `vrmRenderScale` (~168.75 wu/m for 270wu Milady) — omitting it under-compensated by ~150×. Each call site passes its own scale. Verified: standing frame = 0 wu; deepest squat ≈ 44 wu. **BUG 2 (foot-slide):** `jumpState.chargeMode` (`'squat'|'run'|'none'`) added to `jump-state.ts`. Walk/idle entry → halt WASD + squat wind-up; run entry → keep running, skip squat. §6z §7a updated.
...
**7a. Jump charge motion rules (2026-06-17).**
...
- **Idle/walk → squat charge (`chargeMode === 'squat'`):** horizontal velocity zeroed, surfaceClip = `'squat'`, `group.position.y` lifted by `animator.getSquatGroundLift(animator.getSquatClipTime(), vrmRenderScale)` BEFORE `animator.update()` so boneMatrices are uploaded at the corrected Y. Visual result: pelvis lowers toward planted feet.
...
- **Squat clip foot-grounding mechanism (corrected 2026-06-17):** `'squat'` was removed from `IN_PLACE_CLIPS` in `vrm-character-animator.ts`. The retargeter's hip-Y track is preserved. `VRMCharacterAnimator.getSquatGroundLift(t, fitScale)` samples the track at time `t` and returns `(restHipY − currentHipY) × fitScale` in world units. Critical correctness notes: (1) The retargeter keeps ABSOLUTE hip-Y (e.g. ~0.89 VRM-meters for Milady at standing frame), NOT zeroed at rest — `−currentHipY` would give negative lift because all values are positive; the correct formula uses relative descent from rest. (2) `fitScale` = `vrmRenderScale` (~168.75 wu/m for a 270wu Milady) converts VRM-meter descent to world-unit lift; omitting it under-compensates by ~150×. (3) Each call site passes its OWN scale: player-avatar.tsx passes its `vrmRenderScale`; arena-npcs.tsx VRMNpcMesh passes its own `vrmRenderScale` from `computeVRMNpcScale`. Verified: standing frame lift = 0 wu; deepest squat ≈ 44 wu for Milady 270wu (~30% hip descent). The 2026-05-22 approach (strip positions → rotation-only knees → feet float up) is replaced.
