...
**Last Audited:** 2026-05-22 — Jump pre-launch glitch fix: added `'squat'` to `IN_PLACE_CLIPS` in `vrm-character-animator.ts` (was missing — Mixamo squat bake animates hip Y track downward then snaps at loop boundary, dropping the rendered body below the floor each loop while setSurfaceClip kept it as the charging-phase clip). With positions stripped, the squat is rotation-only — knees bend, feet stay flush at terrainY. Also removed the broken runtime foot-anchor block in `player-avatar.tsx` PlayerAvatarVRMInner (was reading `footNode.matrixWorld.elements[13]` BEFORE the same-frame `animator.update()`, so it always read the previous frame's pose — turned the correction into a one-frame-lagged feedback loop that diverged on every squat loop boundary). User-reported screenshots showed the avatar's head barely above the sand on one frame and a normal crouch above ground on the next during SPACE-hold charging. Root cause was the position-track preservation; the foot-anchor was a downstream band-aid that itself broke. §6f drift note: charging-phase surfaceClip='squat' now satisfies the IN_PLACE invariant alongside run/swimming/flying/praying/wipeout/kip_up.

**Prior Last edit:** 2026-05-22 — Per-GLB collision rework + walkable surfaces: `Collider2D` gains `walkable`/`topY`; `clampMovement2D` returns `groundY`; player/NPC Y-lift via `effectiveFloorY = Math.max(terrainY, walkableY)`. Shisha-oasis split to two zones (walkable outer approach halfX=348/halfZ=340/topY=38 + solid inner kiosk halfX=200/halfZ=195) at corrected mesh center (1178,−240). All call sites pass entity half-width. Total: 19→20 colliders. `packages/shared` `ServerCollider2D` gains `walkable`/`topY`. §2h updated.
...
```
zone tile extent  = 14 × 32 = 448 wu (full side)
half-extent       = 224 wu
BUILDING_SCALE_FACTOR = 0.92  (AABB cutover 2026-05-21; was 0.85 disc)
BUILDING_HALF     = 224 × 0.92 ≈ 206 wu
```
