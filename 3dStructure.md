...
| **NPC locomotion pairing (settled 2026-06-10)** | Server NPCs walk 220 wu/s (`moveNpcs` baseStep 44 @5Hz — small ticks the client lerp absorbs smoothly); walk clip plays at timeScale 1, which visually matches ~220 (user-confirmed in demo-wander). A velocity-synced timeScale + a 550 wu/s server raise were both tried and REVERTED same day: big ticks glitch the interp, and syncing legs to broken rendered motion makes stalls look worse. Perceived stride/speed mismatch in deployed builds = world-stream interp stalls (rendered speed << server speed) — fix the interp, never these constants. | `arena-npcs.tsx:VRMNpcMesh`, `apps/api/src/services/npc-simulation.ts:moveNpcs` |
