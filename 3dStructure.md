...
| Cost | Throttle | Where |
|---|---|---|
| **Wandering NPC render count** | No active count cap. The prior low-end cap was removed 2026-05-22 per user direction because it hid world activity. Current perf work must use cheaper assets/animation LOD/impostors rather than silently dropping NPCs. | `arena-npcs.tsx:ArenaNpcs` |
...
|---|---|---|---|
| **1 — critical** | Immediately | 11 building GLBs + procedural Sandy, 3 locomotion GLBs, 8 wandering NPC GLBs, 4 town-prop GLBs, 6 wandering VRM bytes | Parallel with canvas chunk download |
...
- 2026-05-25 — Emergency perf round 3: `arena-npcs.tsx` swaps high-poly wandering sea-creature GLBs plus chibi/Hermes/Tekk wandering VRMs to low-poly procedural `MeshBasicMaterial` proxies. Local production measurement dropped visible render triangles from ~625k to ~361k before the chibi add; live staging then exposed two chibi VRMs at ~97k tris each and they joined the procedural path. Wanderer count, labels, movement, and collision are preserved.
