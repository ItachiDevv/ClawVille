...
## 12. Cosmetic render pipeline (Phase 3.3 → Phase B — 2026-04-28, updated 2026-06-07)

Single file: `apps/web/src/lib/three/cosmetic-loader.tsx`. Equipment slots that overlay on the player avatar:

| Category | Strategy | Phase B status |
|---|---|---|
| `hat` | Attached to raw VRM head bone via `computeCosmeticHeadFit()` (proportion-aware, axis-sign-safe). Legacy fallback = `findHeadBone()` name scan for non-VRM rigs. assetMeta `offsetXYZ`/`scaleHint`/`rotationXYZ` applied on top. | Live |
| `glasses` | Same as hat but placed at eye level (25 % below head-top) + forward offset along body-facing direction. | Live |
| `aura` | GLSL ShaderMaterial Fresnel+pulse sphere around avatar root. ShaderMaterial (not NodeMaterial) — R3F main world canvas uses WebGLRenderer; NodeMaterial causes per-frame crash. `compileAsync` on mount. | Live |
| `particle` | `emitParticles()` at throttled rate from avatar world position. | Live |
| `board` | GLB prop at player pos, context-gated to `activity:reef-race`. | Live |
| `palette` | **Stub (Phase B guard):** UV-region canvas-composite blit not implemented. Returns null. No material mutation until full implementation. | Stub |
| `outfit` | **Stub (Phase B guard):** Marvelous Designer skin binding not implemented. Returns null silently. | Stub |

**`computeCosmeticHeadFit(vrm, category, renderScale)`** (in `vrm-avatar-sizing.ts`):
1. Gets raw VRM head bone via `humanoid.getRawBoneNode('head')` — raw bone is what the animator drives.
2. Builds WORLD-space AABB by traversing head-bone subtree, skipping `SkinnedMesh` nodes (bind-pose bbox unreliable).
3. Falls back to top-15 % of body bbox if no non-skinned geometry found (chibi, low-poly).
4. Derives world-space target: hat = head-top + HAT_CLEARANCE_WU (2 wu) × localScale; glasses = eye-level (0.25 from top) + forward GLASSES_FORWARD_FACTOR (0.5 × headWidth) along `vrm.scene.getWorldDirection()`.
5. Converts to head-bone-LOCAL via `invBoneMat = headBone.matrixWorld.clone().invert()` — axis-sign safe, handles VRM0/VRM1/Mixamo/VRoid conventions.
6. `localScale = clamp(headWidthWU / 30, 0.25, 4.0)` (30 wu = `COSMETIC_REF_HEAD_WIDTH_WU`).

Called at EQUIP TIME (effect setup), never per frame. All Vector3/Matrix4 are local to the call — no per-frame allocation (Iris Xe invariant).

**Player avatar wiring** (`player-avatar.tsx`): `rigType='universal'`, `vrm={vrm}`, `vrmRenderScale={vrmRenderScale}` passed to `<CosmeticLoader>`. Raw VRM instance (from `useVRMInstance`) and its render scale (from `computeVRMAvatarFit`) are threaded through.

Draw-call budget (full equipped set): hat ≤ 1, glasses ≤ 1, aura = 1 (ShaderMaterial sphere, safe), particle = 0 (pool), board ≤ 1.
...
- 2026-06-07 — Phase B cosmetic fit: `computeCosmeticHeadFit()` added to `vrm-avatar-sizing.ts` (proportion-aware, axis-sign-safe hat/glasses placement via inverse-world-matrix; skips SkinnedMesh bbox; fallback to body-bbox fraction for low-poly VRMs). `cosmetic-loader.tsx` `HatOrGlassesRenderer` refactored to use auto-fit when `vrm`+`vrmRenderScale` present (legacy bone-name path preserved as fallback). `player-avatar.tsx` `<CosmeticLoader>` updated: hardcoded `rigType="milady-vrm"` → `"universal"`, added `vrm={vrm}` + `vrmRenderScale={vrmRenderScale}`. `OutfitRenderer`/`PaletteRenderer` guarded off (return null, no-op stubs were emitting console.info per frame). §12 rewritten.
