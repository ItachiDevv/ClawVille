...
| **Velocity-synced locomotion playback** | (2026-06-10 "reverse moonwalk" fix.) Walk/run clips' natural ground speed = player 550 wu/s (`NPC_CLIP_GROUND_SPEED_WU_S`; run ×1.5). Server NPCs translate at 220 wu/s → legs at timeScale 1 slid 2.5×. `VRMNpcMesh` accumulates rendered distance/time over 0.4s windows → `animator.setLocomotionTimeScale(speed/ref)` (clamped [0.3,1.8], deduped, survives crossfade `reset()` AND the one-shot emote return path — both re-apply it; idle/surface clips never scaled). Windowed (not per-frame) because demo-wander steps + 5Hz interp make frame deltas spike/zero and bias the scale low. Change player SPEED ⇒ update the constant same diff. | `arena-npcs.tsx:VRMNpcMesh`, `vrm-character-animator.ts:setLocomotionTimeScale/applyCrossfade` |
