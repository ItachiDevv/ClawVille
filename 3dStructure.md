...
| Segments | 120 × 120 |
| Material | `MeshStandardNodeMaterial` (TSL) — desktop/WebGPU; plain `MeshStandardMaterial({ vertexColors:true })` on iOS/forceWebGL |
| Render Y | -2 |
...
**iOS/forceWebGL:** `MergedSeaweed` is not mounted when `FORCE_WEBGL` is true. The TSL `positionNode` wind animation compiles to per-vertex GLSL loops on the WebGL2 backend; 4500 blades spike the A-series GPU budget on first draw. The blade count in the table above reflects desktop only.
...
**Hotspots:** `GAMEREADY_HOTSPOTS` — 4 invisible boxes at cabinet faces. `HotspotDef` shape: `{ position, size, machineSlug, paytableId, isBonus }`. Click → `openSlotScreen(def.machineSlug, def.paytableId, avatar.clawTokens)`. Cabinets 0+1: `machineSlug:'classic-3x5'`, `paytableId:'classic-3x5'`, `isBonus:false`. Cabinets 2+3: `machineSlug:'classic-3x5-bonus'`, `paytableId:'classic-3x5-bonus'`, `isBonus:true`. `FALLBACK_HOTSPOTS` — 2 boxes for cartoon GLB path (both classic, isBonus:false).
...
- 2026-05-21 — Firefox crash fix: probe canvas releases WebGL2 context; glFactory catch uses WebGPURenderer(forceWebGL:true) on fresh canvas instead of plain WebGLRenderer from 'three' (dual-instance trap).
- 2026-05-20 — iOS double-load root-cause: removed early-return in game/page.tsx that tree-swapped SeaLoadingScreen during auth fetch. Canvas + single loader now always mounted.
- 2026-05-20 — iOS double-load + lag fix: (1) sw.js CACHE_VERSION v3→v4 (bust stale cc26908 chunks); (2) kickRenderLoop __W3D delayed to post-first-frame RAF (was set in onCreated before shaders compiled); (3) MergedSeaweed gated !FORCE_WEBGL (TSL wind animation stalls iOS A-series); (4) SandFloor plain MeshStandardMaterial fallback on iOS (TSL NodeMaterial 3-channel compile spike). §7 + §8 updated.
