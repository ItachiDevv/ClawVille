...
**Last edit:** 2026-06-07 (combined perf integration): merged the `perf2` adaptive/runtime quality-tier pass with `feat/perf-cpu-framebudget` CPU/assets work. Runtime rules now include the perf2 adaptive governor, tier-4 building/resident proxies, HUD collapse, moving-avatar cap, and DPR emergency clamp, while `arena-npcs.tsx` also keeps the CPU branch's once-per-frame `getNpcFrameShared()` cache for `controlMode` + camera position. Conflict resolution preserved the multiplayer remote-player guard: GLB/VRM NPC entity-vs-player push-out still checks `!d.isRemotePlayer` so remote players do not diverge per client. Asset pipeline also includes texture-only building recompression, bazaar stand compression, quest-bounty pavilion compression, SW cache updates, and the documented rule that building GLBs flowing through `mergeStaticMeshesByMaterial` must not be meshopt-quantized. See `docs/perf-integration-change-ledger-2026-06-07.md` for rollback candidates, especially quality-reducing tier-4 features like DPR clamping and visual proxies.
...
- 2026-06-06 — Perf2 adaptive/runtime pass: default `/game` gets adaptive quality flags; tier 4 can collapse HUD chrome, clamp DPR to 0.4, proxy far residents and all building GLBs, and reduce moving full-detail NPC/remote-player budget to 2. `?fast=1` locks tier 4 for deterministic measurement. Quality-risk features are intentionally marked in the integration ledger.
