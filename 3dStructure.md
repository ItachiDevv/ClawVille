...
| 2 | ENE (60°) | 293 | 115 | mcp-tool-use | −1.049 | 1400 | childScaleOverrides: `The_Krusty_Krab` ×1.5; bodyAnchorChild: `The_Krusty_Krab` (runtime GLTFLoader key, verified 2026-05-25) |
...
**childScaleOverrides** (added 2026-05-18): `Record<literalRuntimeNodeName, multiplier>` applied to named Object3D child nodes AFTER `computeBuildingScale` but BEFORE `mergeStaticMeshesByMaterial`. Keys must come from a runtime `GLTFLoader`/CDP dump, not raw GLB string inspection. Verified active keys: `Squidward's_House` and `The_Krusty_Krab`. The override multiplies the child's local `.scale` so the differential is baked into merged vertex positions. Use when a GLB bundles a large-footprint pathway or sign that compresses the building body via the max-dim normalizer. `targetMaxDim` sets the overall baseline; `childScaleOverrides` lets the building body punch above that baseline independently.
...
- `mcp-tool-use` (`krusty-krab-v2.glb`): `The_Krusty_Krab` × 1.5 childScaleOverride; `bodyAnchorChild: "The_Krusty_Krab"` — restaurant center aligns to slot, sign/pole extends forward. Runtime browser verification on 2026-05-25 showed the previous `The Krusty Krab` key did not resolve.
