...
**Last edit:** 2026-06-08 (VRM parse queue local recovery pass): `vrm-loader.ts` now queues GLTFLoader VRM parses one-at-a-time instead of letting every visible VRM parse concurrently on first scene mount. This is a fidelity-preserving fix for the blue-screen/readiness failure: Suspense fallbacks stay null while real VRMs stream in progressively, so RAF/requestIdleCallback can keep running and `StaggeredTextureUpload` can reach `__W3D_TEXTURES_READY`. Metrics now include `queueWaitMs` separately from true `parseMs`. This does not add primitive building proxies, DPR clamps, or capsule/cylinder character stand-ins.
...
| **Multiplayer LOD orchestrator** | Historical moving-entity LOD system: `FULL_CAP=14` full-skeleton VRM/GLB cap across wandering NPCs + remote players; low-end/mobile used `ACTIVE_FULL_CAP=8`. It frustum-tested candidates, sorted by camera distance², and demoted entities beyond the cap to cheap capsule proxies. **2026-06-07 playable rollback:** `LodOrchestrator` is no longer mounted in `/game`, `NpcEntry` and `RemotePlayerEntry` always render visible entities as real GLB/VRM models, and the adaptive tier-4 cap-to-2 behavior was rejected. The file remains available for diagnostics/refactor reference only. Future LOD must preserve character silhouettes/identity instead of showing capsule/cylinder stand-ins. Building residents also keep real detail under adaptive tiers. | `lod-orchestrator.tsx`, `arena-npcs.tsx:NpcEntry`, `remote-players.tsx:RemotePlayerEntry`, `remote-player-proxy.tsx:EntityProxyMesh`, `arena-location-npcs.tsx:LocationNpcProxy` |
