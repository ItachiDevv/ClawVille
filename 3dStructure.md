...
- `clampMovement2D(fromX, fromZ, toX, toZ, entityHalf?)` — called once per entity per frame, after position delta is computed, before writing to the scene graph. Returns `{ x, z, hit }`. `entityHalf` defaults to 0 (point entity); pass 25–60 wu for entity body half-width (Minkowski expansion).
- Module-scope scratch scalars `_sCx`/`_sCz` — zero `new Vector3()` or object allocations in the hot path.
...
| Prop (town center) | 7 | 40–280 wu (anisotropic) | Hardcoded from each prop's TSX constants |
| **Total** | **19** | | |
...
**Prop colliders (positions verified from each TSX's exported constants, 2026-05-21):**

| ID | World XZ | halfX × halfZ |
|---|---|---|
| auction-podium | (0, −1000) | 160 × 160 wu |
| town-directory-sign | (0, −120) | 70 × 40 wu |
| bazaar-stall | (−1273, −120) | 180 × 140 wu |
| marketplace-stall | (1273, −120) | 200 × 160 wu |
| quest-bounty-pavilion | (0, −1220) | 280 × 280 wu |
| quest-npc | (−110, −60) | 40 × 40 wu |
| town-guide | (0, 240) | 40 × 40 wu |

(Removed 2026-05-21: `bounty-board` at (50, 0) — superseded by the QuestBountyPavilion.)

**Integration points:**
- `player-avatar.tsx` — VRM branch + GLB branch. Both call `clampMovement2D(prevWX, prevWZ, newX − HALF_W, newY − HALF_H, entityHalf)` then convert back to game-px.
