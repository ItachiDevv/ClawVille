...
**Last Audited: 2026-09-19 (cove GLB cache-bust re-bumped).** Drift note: `INTERIOR_GLB` `?v=2` -> `?v=3` and the landing vignette `?v=6` -> `?v=7`. A prod curl of the new URLs before the web flip made Cloudflare cache the OLD bytes under `?v=2` / `?v=6`, so players would still see the black slot-screen faces. Same files, new URLs. See the cove section, "Cache-bust on GLB swap".

**Last Audited: 2026-09-18 (cove slot screens: black cover faces removed from the interior GLB).** Drift note: a report from another session showed a large black triangle on every cove slot screen. It is in the source asset, not a recent regression: the same 426 black-band faces exist in every version back to 2026-05-19. A live raycast from the spawn camera found the black pixels on `auto_24` faces about 1 wu in front of the screen picture; dropping those faces from the live geometry showed full screens with no holes, and dropping the other same-band group (the lower door panel) opened holes, so only the screen group goes. The offline rule selected the same 150 face indices as the live test. `INTERIOR_GLB` is now `...-mo-ktx.glb?v=2` and the landing vignette `cleaned-v1.glb?v=6`. Local prod build: every slot screen in the spawn view shows the full picture. Details: the cove section, "Slot screen cover faces". No shader, material or draw-call change; 150 fewer triangles. PARITY: human-only room visuals.

**Last Audited: 2026-09-18 (salvage state poll: signed-in accounts only).** Drift note: `SalvageStateHydrator` (`lib/three/land-salvage-render.tsx`) polled `GET /api/land/salvage/state` every 45 s for every visitor, but the route refuses guests, so each guest tab logged a 401 per poll (~80 lines/hour on prod). It now polls only for a signed-in non-guest account, restarts on an account change, and the store's private numbers (material balance, claim counters) reset on sign-out, account switch (`clear-identity-state.ts`) and when no account is present. Render of the nodes is unchanged.

**Last Audited: 2026-09-18 (cove exit in NPC mode).** Drift note: "Back to World" set only `avatarPosition`, but in NPC mode NpcController treats the possessed body's x/y (`__player-npc__` in `stores/npc.ts`) as the truth and copies it into `avatarPosition` every idle frame, so a guest's first step after the exit snapped ~300 px back to where the body entered: inside the tunnel's auto-enter band (world x ~-3487; seen on prod and locally, did not re-enter in 3 runs). The exit now also calls `useNpcStore.placePlayerNpc(x, y)` (sets x/y AND prevX/prevY, direction idle; no-op in player mode). Local NPC-mode exit walks, 2/2: east from 8114 with no snap. Test: `stores/__tests__/npc-place-player.test.ts` (CI web lane). PARITY: human-only client placement; agents do not use the cove walk-in.

**Last Audited: 2026-09-18 (cove: fallback-room switch warms up per visit and follows the phone frame cap; stage frame-cap delta fixed; table signs readable and clickable from the door).** Founder-reported on prod: the cove showed a dark room with black table slabs and the BLACKJACK sign covering BACCARAT. **Cause 1, measured:** that room is the cartoon FALLBACK GLB (reproduced exactly with `/cove?fallback=1`). The FPS auto-switch took the MEAN FPS over the first 5 s after mount. A walk-in from the world carries the stage transition, pipeline compiles and texture uploads (measured locally: 5 frames in the first second, single frames of 1017, 217 and 117 ms; one local walk-in averaged 35.6 FPS over its first 5 s at the page level), so stalls could decide it on a machine that renders the room at 52-60 FPS. Three more defects in the same sampler (Codex review): (a) phones are capped at 30 FPS by the stage (`PHONE_PROFILE.fpsCap`), so the fixed 40 FPS threshold sent EVERY phone to the fallback room; (b) a retained stage slot resumed an unfinished sample on the next visit with no warm-up; (c) frames over 1 s were skipped, so a machine under 1 FPS never got a decision. **Fix:** `lib/three/cove-fps-sampler.ts` (`CoveFpsSampler`, pure, 13 unit tests incl. every case above): every visit (`useSceneActive` turning true) starts with a 2 s warm-up that is not sampled; then WALL-CLOCK frame times (`performance.now()` between this scene's own frames, independent of the stage scheduler's delta) go into a preallocated `Float64Array` until 5 s AND at least 30 frames; the decision is the mean frame time after dropping the longest 5 % of frames, against a threshold of `min(40, 0.8 x stage frame cap)` (no cap: 40; phone cap 30: 24). Stalls AFTER the warm-up still count unless they are among the slowest 5 % of frames. The log prints the trimmed and the raw FPS. Local walk-in: `FPS OK (60.4 FPS trimmed / ~52 raw ..., threshold 40)` on the run whose page-level first 5 s averaged 35.6. **Stage-wide fix found while verifying the phone path:** with a frame cap, `StageFrameScheduler` (`use-scene-frame.ts`) handed scenes the gating accumulator, which includes the carried remainder, and also carried that remainder into the next admission, so every carry counted twice and scene time ran faster than wall time whenever native frame times are not an exact divisor of the 33.3 ms interval (measured: ~50 Hz rAF made the cove sampler see 22.6 FPS for a real 30). The admission step is now `components/three/world-stage/stage-frame-cap.ts` (pure, 7 tests): gating is unchanged (Codex replayed 100,000 randomized steps against the old algorithm: identical), and the admitted delta is the real time since the last admitted frame, with the same 2-interval clamp and recovery frame. On 60/90/120 Hz displays the carry is ~0, so phones there see no change. Local phone emulation after both fixes: `FPS OK (30.2 FPS trimmed / 30.0 raw, threshold 24)`, real room rendered. Not verified: a real Iris Xe or a real phone (needs your eyes). **Cause 2:** the baccarat table stands straight behind the blackjack table on the right lane (X ~400, Z 818 vs 469) and both banners were at y 280, so from the entrance BLACKJACK covered BACCARAT. **Fix:** the BACCARAT banner is at y 460 and 100 wu toward the aisle (`_BACCARAT_BANNER_Y`, `_BACCARAT_BANNER_X_OFFSET`); from the spawn camera (-17, 190, -1025) at 1600x900 it spans screen x 620-718, y 134-160, clear of BLACKJACK and of the right-lane chandelier (y 420 at the table centre still lost its first letter behind the chandelier, measured). The baccarat click box keeps its 200x340x150 table footprint. **Cause 3, pre-existing, reproduced on prod:** a click ON the BLACKJACK sign from the entrance opened BACCARAT: a sign was clickable only because its table box enclosed it, and the click-yield rule passed the click on to the farther baccarat box (no room geometry in front of it). **Fix:** `BankBanner` takes `onActivate`; the three table signs are now their own click targets (planes flagged `coveHotspot`, so a nearer box the ray crosses first yields to the sign), and the table actions are shared module functions (`_enterBlackjackFromCove` etc.). A sign plane's raycast keeps only hits inside the drawn capsule (`lib/three/cove-click-routing.ts`, half-width `120 - 30(1 - sqrt(1 - |dy|/30))^2` from the texture's quadratic end curves), so a ray through a transparent corner never becomes a sign hit (round-2 review case: from inside the right lane a BLACKJACK corner took a click aimed at the visible BACCARAT sign). The yield decision itself moved to `shouldYieldToFartherHotspot` in the same pure module; `cove-click-routing.test.ts` replays both review counterexamples with real three.js raycasts. A first attempt (counting every sign as an occluder in the yield rule, plus a bigger baccarat box) was rejected in Codex review: it misrouted BLACKJACK clicks from the rear aisle. Local real-mouse clicks after a walk-in, from the spawn and after 1.5 s up the aisle: each sign opens its own table. No asset, shader, material or draw-call change; one plane pair moved. PARITY: human-only room visuals and click routing; agents enter tables through the API, unchanged.

**Last Audited: 2026-09-18 (Land showroom covers the outer c ring).** Drift note: `land-showroom.tsx` now renders 56 showroom lots, not 36: the 20 outer c-ring plots (half-side 322 t, 1664 wu plots) get cottage/stall GLBs at level 2-3. Measured by Codex against the decoded GLBs: level-2/3 shells are 1,092/1,120 wu wide versus 1,177 wu at founder level 5 on the same plot size, at least 101 wu inside every plot edge, clear of the 682 wu FOR SALE sign offset; +102 mesh primitives / +72.5k triangles before culling, shared geometry and textures, 20 more constant-cost frame callbacks with module scratch vectors. Renderer code unchanged.
...
`RUN_SPEED_MULT` (2.025×) on `KELP_REALM_PLAYER_SPEED_WU_PER_SEC` (430 → ~871
wu/s) and the run locomotion clip now flows to the kelp VRM animator
(`updateAnimation` threads `state.running`; the GLB body stays static). New
generic intent extension: `readsSharedTouch` policies (kelp is the only one)
trigger sprint at the same 0.7 magnitude threshold from the shared touch
joystick, giving mobile parity with the world's store-joystick behavior. SERVER
COUPLING: the kelp beacon anti-cheat time floor divides by the new shared
`KELP_REALM_SPRINT_SPEED_MULTIPLIER` (2.025, cross-pinned to `RUN_SPEED_MULT`
by web test) so sprint traversal is not 429'd `too_fast`; effective tolerated
speed = 430 × 2.025 × 1.2 grace. Founder feel-check pending.
...
`KELP_POLICY` (sprint enabled 2026-08-09, see above; camera-relative movement and
...
**R19 Last edit / Last Audited:** 2026-08-09 (**Founder playtest follow-ups §5b — whirlpool made real, urchin heading restore, track widen, projection continuity; server-sim + shared-geometry change, founder feel-check pending.**) Four coupled changes, all spline-sim (`REEF_RACE_USE_SPLINE=true` lane): **(1) Whirlpool now does what it advertises** — the HUD/toast promised "Spin nearby rivals · 3s" but `collectWhirlpool` only pulled+slowed inside a 300wu bubble on a 909–3231wu-wide corridor (~0.23s of travel at cap — effectively never landed; founder: "whirlpool items don't seem to really work"). Now: standard 900ms spinout + 3s hazard-slow on every unshielded rival within `WHIRLPOOL_RADIUS` **900wu**; the violent inward pull stays inside `WHIRLPOOL_PULL_RADIUS` **300wu** — at 900 the pull dragged victims backward past the 0.02-lap anti-cheat progress tolerance (integration-test bisect). Tide-wave's identical dead-radius (hardcoded 250) raised to `TIDE_WAVE_RADIUS` 600. **(2) Spinout heading restore** — `OBSTACLE_SPIN_RATE × OBSTACLE_SPINOUT_MS` = 9.896 rad ≈ 1.575 turns, so every urchin/creature/puffer/whirlpool spinout exited ~207° off the racing line, deterministically near-backwards. New `spinoutEntryRot` captured on FRESH spinout at all four contact sites, restored on the expiry tick in `applyIntentForTick` (velocity was only damped, never rotated, so heading and momentum re-agree; wipeout/respawn paths zero the flag directly and own their own rot). The spin remains authoritative-rot theater — no client change needed. **(3) Track widen, width-only** (founder: "widen the track, same structure") — zero CP moves; arc 95741.0, curvature, reversals, start grid all bit-identical. Chicane pinch 470→600-class (hw 597.8–799.7), hairpin entry/recovery legs +50–80, hairpin APEX kept as THE tight moment (462–465; its minR 1020.1 caps carve headroom at R−550). Locked figures now: hw sweep 458.3–1613.5, carve margin 554.1 (>550), inter-pass edge clearance 487→**393.9** (>300 — the hairpin in/out legs share this budget; watch it first on any further widening). Consumers that scale automatically: obstacle lateral fractions, rip-current lanes (width×0.62), bot lateral limits, pickup spans, client ribbon/cliffs/finish gate. NOT width-scaled (flagged, unchanged): boost-pad absolute lateral offsets (0/±35/±45 — now relatively more centered). **(4) Projection continuity window** — the widen pass exposed that `closestPointOnSpline`'s GLOBAL coarse scan re-decides the basin every tick: bodies riding wide inside lines saw 0.03-lap projection jumps (false anti-cheat regressions, wrong-leg wall clamps). The shared spline gains an optional arc-space continuity window (`nearT` + `windowArcWu`, Newton result clamped to the window, cyclic) with a `escapeHysteresisWu` teleport escape (current-swap/test placements follow a decisively-closer basin); `resolveProgress` + `enforceSplineWallClamp` track per-body with window 600wu / hysteresis 500wu. Verified: `verify-track-v7` geometry green (pre-existing pad-placement assertions unchanged), full sim suite 298/298 ×3 (integration harness now exempts resolved current-swap teleports from its regression tracker, matching the sim's own anti-cheat exemption). Perf: no new draws, materials, or per-frame allocations; the windowed scan still walks the same 1000-sample LUT. Stale P2-T19/T20 legacy placement-table pins updated (pre-existing drift). **Codex R19 round-1 BLOCK findings folded (round 2 re-review in flight):** (1) rotation-owning mechanics can no longer strand a stale deferred heading restore — the remora branch clears `spinoutUntil` on entry, `collectWhirlpool` skips remora-active rivals (same rule as puffer mines), and a resolved current-swap carries the full spinout triplet (until/direction/entryRot) with the pose both ways; (2) `event.hit` gains optional `spinoutDurationMs` (whirlpool + puffer emits) and the client hit handler raises the SAME `selfObstacleControlLockedUntil` prediction lock as obstacle spinouts (Math.max — never shortens), so victims stop locally steering against the authoritative spin; (3) spin direction is target+sim-clock parity captured only on a fresh spinout — two same-tick whirlpools from any sources produce identical state; (4) the carve-margin contract now EXPLICITLY defines R(t) as the verifier's h=0.0005 stencil-smoothed curvature (finer stencils resolve the apex knot to ≈511 wu on the ORIGINAL v7 widths too — 550 was always a smoothed floor); (5) the projection window clamps (not wraps) `nearT` on open splines + 5 direct unit tests; (6) the committed A/B runner fails nonzero on server/probe/report failures and uses batch-unique profile basenames; (7) the R18d seeded-race collections floor 40→35 (deterministic trajectories shifted; 38 observed stable — tuning floor, not a pin). See §10b.
...
5. **Always render TWO `BankBanner` labels** (procedural `THREE.CanvasTexture` + `PlaneGeometry`) at `(classicCentroid[0], 280, classicCentroid[2])` and `(bonusCentroid[0], 280, bonusCentroid[2])`. Banner Y is **pinned to 280wu** (just above cabinet tops, below ceiling) — never computed from bbox centroid + offset, since floor decals + ceiling trim in the same material can drag the centroid out of view. **One exception (2026-09-18):** the BACCARAT table banner is at y 460 and 100 wu toward the aisle, because the baccarat table is straight behind the blackjack table from the door; place any new banner by measuring it from the spawn camera against the other banners AND the chandeliers.
...
**Cache-bust on GLB swap:** every replacement of the served interior GLB MUST bump `?v=N` in the `INTERIOR_GLB` URL (currently `/models/cove/cove-interior-cleaned-v1-mo-ktx.glb?v=3`; the landing `CoveVignette.tsx` serves `cove-interior-cleaned-v1.glb?v=7`). `?v=2` / `?v=6` are BURNED: on 2026-09-19 a curl of them on prod before the web container flipped made Cloudflare cache the pre-fix bytes under those URLs (`cf-cache-status: HIT`, `max-age=31536000, immutable`). Never reuse a `?v` value, and never fetch a new `?v` URL on prod before `SOURCE_COMMIT` on the web container equals the pushed sha. Cloudflare edge cache TTL is 7 days, deploy token has no `cache_purge` scope, and the service worker caches assets cache-first by full URL. See §6f rule 9.

**Slot screen cover faces (removed 2026-09-18):** the source asset (every version back to the 2026-05-19 backup) had, on each of the 48 slot cabinets, 3-6 faces of material `auto_24` (the cabinet atlas `Image_6`) about 1 wu in front of the screen picture (`auto_4`, `Image_3`). Their UVs sit in the atlas's black top band (u 0.49-0.63, v 0.00-0.13; several are near-zero-width slivers), so each screen showed a large black triangle. `scripts/patch-cove-slot-screen-covers.mjs` removes them: an `auto_24` face with all UVs in that band AND its centroid close to an `auto_4` triangle (point-to-triangle distance relative to the cabinet primitive's size). The same band also textures each cabinet's lower door panel (138 faces, far from any screen); they stay. The script is pinned to this asset (150 removed / 138 kept, and both groups on their side of a fixed relative distance of 0.0112, else it refuses to write). A second run reports `alreadyPatched` only when the 138 remaining band faces are all far from every screen. Applied to `cove-interior-cleaned-v1-mo-ktx.glb` (only the `auto_24` index count changed, 13896 -> 13446) and `cove-interior-cleaned-v1.glb` (the Draco re-encode re-indexes and merges duplicate vertices, so vertex counts drop in several primitives; every kept triangle's positions, UVs, normals and winding are identical, Codex-verified). `cove-interior-cleaned-v1-ktx.glb` is NOT patched (no runtime path serves it); run the script on it before any future chain rebuild from that file.
...
- **Cove "both prompts"** (`character-positions.ts` + `cove-entrance.tsx` + `player-avatar.tsx`):
  ...
  - **Auto-enter**: `isInsideCoveTunnel` (X ≤ `COVE_AUTO_ENTER_MAX_X = -3450`, |Z| ≤ 120) fires `triggerCoveWalkIn()` once the player walks deep into the corridor. The cove **collision AABB** east edge is world X ≈ **-3586** (`world-colliders.ts BUILDING_EXTENTS.cove halfX=546` + 28 buffer), so -3450 is reachable on foot (mid-corridor, east of the wall). Runs in a `useFrame` in `cove-entrance.tsx`, gated to `player`/`npc` (autonomous agents + explore free-cam never auto-enter); one trigger per entry via a module guard `_coveAutoArmed`; also guarded by `!movementFrozen` + `!useTransitionStore.active`. **Corrected 2026-09-18:** the guard is NOT re-armed at a `COVE_REARM_X = -3250` line (no such constant exists in code). `cove-entrance.tsx` re-arms it the moment the avatar is OUTSIDE the auto-enter band.
  - **Exit spawn invariant (2026-09-18).** "Back to World" lands the avatar at `COVE_EXIT_WORLD_X/Z` (`character-positions.ts`), derived as `COVE_TUNNEL_PROMPT_MAX_X + 30` = world **(-3150, 0)**, just outside the tunnel mouth (-3275). It MUST be EAST of the prompt band and so east of the auto-enter band, because walking to town means walking east. The previous exit was a hand-set world -3760 in `cove/page.tsx`, set 2026-06-15 before the tunnel existed: 40 wu WEST of the auto-enter band [-3720, -3450]. Combined with the immediate re-arm above, the first step toward town re-entered the cove on every exit (founder-reported). The old point was also INSIDE the cove's solid collision AABB (east edge -3586, -3561 once expanded by the player's 25 wu half-width), so the first step east was pushed straight to -3561, inside the band. The new point has 411 wu of collider clearance, and walking west from it still reaches the band, so deliberate re-entry works. `cove/page.tsx` now places the avatar at the exit on ALL three exit paths (fade midpoint, expired navigation, refused navigation), matching the kelp page; before, only the midpoint did. Pinned by `cove-exit-spawn.test.ts` (band geometry only; it does not model collision).
