...
**DPR cap:** `dpr={LOW_END_GPU_DETECTED ? [0.5, 0.65] : [0.75, 1]}`. `LOW_END_GPU_DETECTED` is computed once at module load via `detectLowEndGpuClass()` in `gpu-tier.ts` using `WEBGL_debug_renderer_info` plus the coarse-pointer mobile heuristic. It covers Intel/Iris/UHD/Adreno/Mali/PowerVR/Apple-integrated. The async `createWebGPURenderer()` stamps the same DPR range before `init()`; keep the two paths in sync. Do NOT call `gl.setPixelRatio()` inside `onCreated` — that overrides the Canvas prop.

**Adaptive quality governor (2026-06-06, perf2):** default `/game` (no `perfFlags`) samples RAF every 2.5s after a 5s warmup and increments `window.__W3D_QUALITY_TIER` when FPS <58. Tier 1 disables `groundCover` (seaweed only), tier 2 disables `activityFx` (activity indicators + floating rewards), tier 3 disables labels. It restores one tier only after three high samples >72 FPS. `/perf` passes explicit flags, so it bypasses the governor for clean A/B measurements.
...
| Texture upload | `requestIdleCallback` with a one-texture minimum, max 4 textures per slice, and 6 ms elapsed budget. Local 2026-06-08 run: 179 textures, 47 idle slices, 20.6 s total, 9 ms max slice, `ready=true`. | `World3DCanvas.tsx:StaggeredTextureUpload` |
...
- `<StaggeredTextureUpload />` — `requestIdleCallback` with one-texture minimum, max 4 textures per callback, and 6 ms elapsed budget. Safari fallback: rAF batched 4/frame. 2026-06-08 local production report uploaded 179 textures in 47 idle slices over 20.6 s with 9 ms max slice and reached `__W3D_TEXTURES_READY`.
