...
**VRM fit + grounding (fixed 2026-06-05):** `PlatformModelVRM` uses `computeVRMAvatarFit(vrm, reg.animatorId, PICKER_TARGET_HEIGHT_WU=22)` (the canonical helper from `vrm-avatar-sizing.ts`). This resets `vrm.scene.scale` to 1 before measuring bbox (prevents contamination from prior state), scales to 22wu target, and returns `offsetY = -box.min.y * scale`. Group is placed at `y = PLATFORM_TOP_Y(0) + offsetY`. For Milady/VRoid rigs offsetY approx 0 (feet at bbox.min.y=0); for Mixamo/Hermes/Tekk/chibi rigs offsetY > 0 (hips at origin, feet below -- offsetY lifts them onto the disc).

**VRM facing (fixed 2026-06-05):** VRM 0.x models (Milady) have `VRMUtils.rotateVRM0` applied in `vrm-loader.ts` which sets `scene.rotation.y = pi`, making them face -Z toward the picker camera at +Z. VRM 1.x models (Hermes / Tekk / chibi) skip `rotateVRM0` (no-op unless `vrm.meta.metaVersion === "0"`) and face +Z natively (back to camera). The registry field `faceYaw: Math.PI` on VRM 1.x entries carries the correcting delta; `PlatformModelVRM` applies it once per VRM load via `vrm.scene.rotation.y += reg.faceYaw ?? 0`.
