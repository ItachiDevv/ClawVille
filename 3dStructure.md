...
**Last edit:** 2026-06-17 (jump animation BUG 1 foot-grounding fix [v3] + BUG 2). **BUG 1 (midair squat):** Headless integration test (parse real VRM + `_emotes.glb`, run the REAL `retargetMixamoClip`) proved the `squat` clip is ROTATION-ONLY — raw hips.position Y is a flat constant (Y=104.226, 2 keyframes, zero descent), so the knee-bend with the body pinned at standing height lifts the FEET toward the pelvis (the midair squat). TWO earlier fixes were dead ends: (a) `getSquatGroundLift` (preserve a non-existent descent) → always 0; (b) procedural `group.position.y -= crouchDepth` + foot-replant → CANCELS (lowering a rigid body then replanting the feet nets ~0; a body translate cannot synthesize a crouch). Correct fix: FOOT-GROUNDING — after `animator.update()` poses the crouch, read the lowest foot/toe world-Y via `getFootWorldYMin()` and set `group.position.y -= (lowestFootY − effectiveFloorY)` so the lifted foot replants on the floor → the body settles toward the planted feet = squat DOWN (correct direction). Depth == the clip's knee-bend (headless-measured ~2.3 wu for Milady — shallow; a deeper crouch needs a stronger/re-baked squat clip, tracked). Dead code retired: `extractSquatHipYTrack`, `sampleHipYAtTime`, `_squatHipYTrack/Times/RestY`, `getSquatGroundLift`, `getSquatClipTime`, `SQUAT_CROUCH_VRM_M`, `SQUAT_CROUCH_RATE`, `squatCrouchRef`. Kept: `getFootWorldYMin()` + module-scope `_squat_footScratch`. **BUG 2 (foot-slide):** `jumpState.chargeMode` (`'squat'|'run'|'none'`) set on rising edge in `player-avatar.tsx` + `arena-npcs.tsx`; walk/idle → halt WASD; run → keep running. §7a updated.
...
**7a. Jump charge motion rules (revised 2026-06-17 — foot grounding [v3]).**
...
- **Idle/walk → squat charge (`chargeMode === 'squat'`):** horizontal velocity zeroed, surfaceClip = `'squat'`. AFTER `animator.update()` poses the (rotation-only) crouch, `group.updateMatrixWorld(true)` → `animator.getFootWorldYMin()` reads the lowest foot/toe bone world-Y → `group.position.y -= (lowestFootY − effectiveFloorY)` plants the lowest foot exactly on the floor (second `updateMatrixWorld`). Because the rotation-only squat LIFTS the feet, this LOWERS the whole body so it settles toward the planted feet = a squat DOWN toward the ground. Foot stays grounded throughout.
