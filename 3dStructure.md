...
| Texture upload | Deadline-aware `requestIdleCallback` (restored 2026-06-10): real idle window (`!deadline.didTimeout && timeRemaining()>0`) → upload until `timeRemaining()<2ms`, NO fixed count cap (one 50ms idle window can drain the whole queue); timed-out callback → fall back to `IDLE_MAX_TEXTURES_PER_SLICE=4` + `IDLE_SLICE_BUDGET_MS=6ms`. Always ≥1 texture per slice. The spike-era hard 4-cap (no deadline check) doubled time-to-ready 10.8s→20.5s and is reverted. | `World3DCanvas.tsx:StaggeredTextureUpload` |
...
- `<StaggeredTextureUpload />` — deadline-aware `requestIdleCallback` (restored 2026-06-10): with a real idle window (`!deadline.didTimeout && timeRemaining()>0`) uploads until `timeRemaining()<2ms` with no count cap; on a timed-out callback falls back to max 4 textures + 6 ms elapsed budget; always uploads ≥1 per slice. Safari fallback: rAF batched 4/frame. The spike-era hard 4-per-slice cap (no deadline term) had doubled time-to-ready 10.8s→20.5s. `window.__CV_TEXTURE_UPLOAD_METRICS` is only written when VRM metrics are enabled (`?perf=1` or `__CV_PERF_HARNESS__` — see §9e parse-queue contract).
