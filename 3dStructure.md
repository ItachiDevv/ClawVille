...
**Last edit:** 2026-05-26 — WebGPU/WebGLBackend canvas presentation fix: `World3DCanvas` now constructs the renderer with `alpha:false` and forces `setClearColor(SKY_COLOR, 1)` / `setClearAlpha(1)` on init and Canvas creation. Live staging showed scene objects and draw calls were present but the canvas presented transparent pixels, exposing the page's blue background.
