...
**Last edit:** 2026-06-06 (perf2 adaptive FPS pass): `World3DCanvas.tsx` now resolves default `/game` perf through `useAdaptiveWorldPerfFlags()` when no manual `perfFlags` are passed. Quality tiers sample RAF every 2.5s after a 5s warmup: tier 1 disables animated ground cover (`MergedSeaweed`), tier 2 disables activity/reward FX, tier 3 disables world labels, and tier 4 switches far building residents/companions to cheap capsule proxies while preserving full GLB detail when the camera comes within 2600wu (demote again after 3200wu). The governor steps down when FPS <58 and restores one tier only after 3 high samples >72 FPS, so capable GPUs keep full quality while low-end/mobile devices shed the least-visible effects first. `?fast=1` starts at tier 4 for measurement. Manual `/perf` flags bypass the governor. `WorldPerfFlags` split fog from decorative effects via new `groundCover`, `activityFx`, and `residentDetail` flags, so fog remains available when seaweed/FX/resident detail are off. Low-end DPR is now actually `[0.5,0.65]` in both Canvas and async WebGPU init (the previous code still used `[0.55,0.7]` despite the doc comment). `gpu-tier.ts` owns the shared synchronous low-end probe. `lod-orchestrator.tsx` keeps `FULL_CAP=14` on capable GPUs but uses `ACTIVE_FULL_CAP=8` on low-end/mobile for wandering/remote entities only; the possessed NPC remains full. `/perf` adds decorative-FX-off and far-resident-proxy tests plus separate toggles for fog, ground cover, activity FX, and resident detail. No new per-frame allocations, no new lights, no drei Text/Billboard, no InstancedMesh.
