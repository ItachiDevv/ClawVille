...
**Last edit:** 2026-06-07 (combined perf integration + DPR retune): merged the `perf2` adaptive/runtime quality-tier pass with `feat/perf-cpu-framebudget` CPU/assets work. Runtime rules now include the perf2 adaptive governor, tier-4 building/resident proxies, HUD collapse, and moving-avatar cap, while `arena-npcs.tsx` also keeps the CPU branch's once-per-frame `getNpcFrameShared()` cache for `controlMode` + camera position. Staging verification showed normal `/game` entering tier 4 at renderer DPR `0.4` with visibly soft output, so `AdaptiveRendererDpr` no longer applies the automatic tier-4 `0.4` DPR clamp; it only reapplies the existing low-end/touch or standard Canvas DPR caps. Conflict resolution preserved the multiplayer remote-player guard: GLB/VRM NPC entity-vs-player push-out still checks `!d.isRemotePlayer` so remote players do not diverge per client. Asset pipeline also includes texture-only building recompression, bazaar stand compression, quest-bounty pavilion compression, SW cache updates, and the documented rule that building GLBs flowing through `mergeStaticMeshesByMaterial` must not be meshopt-quantized. See `docs/perf-integration-change-ledger-2026-06-07.md` for rollback candidates, especially quality-reducing tier-4 features like visual proxies and HUD collapse.
...
| **Multiplayer LOD orchestrator** | `FULL_CAP=14` full-skeleton VRM/GLB cap across (wandering NPCs + remote players in same room); low-end/mobile uses `ACTIVE_FULL_CAP=8`, and adaptive tier 4 tightens that to 2 nearest full moving entities. `VISIBILITY_RADIUS_WU=3500` early-rejects entities outside the commons. Frustum-tested via `Frustum.setFromProjectionMatrix(camera.projectionMatrix × camera.matrixWorldInverse)` per frame, sphere = 200wu. Sorted ascending by camera distance² (no sqrt). Top cap → full mesh (VRMNpcMesh / GLBNpcMesh); rest → cheap capsule proxy (`EntityProxyMesh`). **Hysteresis ≥ 500ms** between tier flips per entity prevents flicker at the cap boundary. Building residents (`ArenaLocationNpcs`) are excluded from this shared moving-entity candidate pool; under adaptive tier 4 only, far residents use stationary `LocationNpcProxy` capsules until the camera is within 2600wu, then remount full GLB detail. Module-scope `_camPos`, `_frustum`, `_projScreenMatrix`, `_entitySphere`, `_candidatePool[]` scratch — zero per-frame allocations. (Multiplayer Phase 1, low-end cap refined 2026-06-06; resident tier added 2026-06-06) | `lod-orchestrator.tsx`, `arena-npcs.tsx:NpcEntry`, `remote-players.tsx:RemotePlayerEntry`, `remote-player-proxy.tsx:EntityProxyMesh`, `arena-location-npcs.tsx:LocationNpcProxy` |
...
- 2026-06-07 — DPR retune after staging verification: normal `/game` hit tier 4 at renderer DPR 0.4 and looked visibly soft, so `AdaptiveRendererDpr` no longer applies the automatic tier-4 DPR clamp. Tier 4 can still collapse HUD chrome, proxy far residents/building GLBs, and reduce moving full-detail NPC/remote-player budget to 2. `?fast=1` still locks tier 4 for deterministic measurement.
