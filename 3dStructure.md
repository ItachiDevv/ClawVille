...
| **Multiplayer LOD orchestrator** | `FULL_CAP=14` full-skeleton VRM/GLB cap across (wandering NPCs + remote players in same room). `VISIBILITY_RADIUS_WU=3500` early-rejects entities outside the commons. Frustum-tested via `Frustum.setFromProjectionMatrix(camera.projectionMatrix × camera.matrixWorldInverse)` per frame, sphere = 200wu. Sorted ascending by camera distance² (no sqrt). Top 14 → full mesh (VRMNpcMesh / GLBNpcMesh); rest → cheap capsule proxy (`EntityProxyMesh`). **Hysteresis ≥ 500ms** between tier flips per entity prevents flicker at the 14/15 boundary. Building residents (`ArenaLocationNpcs`) are EXCLUDED from candidate pool — always full. Module-scope `_camPos`, `_frustum`, `_projScreenMatrix`, `_entitySphere`, `_candidates[]` scratch — zero per-frame allocations. (Multiplayer Phase 1 — 2026-05-27) | `lod-orchestrator.tsx`, `arena-npcs.tsx:NpcEntry`, `remote-players.tsx:RemotePlayerEntry`, `remote-player-proxy.tsx:EntityProxyMesh` |
