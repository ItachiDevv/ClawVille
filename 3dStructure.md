...
| **Velocity-synced locomotion playback** | (2026-06-10 "reverse moonwalk" fix, final form.) Walk/run clips' natural ground speed = player 550 wu/s (`NPC_CLIP_GROUND_SPEED_WU_S`; run ×1.5). Server NPCs now ALSO translate at 550 wu/s (`npc-simulation.ts moveNpcs baseStep 110 @5Hz` — raised from 220, which forced a choice between foot-slide at timeScale 1 or sluggish honest legs at 0.4; "both at normal speed" per user). `VRMNpcMesh` accumulates rendered distance/time over 0.4s windows → `animator.setLocomotionTimeScale(speed/ref)` — steady-state ≈ 1.0, only guarding interp dips (clamped [0.3,1.8], deduped, survives crossfade `reset()` AND the one-shot emote return path; idle/surface clips never scaled). Windowed (not per-frame) because demo-wander steps + 5Hz interp make frame deltas spike/zero and bias the scale low. Change player SPEED ⇒ update the constant AND server baseStep same diff. | `arena-npcs.tsx:VRMNpcMesh`, `vrm-character-animator.ts`, `apps/api/src/services/npc-simulation.ts:moveNpcs` |
