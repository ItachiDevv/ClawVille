...
**Casino input (bug-fix 2026-05-18):** Arrow keys (`ArrowUp/Down/Left/Right`) now mapped to w/s/a/d slots in `casinoKeys` via `e.key.length === 1 ? e.key.toLowerCase() : e.key` guard. Previously `e.key.toLowerCase()` converted `"ArrowUp"` → `"arrowup"` (no match).

**Casino camera (bug-fix 2026-05-18):** Module-scope `_casinoCamYaw` (init π) lerps toward avatar `rotRef.current` at `CAM_YAW_LERP=0.05` (vs avatar turn rate 0.15). Both VRM (`CasinoVRMAvatarInner`) and GLB (`CasinoGLBAvatarInner`) branches use `_casinoCamYaw` for behind-position, not the avatar's live yaw — prevents 45° viewport snap on A/D press. `CAM_ABOVE=270wu CAM_BEHIND=520wu CAM_LOOK_Y=120wu`.
