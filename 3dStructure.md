...
**Last edit:** 2026-05-22 — NPC speech-bubble parity with name tags: `npc-speech-bubbles.tsx` opts into the existing `useWorldLabel({ occlude: true })` raycast (same 10 Hz staggered camera→anchor test the GLB+VRM name labels use) so bubbles disappear behind every structure carrying `userData.isOccluder = true` (12 buildings + shisha-oasis + bazaar-stall + auction-podium + quest-bounty-pavilion + town-directory-sign + Nori). Proximity fade tightened from 4000/10000 → 1800/5000 wu so off-ring NPC chatter no longer fills the screen with unreadable text; near-camera bubbles still at full opacity. No change to `world-labels-overlay.tsx` infrastructure — feature was already implemented, just wasn't enabled for bubbles. Same diff: `apps/web/src/lib/three/npc-speech-bubbles.tsx` (~10 LOC).
...
- 2026-05-22 — NPC speech-bubble parity with name tags: `npc-speech-bubbles.tsx` adds `occlude: true` to `useWorldLabel` (same 10 Hz staggered raycast against `userData.isOccluder` meshes that name tags use) — bubbles now hidden behind buildings, shisha-oasis, bazaar/marketplace stalls, auction podium, quest pavilion, town-directory sign, Nori. Proximity fade tightened 4000/10000 → 1800/5000 wu (off-ring chatter was cluttering the screen). No infra change in `world-labels-overlay.tsx` — feature was already shipped, just unused for bubbles.
