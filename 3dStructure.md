...
| `apps/web/src/lib/three/casino-interior.tsx` | Scene component + walkable player system. Loads `casino-interior.glb` (gameready, Draco, ~211k tris, 4.2MB). Auto-falls back to `casino-interior-fallback.glb` (cartoon, 58KB) if avg FPS < 40 in first 5s or `?fallback=1`. Box3 max(X,Y,Z) auto-fit to `INTERIOR_TARGET_HEIGHT=2000wu` (was 600wu — see bug-fix note below). `CasinoPlayerAvatar` routes VRM/GLB based on `avatarModelKey`. WASD at 830wu/s (was 250), bounded to `x∈[-383,+383] z∈[-900,+900]`. Follow camera: +200wu above, +450wu behind, lookAt Y=100, exp-decay lerp. 4 primitive slot cabinets on left wall (x=−383). `SlotHotspot` on each cabinet face → `useCasinoStore.openSlotScreen`. GLB material clone on useMemo fixes cross-renderer-context purple rendering. |
...
**Bug fix 2026-05-18 — avatar too big + purple legs:**
- `INTERIOR_TARGET_HEIGHT` 600→2000wu. Room ceiling was ~120wu vs VRM avatar ~270wu → head clipped ceiling + follow camera at Y=55 saw only legs. At 2000wu ceiling is ~400wu — ample headroom.
- All room-relative coords scale by 3.333: bounds, spawn, cabinet positions, cabinet geometry dims, movement speed, fog.
- Camera offsets updated: +55→+200 above, +160→+450 behind, lookAt Y 18→100 (mid-chest of 270wu avatar).
- GLB avatar (`CasinoGLBAvatarInner`): `scene.clone(true)` shares `MeshStandardMaterial` instances compiled against the world canvas WebGL context; casino canvas is a separate context → purple/pink fallback. Fix: traverse cloned scene and `mesh.material = mesh.material.clone()` before use.

**Player spawn:** `(0, 0, 800)` (≈ 240 × 3.333) — near front entrance wall (+Z side), facing −Z into the room (rotation π). WASD key state is module-scope (`casinoKeys`) separate from world canvas to avoid cross-canvas contamination.

**Slot cabinets (Concern 6.0.5):** 4 cabinets at x=−383, z ≈ −583 / −333 / −83 / +166. Each = base (140×20×107) + body (127×227×93, dark purple) + emissive screen (80×113×7, cyan, emissiveIntensity 1.2) + lever (CylGeo r10 h73, red). All dimensions scaled ×3.333 from original 600wu. All geometry module-scope. Rotated π/2 (face into room). `matrixAutoUpdate=false`. Draw calls: 16 (4 cabs × 4 meshes).
...
- 2026-05-18 — Casino interior bug fixes: `INTERIOR_TARGET_HEIGHT` 600→2000wu so ceiling (~400wu) clears VRM avatar (~270wu); follow camera offsets updated (+55/+160/18 → +200/+450/100); all room-relative coords ×3.333 (bounds, spawn, cabinet positions + geometry, speed, fog). GLB `CasinoGLBAvatarInner`: material clone added to prevent cross-renderer-context purple rendering. §10c updated.
