...
**Current drift note:** 2026-05-26 — NPC route contract tightened: server A* outputs raw tile-center waypoints, `npc-simulation.ts` accepts only paths whose every segment samples collision-free through `clampPosition2D`, spawns resolve through planner-valid walkable points, and connected SSE snapshots apply idle positions so the 3D renderer does not show stale sideways movement.
