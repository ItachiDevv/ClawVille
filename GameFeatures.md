...
- **v2 CLOSED-LOOP ring (flag ON; `/preview/reef-race-v2` hardcodes ON):** the curvy lap circuit in §18d.v2. **This is the founder-locked direction** (Reef Race becomes "Mario Kart but surfing"). **⚠️ The track + scene are SUPERSEDED by §18d.v5 — "SURF ROAD" (2026-06-23): a Rainbow-Road glowing FLOATING WATER RIBBON in a cosmic VOID, NO land/island. 32 CPs / arc 60 257 wu / 26 reversals / render-only elevation profile.** §18d.v2–v4 describe the prior land-disc framing and the unchanged lap/position/finish machinery.
...
**Status:** built — needs verification. Track + sim params + the render-only elevation/bank profile verified by tests (reef suites **294 pass / 0 fail**, incl. the 8-body integration smoke that caught + drove the corridor-width fix) + `bun run build` 9/9 + web build OK. NEEDS an in-browser check at `/preview/reef-race-v2` (a glowing floating water ribbon in a cosmic void — NO land/island; aggressively zig-zagging + undulating; neon rails with bloom; surfer riding ON the ribbon through climbs/drops; FPS ≥ floor) + the founder's real-keyboard playtest (Rule E4). Feel (prediction / surf-carving / drift-mini-turbo) is the next pass on top of this geometry.
