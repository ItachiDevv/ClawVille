...
- **v2 CLOSED-LOOP ring (flag ON; `/preview/reef-race-v2` hardcodes ON):** the curvy lap circuit in §18d.v2. **This is the founder-locked direction** (Reef Race becomes "Mario Kart but surfing" — a curvy closed circuit, N laps around a central island).
...
**Status:** built — needs verification. Track + sim params verified by tests (reef suites GREEN in isolation) + `bun run build` 9/9. NEEDS staging deploy + an in-browser parity check at `/preview/reef-race-v2` (WIDE water corridor reads as water, loop winds + uses the footprint, land frames the water, island fits, FPS ≥ floor) + the founder's real-keyboard playtest (Rule E4). Feel (prediction / surf-carving / drift-mini-turbo) is the next pass on top of this geometry.
