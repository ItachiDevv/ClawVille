...
**Current drift note:** 2026-05-26 — NPC free-roam planner now derives wanderer status from `NPC_DEFINITIONS.buildingId === ''` and keeps all default wandering avatar families inside a 1700-3000wu town-commons annulus, avoiding central prop AABBs and building-ring wall paths while improving NPC chat proximity.
...
All default wanderers are free roamers (`buildingId === ''`), including Milady, Hermes, chibi, and the single crustacean. The server planner keeps them inside the town-commons annulus (1700-3000wu from center) and routes every target through the same AABB-aware pathfinder/collider map used by movement collision.
