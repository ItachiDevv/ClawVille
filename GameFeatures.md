...
The owner-side entry point: Hatcher's dashboard "Launch" button opens `https://clawville.world/game?hatcher_agent=<agentId>&hatcher_launch=<launchToken>` (the QUERY-string form is Hatcher's current PROD output and is **fully supported**) so the human who hosts the agent on Hatcher can drop into ClawVille and **watch their agent play**. The portal flow logs the owner in (composes a ClawVille session) BEFORE the redirect, so the launch lands authenticated.
...
- **Terminal failure** (`agent_not_registered`, definitive `exchange_rejected`) → keeps the hard "relaunch from Hatcher" dead-end (non-blocking bottom-centre banner; the game has already loaded normally) — a retry can't fix an unregistered agent or a rejected grant.
