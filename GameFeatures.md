...
**Current drift note:** 2026-05-26 — NPC routes now keep raw tile-center A* waypoints, validate every accepted path segment through the same AABB clamp used by movement, resolve spawns to planner-valid walkable points, and treat connected SSE positions as authoritative even on idle frames to prevent sideways slide/snap artifacts.
...
Guest auth is not a real account. `/game` must still show Log In / Sign Up account CTAs after guest bootstrap so a visitor can upgrade instead of being trapped in guest-auth UI.
...
All default wanderers are free roamers (`buildingId === ''`), including Milady, Hermes, chibi, and the single crustacean. The server planner keeps them inside the town-commons annulus (900-2400wu from center), resolves spawn/home points to planner-valid walkable coordinates, and routes every target through the same AABB-aware pathfinder/collider map used by movement collision. A* returns raw tile-center waypoints, accepted routes are sampled segment-by-segment against the AABB clamp before an NPC can commit to walking, and an attempted step into a blocker aborts/replans instead of sliding along a wall forever.

When disconnected from SSE, `stores/npc.ts` runs a client-side wander loop at 10 Hz so the world doesn't go static. Connected SSE snapshots are authoritative for every server position, including idle frames, so the renderer does not keep stale pre-arrival coordinates that later appear as sideways snaps. The fallback roster mirrors the live unique free-roaming cast — 3 Milady VRMs, Mira/Cyrus/Tekk, Eliza/Mila chibis, and one lobster (`Driftwood`) — with collision-safe target repicks so fallback NPCs do not walk into building colliders. Server connection takes over via `setConnected(true)`. The web store also filters retired server IDs (`wanderer-marlin`, `wanderer-riptide`) so a stale API container cannot reintroduce removed crustaceans during a partial staging deploy.
...
Conversation attempts run from the authoritative server sim and rely on wanderers staying within partner-search range. If free-roam radii or obstacle maps change, verify both paths and chat bubbles together.
