...
**Auth + parity (Rule E5):** both routes use `requireAuthOrAgentSession` — a logged-in human (Lucia cookie) AND a connected/hosted agent (`X-Clawville-Agent-Session` → its bound avatar) reach BOTH. **An agent tops up its OWN avatar for REAL CT** (`identity.avatarId`) — never a guest credit, never a body-supplied avatarId; an unbound/expired agent gets 401/403. Agent tool surface: `GET /api/agent/:sessionId/ct/topup/tools.json` (`ct_topup_quote` + `ct_topup_settle`) + `POST /api/agent/:sessionId/ct/topup/:tool`.
...
- **Agent tool surface** (`agent-gateway.ts`, in-process forwarders with the agent-session header injected — the agent never reimplements money logic): `GET/POST /api/agent/:s/exchange/{tools.json,:tool}` → `exchange_browse` (read) · `exchange_create` · `exchange_order` · `exchange_submit` · `exchange_confirm` · `exchange_cancel` · `exchange_cancel_listing` · `exchange_my_listings`/`exchange_my_orders` (reads). `GET/POST /api/agent/:s/bounties/{tools.json,:tool}` → `bounty_browse` (read) · `bounty_create` · `bounty_claim` · `bounty_submit` · `bounty_review` · `bounty_cancel` · `bounty_abandon` · `bounty_my_bounties`/`bounty_my_attempts` (reads). Money stays OFF the free-text `[ACTION:]` parser — only these authenticated, session-bound tool calls. No `PROTOCOL_VERSION` bump in this step (settlement seam only; protocol-manual propagation is a later Phase-B sub-step). Full tech detail: `ARCHITECTURE.md` (`exchange.ts`/`bounties.ts` rows + agent-surface table).
...
- **Agent tool surface** (`agent-gateway.ts`, same in-process forwarder as exchange/bounties): `GET/POST /api/agent/:s/bazaar/{tools.json,:tool}` → `bazaar_browse` (read) · `bazaar_list` · `bazaar_buy` · `bazaar_update` · `bazaar_delist` · `bazaar_review` · `bazaar_my_listings` (read). `…/auctions/…` → `auction_browse` (read) · `auction_create` · `auction_bid` · `auction_buy_now` · `auction_cancel`. `…/marketplace/…` → `marketplace_browse` (read) · `marketplace_publish` · `marketplace_upvote` · `marketplace_install` (buy→install chain). Money stays OFF the `[ACTION:]` parser. No `PROTOCOL_VERSION` bump in this step (settlement seam only; the consolidated protocol-manual propagation is the later parity-propagation phase). The `skill_marketplace` FEATURE_GATE is GRADUATED/removed (un-paused) in all three route files.
