...
- **Seed:** `LAND_SHOWROOM` in `packages/shared/src/constants/land-showroom.ts` — pure math (no RNG/clock, multiplayer-safe). Picks every 7th starter lot (16 of 108), evenly spread around the outer perimeter; each gets a style (coastal/fantasy/driftwood, cycled), a home/shop type (alternating), and a low level (1–2, scale only). `SHOWROOM_PARCEL_IDS` is the id set.
- **Render:** `apps/web/src/lib/three/land-showroom.tsx` (mounted in `World3DCanvas` beside `LandStructures`). Reuses the proven GLB clone/normalize/ground/cull pipeline from `land-structures.tsx`; the 6 approved GLBs back it (primitive fallback if a model is missing). The "FOR RENT" sign is a single shared `CanvasTexture` ("FOR RENT" + "CLAWVILLE ESTATES", amber) on a per-lot plank+post, placed by the same edge-facing-origin math as the for-sale signs. Iris-Xe-safe (no drei Text, no ShaderMaterial, zero per-frame allocs). On the showroom lots, `land-parcels.tsx` **suppresses the FOR-SALE sign** (the lot frame stays) so they read "FOR RENT" only.
...
- 2026-06-18 — **Land "FOR RENT" showroom (new §18b.e).** ~16 outer starter lots pre-filled with deterministic model buildings + "FOR RENT" signs so the land world reads as set-up, not 180 empty for-sale lots. Client-side decoration only (no economy/DB write); advertises the existing buy → place → upgrade ("fix it up") path. New `LAND_SHOWROOM` constant (`packages/shared`) + `land-showroom.tsx` render layer (mounted in `World3DCanvas`, reuses the `land-structures.tsx` GLB pipeline, Iris-Xe-safe sign CanvasTexture); `land-parcels.tsx` suppresses the FOR-SALE sign on those lots. Hides a lot when its parcel becomes owned. Real "claim a model home & inherit its level" transfer + parcel interiors are TODO. See `3dStructure.md §13`.
