...
**Last Audited:** 2026-07-10 (Reef Race ENTRY WIRING — activity-lobby queue now enters the v2 spline race, not the legacy wager lobby; new `§18b.e`. The room page (`app/activity/[activityId]/[roomId]/page.tsx`) unconditionally rendered the `<LobbyLanding>` SOL-wager gate first, so a matchmade Reef Race (`/activity/reef-race/:roomId?shortCode=…`) stranded the racer on "CREATE OR JOIN A LOBBY" and never reached the countdown → 2-lap → boost-pad/mini-turbo race. Fix: seed `lobbyGate='in-game'` when `activityId==='reef-race' && NEXT_PUBLIC_REEF_RACE_USE_SPLINE==='true'` (flag OFF + Bumper Shells keep the exact wager gate; v1 path not deleted). Client HUD `TOTAL_LAPS` aligned 3→2 to match server `REEF_RACE_LAPS` (was showing `/3` at the line). Seed scripts fixed for the live vCLAW schema (bot-avatar raw INSERT zeroes soft/bought/earned so `avatars_vclaw_balance_sum` holds — `soft_balance` DEFAULTs 1000). Browser-verified: queue → new spline HUD (boost meter / boost pads / mini-turbo / item boxes / urchin hazard), adversarial pass APPROVED. NEEDS founder real-keyboard playtest (Rule E4). Same-diff: seed scripts. **Prior Last Audited:** 2026-07-10 (Reef Race MECHANICS — boost pads + surf-carve mini-turbo + `rr-ink-slick`/`rr-whirlpool` item-bug fixes; new `§18d.v7` + `§18g` SPEC 4. Server-authoritative spline-sim only, all boosts capped ≤ 1.85× / 925 wu/s; new wire fields `reefSplineZones` + `miniTurboCharge/Level/boosting` deltas + `event.boost_pad`/`event.mini_turbo_fire` (no `PROTOCOL_VERSION` bump — activity WS frames). Codex physics co-review per Rule E3. Reef sim/anti-cheat/bot tests GREEN; NEEDS founder playtest (Rule E4). Same-diff: `3dStructure.md §10b`; 3-surfaces (Nori/SKILL.md) flagged to reef lead.) **Prior Last Audited:** 2026-06-27 (Reef Race activity gameplay loop — human/agent queue parity confirmation + `earlyBotFill` solo-queue tunable; see §18a.q). The activity queue join (`POST /api/activities/:id/queue`) already works for BOTH a logged-in human (Lucia cookie) AND a connected/hosted agent (`X-Clawville-Agent-Session`): both resolve through `requireAuthOrAgentSession` to the bound active avatar, queue with the right `subjectType` (`agent` carries `agentId`), and earn REAL CT + leaderboard via `reward-pipeline.ts` `activity.match.placed` (per non-bot participant) — Rule E5 write-path parity, confirmed + documented, NO code change on the auth path. NEW: a SOLO racer no longer waits ~6s for the field — `reef-race` opts into a new `earlyBotFill` flag (`packages/shared/src/activities/activities.ts`) so the matchmaker (`activity-queue.ts`) backfills 3 bots at ~3s (`QUEUE_TIMEOUT_MS`) instead of ~6s (`EXTENDED_TIMEOUT_MS`); every other activity (bumper-shells, …) is unchanged. Server-only matchmaker tunable; no schema/route/`PROTOCOL_VERSION` change (the agent in-world ACTION-surface exposure for activity queueing is a separate protected-partner-surface workstream). `bun run build` 9/9 ✓. Same-diff: `ARCHITECTURE.md`. NEEDS founder in-browser playtest (solo → 3-bot fill in ~3s; agent-session queue join) (Rule E4). **Prior Last Audited:** 2026-06-23 (Reef Race v6 — "WIDE SURF ROAD": real waves + 4-player-wide river + watertight canyon; new `§18d.v6`). Founder: "the river is WAY TOO THIN for 4 players … ACTUAL WAVES not FLAT PLANES … OBVIOUS HOLES between canyon and water." Three fixes on the same v5 floating-ribbon-in-a-void concept (float/spline/elevation/banking/dusk KEPT): (1) **WIDER + ENLARGED ring** — `reef` agent rebuilt the shared track (`packages/shared/src/reef-race/track-layout.ts`): 32 CPs, arc 88 052 wu (was 60 257), 30 reversals (was 26), corridor half-width 738–1038 wu → water **1 476–2 077 wu WIDE** (was 559–960, fits 8 sim bodies), min R 2 087 → carve margin (R−hw) 1 282 wu so the wide corridor can never wall-clamp (the v5 trap; re-confirmed by the 8-body smoke). `BANK_GAIN` re-tuned 9.0→0.016 (the old gain pegged the v6 ribbon at the 28° cap). Lap budget 200→300 s (arc-grounded). (2) **REAL WAVES** — `3da` sub-team rebuilt `surf-ribbon.tsx`: the old water had 2 verts per cross-section (flat triangle = the "flat planes"); now a dense 28×224 grid driven by a 6-octave Gerstner model (peak-to-trough ~170 wu, 20× the old) + Jacobian normals + whitecaps, WebGL2 GLSL ShaderMaterial on a plain Mesh. (3) **WATERTIGHT** — new `surf-cross-section.ts` contract (water wave taper-to-0 at banks + 40 wu canyon seal-drop) + a latent canyon banked-Y sign-flip fixed; analytic proof 0.000000 wu edge deviation, 0 holes. Reef suites 292 pass / 0 fail (3 pre-existing machine-flaky perf budgets, identical on baseline); build 9/9; render CDP-screenshot-verified. Sim/anti-cheat LOGIC unchanged (auto-derives from spline). Same-diff: `3dStructure.md §10b`, Nori `town-guide.ts`. NEEDS founder in-browser + real-keyboard playtest (Rule E4). **Prior Last Audited:** 2026-06-23 (Reef Race v5 — "SURF ROAD" floating Rainbow-Road ribbon reframe; `§18d.v5`). NO land/island — a glowing FLOATING WATER RIBBON in a cosmic VOID + a render-only elevation profile; superseded by v6's wider/wave rebuild. **Prior Last Audited:** 2026-06-23 (Reef Race v4 — WATER-DOMINANT track redesign; new `§18d.v3`). Founder art-direction on `/preview/reef-race-v2`: the loop read as a narrow blue creek in a huge unused green LAND disc (terrain even covered the water). Rebuilt the shared closed-ring track to be WIDE + BIG + WINDY so the WATER is the play surface: **27 CPs** (was 20), **arc 53 506 wu** (was 30 434), corridor **half-width 471–910 wu** (was 290–540 → ~940–1820 wu wide water channel), footprint **≈ 15 400 × 15 300 wu** (was ~10 400 × 9 000 — fills the play area), **12 curvature reversals** (was 4), min radius 378.7 wu, min corridor-edge clearance 1 292 wu. Lap count **3 → 2** (the loop is ~75% longer; 2 laps ≈ 4–4.5 min keeps the 2–4 min target) with a new arc-grounded **per-lap 180 s** soft budget (the old 90 s ellipse cap would DNF everyone mid-lap-1). Spawn grid widened for the wide start straight. Sim/bot/anti-cheat LOGIC unchanged (they derive geometry from the spline at runtime). Render recomposed by the `3da` sub-team same-diff (ground frames the water, recede outer disc, island bumped, camera re-framed for ±8000, stale linear-track props removed). Reef suites GREEN in isolation + `bun run build` 9/9. NEEDS staging + in-browser parity + founder real-keyboard playtest (Rule E4). Same-diff: `ARCHITECTURE.md` + Nori `town-guide.ts`. **Prior Last Audited:** 2026-06-23 (Reef Race Phase 1a-BACKEND — CLOSED-LOOP curvy LAP foundation; new `§18d.v2`). Reef Race's founder-locked redesign: the dead-straight open slalom is now a CURVY CLOSED RING (1 lap = one 360° loop around a central island; race = `REEF_RACE_LAPS=3`). Backend foundation shipped: the shared track became a closed Catmull-Rom ring (20 CPs, arc 30 434 wu, +2π heading sweep, min radius 304 wu, NW S-chicane + far-west hairpin, no self-intersection) consumed `{ closed: true }` by the server sim + client ribbon + bot + anti-cheat (they previously built an OPEN spline over closed CPs); the spline sim got per-body `lap`/within-lap `progress`/`startCrossed`, finish-on-lap-N-then-cross, (lap,progress) live position, lap/finish events, and a spawn fix (grid anchored at `centerlineAt(0)` not world origin — it was dropping karts ~3290 wu off-track in the island centre); timeouts scaled by lap count; anti-cheat made seam-wrap-aware (forward wrap ≠ regression; arc-length segment floors); the synced 3-2-1 countdown (`ensureSyncedCountdown`) was ported from the un-merged `d490501f`; and the wire types gained `lap`/`position`/`totalLaps`. Free leaderboard-only (CT stakes spec'd for later). 3D ribbon (visible seam) + responsive prediction/water/surfer are SEPARATE later passes. 324 reef+countdown tests pass / 0 fail; `bun run build` 9/9. NEEDS staging deploy + founder real-keyboard playtest (Rule E4). Same-diff: `ARCHITECTURE.md` + Nori `town-guide.ts`. **Prior Last Audited:** 2026-06-22 (land-world-parity — reactive parcel state + 3D click-to-buy bridge). `LandStateHydrator` (headless, `lib/three/land-state-hydrator.tsx`) mounts in `World3DCanvas`, fetches all parcels in parallel on load + on `invalidateQueries(['landParcels'])` after buy/claim/place, writes keyed-by-parcelCode into `useLandStore.parcels`. Store key bug fixed: `structures` Map now keyed by `parcelCode` (not DB UUID); `PlacedStructure` carries both. `useGameStore.openLandOffice(parcelCode?)` extended — 3D click on a FOR-SALE sign calls `openLandOffice(parcelCode)` → modal opens For-Sale tab auto-scrolled to the matching parcel with a cyan glow. New `§18b.g`. **Prior Last Audited:** 2026-06-22 (F4 — blackjack "bust shows but table is stuck" freeze). A player who busted could land in a dead-end: the over-21 result showed but the table was frozen with no "Next Hand" button. Root cause = a one-off client race (NOT the server — a live-API repro confirmed a busting hit settles correctly): a stale `GET /hand/current` read, taken in the server's narrow commit→settle window where the busted hand is momentarily `status='in_progress'` + terminal, re-seeded the modal into `player-turn` on a terminal hand AFTER `applySettled` had run — so Hit/Stand are disabled (`activeResolved`) and the "Next Hand" button (settled-phase-only) never renders. Fix (client-only, `BlackjackModal.tsx`): (1) ROOT-CAUSE GUARD — track settled `handId`s; `restoreHandFromServer` ignores a "live" hand we already settled (a stale read can't overwrite a settled hand); (2) SELF-HEAL — if ever in `player-turn` with every sub-hand terminal but not settled, re-derive authoritative server state (which has settled the hand) to recover to betting (once per handId, gated on nothing-in-flight, no loop). Server settlement/ledger UNCHANGED. Human + agent parity preserved (autonomous auto-advance untouched; guests can't reach the race — their `/hand/current` 401s). NEEDS staging deploy + founder cold-test (Rule E4). **Prior Last Audited:** 2026-06-20 (Poker CASH (ring) games — P1 wire + money atomicity). New `### 18a.m.` documents the classic fixed-blind CASH poker tables (chips == CT 1:1; SIT debit / LEAVE cash-out credit; RAKE 0 in P1) alongside the MTT tournament — full human↔agent Rule E5 parity from day one (Lucia OR `X-Clawville-Agent-Session` → bound avatar, NO guest demo tier on a CT ring table). Backend made SHIPPABLE: route mounted at `/api/cove/poker/cash`, schema exported, and the previously-non-transactional money writes (`seatSubject`/`cashOutSeat`/`settleHand`) now each run in ONE `db.transaction` with `FOR UPDATE` + the ledger debit/credit composed into the same tx (no CT-debited-no-seat / double-cash-out). Per-hand settle now writes `cove_game_events` (`gameType='poker'`) so a cash hand records to cross-game history + the provable-fair `/verify` (poker branch) + the economy monitor, under the SAME {user,agent} resolution as the write path. The agent ACTION/tool surface + `PROTOCOL_VERSION` bump are a LATER phase (no bump this slice). NEEDS staging deploy + browser sign-off (Rule E4). **Prior Last Audited:** 2026-06-21 (founder batch 2 — chat brevity F3 + hosted-agent reconnect F2). **Chat brevity (F3):** building-teacher + Nori chat replies were rambling multi-paragraph essays; the agent runtime now appends a GLOBAL strict brevity directive ("reply in 1-3 short sentences, plain prose, no greetings/lists/monologues, expand only on an explicit go-deep request") to EVERY location + system-agent system prompt in `convertToElizaCharacter` so it can't drift per-template, plus a `maxTokens` backstop (~200, vs the old 1000) applied to the human↔teacher + human↔Nori chat routes (a `conversational` flag on `processMessage`) — avatar-chat + connected-agent paths keep the larger budget so longer/action-emitting replies are never truncated. **Hosted-agent reconnect (F2):** chatting with a HOSTED agent (a Milady/Hermes ElizaOS runtime that IS the avatar — `/me/agent-session` `mode: 'hosted'`) wrongly surfaced a "Reconnect your agent to chat as it" prompt; there is no external bearer to lose and nothing to reconnect (the avatar already IS the agent, and chatting already routes to it). `avatar-chat-bar.tsx` now reads the cached agent-session `mode` and suppresses the paired-no-bearer reconnect affordance for hosted avatars (external/idle/expired agents still get the correct reconnect CTA). Detail in `ARCHITECTURE.md` 2026-06-21 batch-2. NEEDS live staging verify + founder cold-test (Rule E4). **Prior Last Audited:** 2026-06-21 (building-teacher persona REAL fix + chibi avatar create). **Persona (F1):** the 2026-06-20 template-rename below was necessary but INSUFFICIENT — every building teacher AND Nori still chatted as **Pearl the teen whale** because the agent RUNTIME ignored the seeded persona: a seeded agent carries `config:{}`, so the ElizaRuntime constructor fell back to the `cron-automation` (Pearl) template for EVERY location-agent + Nori and rebuilt system/examples from it, discarding the correct per-character `customization`. FIXED in the runtime — `convertToElizaCharacter` is now customization-first + the `'cron-automation'` fallback is removed (detail in `ARCHITECTURE.md` 2026-06-21) — so each teacher + Nori now chat IN their own character. Reproduced + proven by a regression test (a "Mr. Krabs" agent can never emit a Pearl line). **Avatar create (F5):** the two **chibi** avatars (Eliza Chibi, Milady Chibi — shown under the Milady tab at `/create-agent`) are now first-class selectable; previously picking one silently created a default Milady because the chibi keys were missing from the shared model registry. `create-agent/personality/page.tsx` now shows an error instead of silently substituting an unknown picked avatar. Known follow-up: edit-appearance can't yet swap a milady-harness avatar TO a chibi (milady-binary pool — hosting-contract semantics, needs a product call). NEEDS live staging persona verify + founder cold-test (Rule E4). Prior audit follows.
...
### 1e. Fast travel — World Map (warp) + spawn preference (town-fast-travel, 2026-06-19)
...
| `<WorldMapModal>` (dynamic) | Large fast-travel WARP map (§1e). Building beacons + player blip + owned-parcel diamonds; hover holo card + Quick Travel; click-to-warp. Gates on `worldMapOpen`; warp gated to `controlMode==='player'`. |
| `<WarpOverlay>` | DOM/CSS warp transition (§1e). Gates on `warpTarget`; teleports at the flash midpoint, unmounts at the end. Iris-Xe-safe (no shader). |
| `<SpawnOnLoad>` | Invisible — repositions a logged-in `spawnPreference==='home'` player to their owned home parcel on load (one-shot, race-safe). §1e. |
...
### 18a.q — Matchmaking auth + bot fill (human/agent parity)

**Auth (Rule E5) — `POST /:id/queue` + every identity-required route** (`leave-queue`, `queue-status`, `party/*`, `rooms/:id/state`, `rooms/:id/results`, `results/:id/acknowledge`, `leaderboard/me`): `requireAuthOrAgentSession` resolves EITHER a Lucia human OR a connected/hosted agent (`X-Clawville-Agent-Session` → `resolveAgentSession`) to the **bound active avatar**, then enqueues with `subjectType: 'human' | 'agent'` (+ `agentId`/`userId`). A connected/hosted agent queues, races, and settles exactly like a human — `reward-pipeline.ts` emits `activity.match.placed` per NON-bot participant, so agents earn real CT + leaderboard. An unbound/anonymous agent gets 403 (never a guest demotion); bots (`subjectType:'bot'`) never earn CT or leaderboard points. The WS join already resolved both identities (`resolveActivityIdentity`); the HTTP queue path matches it. (The in-world agent ACTION-surface exposure for activity queueing — Hatcher `[ACTION]` whitelist / tools.json + `PROTOCOL_VERSION` — is a separate protected-partner-surface workstream, not wired here.)

**Bot fill timing:** the matchmaker (`activity-queue.ts` `matchActivity`) holds for human fill up to `QUEUE_TIMEOUT_MS` (~3s), then graduates at `queueMinPlayers`. Bot backfill normally turns on at `EXTENDED_TIMEOUT_MS` (~6s). Activities that set `earlyBotFill: true` in their `ActivityDefinition` (currently **Reef Race**, which needs 4 to start) turn bot backfill on at ~3s instead, so a solo queuer gets a full 4-racer grid in ~3s rather than staring at a 6s empty lobby. Bumper Shells (and every non-opted activity) keep the ~6s grace. Bots fill only the `minPlayers` shortfall and require every selected entry to allow backfill (`allowBotBackfill`).
...
### 18b.e — Room entry: matchmaking scene vs legacy wager-lobby gate (2026-07-10)

`apps/web/src/app/activity/[activityId]/[roomId]/page.tsx` gates the 3D scene behind `lobbyGate === 'in-game'`; by default it renders the reusable **`<LobbyLanding>`** wager screen (`components/game/lobby-landing.tsx` — create/join a `/api/wager/lobbies` lobby, pick a SOL wager or Solo-vs-Bots, lock → scene). That is the **v1 wager flow** and it is retained unchanged for Bumper Shells and for flag-OFF Reef Race.

**Reef Race v2 (spline) is a MATCHMAKING activity, not a wager lobby.** The activity matchmaker (`activity-queue.ts` → `activity-room-manager.createRoom`) creates the room and immediately transitions it `PENDING → COUNTDOWN` with an auto-timer to `LIVE` — the server sim owns the 3-2-1 countdown and runs regardless of any wager lobby. So when the spline build is on, interposing `<LobbyLanding>` after the ActivityLobbyModal queue-match navigation (`/activity/reef-race/:roomId?shortCode=…`) stranded the racer on the wager screen and the new race (countdown → 2 laps → boost pads / mini-turbo / item boxes) was never reached. **Fix:** the page computes `useSplineReef = activityId === 'reef-race' && NEXT_PUBLIC_REEF_RACE_USE_SPLINE === 'true'` and seeds `lobbyGate` to `'in-game'` in that case, mounting the race scene immediately (the shortCode-recovery + participant `/state` gate + WS-enable all sit above the lobby gate and still run). **Flag OFF and every other activity (incl. Bumper Shells) keep the exact production wager-lobby gate; the v1 wager path is not deleted.** PARITY: no new economy write — matchmade Reef Race charges no CT entry fee and `activity.match.placed` stays server-side (§18a.q). HUD lap-count fallback (`TOTAL_LAPS`, client `reef-race-config.ts`) was aligned 3 → **2** to match the server `REEF_RACE_LAPS`, so the counter reads `/2` at the line before the first per-body delta arrives (was showing a stale `/3`). Seed scripts (`scripts/seed-activities.ts`, `scripts/seed-bot-avatars.ts`) fixed for the live vCLAW schema — the bot-avatar insert now zeroes all four balance columns via raw SQL so `avatars_vclaw_balance_sum` (claw_tokens = soft+bought+earned; `soft_balance` DEFAULTs 1000) is satisfied.
...
### 18d. Reef Race — game design

**TWO sim paths, gated by `REEF_RACE_USE_SPLINE` (server) + `NEXT_PUBLIC_REEF_RACE_USE_SPLINE` (client):**
- **LEGACY ellipse (flag OFF):** the bespoke oval + 12-checkpoint sequence below.
- **v2 CLOSED-LOOP ring (flag ON; `/preview/reef-race-v2` hardcodes ON):** the curvy lap circuit in §18d.v2. **This is the founder-locked direction** (Reef Race becomes "Mario Kart but surfing"). **⚠️ The track + scene are SUPERSEDED by §18d.v6 — "WIDE SURF ROAD" (2026-06-23): a Rainbow-Road glowing FLOATING WATER RIBBON in a cosmic VOID, NO land/island, now WIDE for 4-player racing (water 1 476–2 077 wu wide) with REAL Gerstner waves + a watertight canyon. 32 CPs / arc 88 052 wu / 30 reversals / render-only elevation profile.** §18d.v2–v5 describe the prior (narrower/land-disc) framing and the unchanged lap/position/finish machinery.

#### 18d (legacy ellipse path)

| Spec | Value |
|---|---|
| Players per match | 4–8 |
| Track | Bespoke oval, half-axes `A=1650, B=1050` |
| Laps | 3 |
...
| Perfect race bonus | +25 tokens when `bestStreakThisMatch >= TOTAL_CHECKPOINTS_PER_RACE` (= `REEF_CHECKPOINT_COUNT 12 × REEF_LAPS`; **24** in the v4 2-lap race, was 36 at 3 laps) |

#### 18d.v2 — CLOSED-LOOP curvy LAP circuit (Phase 1a-BACKEND, 2026-06-23)

Founder-locked redesign: the dead-straight open slalom became a **curvy CLOSED RING** winding 360° around a small central island. **1 lap = one full circumnavigation.** The track reads as a real circuit — sweeping bends + chicanes + a tight-but-carveable hairpin — not a plain circle and not a straight river. **⚠️ The specific track numbers + lap count in this v2 table are SUPERSEDED by §18d.v3 (the 2026-06-23 WATER-DOMINANT redesign): 27 CPs / arc 53 506 wu / hw 471–910 / 12 reversals / `REEF_RACE_LAPS=2`. The structural lap/position/finish/anti-cheat machinery below is unchanged.**

| Spec | Value |
|---|---|
| Track | Closed Catmull-Rom ring, 20 control points (`new ReefSpline(track, { closed: true })`). Verified on the real spline: **arc 30 434 wu**, heading sweep **+2π** (full circle), **min radius 304 wu** (carveable; >192 floor), **4 curvature reversals** (chicane + hairpin), no self-intersection (min 432 wu). Start/finish straight WIDE (540 halfWidth) at XZ (-1600,-4300), bends tight (~290). |
| Laps | `REEF_RACE_LAPS = 3` (alias of `REEF_LAPS`). A race finishes when a body completes lap 3 then crosses the start/finish line. |
| Lap counting | Per-body `lap` (completed-lap count) + within-lap `progress` (0..1, wraps at the seam). The FIRST forward seam crossing is the "start gun" (bodies spawn just behind the line) and does NOT count as a lap. |
| Position | Live ordering by (lap desc, then within-lap progress desc); finishers by finish time; DNF last. Sent on the wire as `position` (1 = leading). |
| Finish/lap events | `event.lap_completed` on each non-final lap (real `splitMs` + `totalMs` + `totalLaps`); `event.crossed_finish` on the final lap; `activity.match.placed` (unchanged) on finish → free leaderboard. |
| Timeout | Scaled by laps: `REEF_RACE_LOOP_SOFT_TIMEOUT_MS = 90 s × 3 = 270 s` soft + 30 s grace (a 3-lap ring race is ~277 s; the single-loop 90 s cap would DNF everyone mid-race). |
| Countdown | Synced 3-2-1 re-anchored on WS connect (`ensureSyncedCountdown`) so client navigation latency can't burn the countdown. |
| Anti-cheat | `validateProgressMonotonic` treats a forward seam wrap (t 0.99→0.01) as forward progress; `validateSegmentTime` reads the `REEF_RACE_SEGMENTS` t-ranges + derives the per-segment floor from arc length on the closed spline. |
| Mechanics direction (v1 target) | drift-carve mini-turbo + boost pads, slipstream / drafting, environmental hazards (reef rocks / whirlpools / shallows). NO throwable items in v1. |
| Stakes | **Free, leaderboard-only** today (`activity.match.placed`; agents get placement credit). CT entry-fee + payout pot is spec'd for LATER (cove/land money + E5 parity contract) — NOT built. |

**Status:** the backend lap foundation (closed spline, lap/position/finish, scaled timeouts, synced countdown, anti-cheat) is in. The 3D ribbon still renders the OPEN-era shape until the dedicated render pass (a visible seam is expected); responsive client prediction + surf-carving feel + water + surfer stance are Phase 1b–d. PARITY: human + connected/hosted agent both race via the same WS sim + the same `activity.match.placed` leaderboard credit (no new write path in this slice).

#### 18d.v3 — WATER-DOMINANT track redesign (founder art-direction, 2026-06-23)

Founder reviewed the v3 ring at `/preview/reef-race-v2`: it read as a **narrow blue creek in a huge unused green LAND disc** (terrain even covered parts of the water). Direction: make the river **WIDER**, take **MORE TURNS**, and **USE THE SPACE** — convert the wasted land into windy surfing track so it reads as a WATER map you surf, not a land map with a thin creek. The track was rebuilt to a **big, wide, windy closed ring** and the lap count dropped from 3 → 2 (the loop is now much longer). **This supersedes the v2-row track numbers above.**

| Spec | v4 value (supersedes v3 row) |
|---|---|
| Track | Closed Catmull-Rom ring, **27 control points**. Verified on the real spline: **arc 53 506 wu** (was 30 434), heading sweep **+2π**, **12 curvature reversals** (was 4 — sweeping bends + a NE S-ess + an upper-NE chicane + a far-west U-hairpin), **min radius 378.7 wu** (carveable; >192 floor), no self-intersection (min 2 347 wu) AND the WIDE-corridor edges never touch (min corridor-edge clearance **1 292 wu**). Footprint **≈ 15 400 × 15 300 wu** (was ~10 400 × 9 000 — the loop now fills the play area). Start/finish straight at XZ (-2600,-7300), south. |
| Corridor width (the WATER) | **half-width 471–910 wu** (was 290–540) → the water corridor is **~940–1820 wu wide** — a real surf channel for 4–8 racers side-by-side + overtaking lines. The corridor IS the water; land only FRAMES it (the render's ground ribbon starts at the corridor's outer edge). |
| Laps | **`REEF_RACE_LAPS = 2`** (was 3; alias of `REEF_LAPS`). One loop ≈ 125–160 s on the big ring, so 2 laps ≈ 4–4.5 min (a 3-lap race would be ~7–8 min, over the 2–4 min target) while keeping a multi-lap position battle. |
| Timeout | Per-lap **`REEF_RACE_LOOP_LAP_BUDGET_MS = 180 s`** (grounded in arc / slowest-legit pace ~330 wu/s × 1.10 safety — NOT the stale 90 s ellipse value, which would DNF everyone mid-lap-1 on the big ring); `REEF_RACE_LOOP_SOFT_TIMEOUT_MS = 180 s × 2 = 360 s` soft + 30 s grace. |
| Start grid | Widened for the WIDE start straight: 2 columns 640 wu apart, 120 wu back-stagger (was 180 wu apart / 70 wu) — a proper wide starting grid, all in-corridor. |
| Render (water-dominant) | `/preview/reef-race-v2` recomposed by the `3da` sub-team same diff: the ground ribbon frames the water at `hw + 150` out to `+ GROUND_W`, the outer green disc is receded, the central island is bumped to fit the bigger inner radius, the camera is re-framed for the ±8000 footprint, and stale linear-track props (canyon walls / fixed-Z finish gate / distance markers / bridge) are removed/relocated. |
| Sim/bot/anti-cheat logic | UNCHANGED — they derive arc / segment floors / spawn anchor / lookahead from the spline at runtime, so they auto-adapt to the new geometry. Only the timeouts + lap count + spawn spacing are tunables. |

**Status (v4):** SUPERSEDED by §18d.v5 below. The water-dominant land-disc framing was replaced when the founder reframed the whole concept to a floating Rainbow-Road ribbon (no land).

#### 18d.v5 — "SURF ROAD" floating Rainbow-Road ribbon (founder reframe, 2026-06-23)

The founder reframed the entire vision after the two land-disc attempts: **"Think RAINBOW ROAD. A floating river not bound by land — the land is irrelevant, the SURFING is the piece. Not zig-zaggy enough, not using the space."** There is **NO world to circle, NO island, NO land map.** Reef Race is now a glowing **FLOATING WATER RIBBON** winding through an abstract **cosmic VOID** — the ribbon IS the world, hanging in space with neon-glowing banked rails. Aggressively twisty, and it **UNDULATES in elevation** (climbs / dips / banks) like Rainbow Road.

| Spec | v5 value (supersedes v2–v4 track rows) |
|---|---|
| Track | Closed Catmull-Rom ribbon, **32 control points**. Verified on the real spline: **arc 60 257 wu**, heading sweep **+2π** (one clean circumnavigation), **26 curvature reversals** (SE sweeper → east hairpin, flowing L-R-L-R S-chain, north chicane, far-west U-hairpin, mid chicane, long SW return), **min radius 261.2 wu** (carveable; >192 floor), single-winding (no XZ self-overlap; min inter-pass edge clearance 2 079 wu). Footprint **≈ 17 687 × 16 941 wu** (sprawls). Start/finish at XZ (-2400,-8200), south. |
| Corridor width (the WATER ribbon) | **half-width 280–480 wu** (ribbon 559–960 wu wide) — a banked Rainbow-Road ribbon. Widths were tuned UP from a first sharper pass: narrow esses WALL-CLAMP the sim physics (a kart that can't carve the line inside the corridor pins to the wall + stalls — caught by the 8-body integration smoke). Every corner now holds a racing line. |
| **Elevation (RENDER-ONLY)** | `reefTrackElevationAt(t)` + `reefTrackBankAngleAt(t)` in `@clawville/shared` — render-only Y profile (Y span **1 634 wu**, max grade **29.3 %**, fully periodic / C1 at the seam) + bank lean (±28°). **The server sim stays purely 2D (XZ); laps/finish/physics/anti-cheat are UNCHANGED.** The 3D ribbon, the rider group, AND the chase camera all lift+tilt by the SAME elevation(t)+bank(t) so the surfer rides ON the ribbon and the camera frames it through climbs/drops (the "shared vertical datum" invariant, now a function of t). Per-body `heightOffset` (jump/ramp) is added on top. |
| Laps | **`REEF_RACE_LAPS = 2`** (unchanged). One loop ≈ 145–175 s, 2 laps ≈ 4.8–5.8 min. |
| Timeout | Per-lap **`REEF_RACE_LOOP_LAP_BUDGET_MS = 200 s`** (re-grounded: 60 257 / 330 wu/s × 1.10); soft = 200 × 2 = 400 s + 30 s grace. |
| Render (SURF ROAD scene) | `3da` sub-agent same diff: **land/island/ground DELETED**; new glowing water ribbon (`surf-ribbon.tsx`, analytic GLSL on a plain Mesh) + neon banked rails + crest glow; **cosmic void backdrop** (`cosmic-void.tsx` — gradient dome + starfield + drifting glow motes, Points/MeshBasic only); **selective bloom** (`surf-bloom.tsx`, half-res UnrealBloomPass) on the emissive rails/crests; render-only elevation/bank module (`reef-race-elevation.ts`) with a cached XZ→t lookup. Iris-Xe-safe (no InstancedMesh+ShaderMaterial, no drei Text/Billboard, no per-frame allocation). |
| Sim/bot/anti-cheat logic | UNCHANGED — derive arc / segment floors / spawn anchor / lookahead from the spline at runtime; only timeouts + lap budget are tunables. |

**Status (v5):** SUPERSEDED by §18d.v6 below — the v5 ribbon was **TOO THIN for 4 players** (water 559–960 wu wide) and the water read as flat planes. v6 widens it and rebuilds the water as real waves.

#### 18d.v6 — WIDE SURF ROAD: real waves + 4-player-wide river + watertight canyon (founder, 2026-06-23)

The founder, verbatim: **"The river is WAY TOO THIN, not nearly enough for 4 players … ACTUAL WAVES — there's variation but it's just a bunch of FLAT PLANES … the rocks — there are OBVIOUS HOLES in the fucking ground between canyon and water."** Three fixes, all on the same v5 floating-ribbon-in-a-void concept (float / winding spline / elevation undulation / banking / dusk backdrop all KEPT):

| Spec | v6 value (supersedes the v5 track row) |
|---|---|
| Track (WIDER + ENLARGED) | Closed Catmull-Rom ribbon, **32 control points**. Verified on the real spline: **arc 88 052 wu** (was 60 257), heading sweep **+2π**, **30 curvature reversals** (was 26 — broad alternating sweeps: SE sweep → wide east bend → L-R sweeps → north sweep → far-west bend → SW return), **min radius 2 087 wu**, single-winding (min inter-pass edge clearance **4 342 wu**). Footprint **≈ 30 648 × 25 926 wu** (was ~17 687² — uses far more space). Start/finish at XZ (0,-11 425), south. |
| Corridor width (the WATER — THE founder fix) | **half-width 738–1038 wu** → water surface **1 476–2 077 wu WIDE** (was 559–960). Even the narrowest point fits **8 sim bodies** (REEF_BODY_RADIUS 22 → 44 wu diameter) side-by-side — well past the 4-board target, with overtaking + surf room. THE WALL-CLAMP FIX: a wide road can only carve a corner of radius R if `R − hw > 192` (carve floor), else the sim wall-clamps + stalls (the v5 trap). v6's broad sweeps give a **carve margin (R − hw) of 1 282 wu** — wall-clamp is now geometrically impossible (re-confirmed by the 8-body integration smoke producing a finisher). |
| Water (REAL WAVES — flat-planes fix) | `surf-ribbon.tsx` rebuilt: the old water had only **2 verts per cross-section** across a ~2000 wu river (the mid-channel was one flat interpolated triangle — the "flat planes" root cause). Now a dense **`WIDTH_SEGS=28 × RIBBON_SAMPLES=224` grid (~12.5k tris)** driven by a 6-octave **Gerstner/trochoidal** model (sharp pinched crests + broad troughs; peak heave ~85 wu / peak-to-trough ~170 wu, 20× the old ≤4 wu; every wave `Q·w·A<1` so no cusp) with analytic Jacobian normals (crest-riding specular) + Jacobian-compression **whitecaps**, swell flowing downstream. Iris-Xe-safe GLSL `ShaderMaterial` on a plain Mesh (WebGL2 — `threejs-water-optics` path, NOT the WebGPU FFT `threejs-spectral-ocean`). |
| Watertight (HOLES fix — the founder's "more important" piece) | new `surf-cross-section.ts` contract: the water's wave displacement **tapers to 0** within `WATER_EDGE_TAPER`(0.10) of each bank so the edge verts pin to the static banked datum the canyon shares, AND the canyon inner base drops `WATER_SEAL_DROP`(40 wu) below it (submerged sealing lip). Canyon raised to `CANYON_SAMPLES=224` to match the water grid + a latent **banked-Y sign-flip** in the canyon wall fixed (`cy − uny·hw·side` → `cy + uny·hw·side`, which diverged from the water edge by 2·uny·hw on every banked turn — the real hole source on leans). Analytic proof: water-edge↔canyon-base XZ deviation **0.000000 wu** both banks, rock 40 wu below water everywhere, 0 samples with rock above water. |
| Elevation / banking (RENDER-ONLY, KEPT) | same `reefTrackElevationAt(t)` (Y span 1 634 wu; grade now **14.8 %**, gentler because the arc is ~46 % longer) + `reefTrackBankAngleAt(t)`. **`BANK_GAIN` re-tuned 9.0 → 0.016** — the v5 gain pegged the whole v6 ribbon at the 28° cap (a violently rolling ribbon); now near-flat on straights, ~12–18° into the tighter sweeps. Sim still purely 2D; the ribbon/rider/camera share the datum. |
| Laps | **`REEF_RACE_LAPS = 2`** (unchanged). One loop ≈ 215–266 s on the wider/longer ring. |
| Timeout | Per-lap **`REEF_RACE_LOOP_LAP_BUDGET_MS = 300 s`** (re-grounded: 88 052 / 330 wu/s × 1.10 ≈ 294 s); soft = 300 × 2 = 600 s + 30 s grace. |
| Sim/bot/anti-cheat logic | UNCHANGED — derive arc / segment floors / spawn anchor / lookahead from the spline at runtime; only the track CPs + widths + arc constant + lap budget + bank gain are tunables. The 5 themed segments re-pinned to the new CP→t boundaries (lagoon 0–0.083 / kelp –0.273 / shipwreck –0.436 / coral –0.728 / finish –1.0). |

**Status:** built — needs verification. Track + sim params verified by tests (reef suites **292 pass / 0 fail** [3 pre-existing machine-timing-flaky perf budgets, confirmed identical on the clean baseline], incl. the 8-body integration smoke producing a finisher on the wide ring — no wall-clamp) + `bun run build` 9/9. Water + watertight render rebuilt by the `3da` sub-team same-diff (CDP-screenshot-verified: wide channel of 3D wave crests + whitecaps, no holes, dusk void + bloom intact). NEEDS the founder's in-browser check at `/preview/reef-race-v2?mode=cinematic` (wide river / real waves / no holes) + real-keyboard playtest (Rule E4). PARITY unchanged: human + connected/hosted agent both race via the same WS sim + `activity.match.placed` credit (no new write path; free leaderboard-only).

#### 18d.v7 — RACE MECHANICS: boost pads + surf-carve mini-turbo + item-bug fixes (2026-07-10)

Competitive-feel layer added to the spline sim (`reef-race-spline-sim.ts` + `reef-race-config.ts`), server-authoritative, fixed 30 Hz, all CAPPED so nothing chains into infinite speed. Spline-sim only (`REEF_RACE_USE_SPLINE`); the ellipse sim is untouched. Builds on the already-shipped ramps / manual jump / slipstream / pickups (§18d.v2 machinery).

**DETERMINISTIC SIM CLOCK (backbone, Codex round-2 finding 6):** the tick loop no longer reads `Date.now()`. `state.simTimeMs += REEF_TICK_MS` each tick (seeded at `startedAt`) and `now = state.simTimeMs` drives EVERY expiry/cooldown (launch/slip/pad/mini-turbo/items/ramp/pickup-respawn/finish-wait/timeouts). Identical input+tick sequences produce identical trajectories regardless of event-loop stalls. (The pre-existing wall-clock effect system is superseded by the sim clock for the spline sim.)

| Mechanic | Rule |
|---|---|
| **Boost pads (NEW)** | 4 spline-placed pads (`buildSplineBoostPads`, t=0.15/0.42/0.58/0.85, offset ±90 wu to one side so the pad line is a choice). Trigger is **ENTRY-EDGE latched** (`state.boostPadInside` set): a body fires only on a grounded outside→inside transition, never re-fires while sitting/coasting inside, and the latch clears only on a grounded exit (a jump over the pad is not an exit — no bunny-hop refire). **Airborne bodies are ignored** (floor pads have no vertical reach). On fire: an instant along-heading velocity KICK (`BOOST_PAD_KICK=160` wu/s, clamped to the 925 wu/s hard cap) **plus** a timed decaying `pad-boost` speedMod (`+0.30` for 1.5 s). Fires for bots too. The kick is applied in a POST-integrate pass so the intra-tick velocity-delta validator never mis-flags it. Broadcasts `event.boost_pad`. |
| **Mini-turbo — surf-carve whip (NEW, signature verb)** | Drift is retired (its bit → JUMP), so the "release for a boost" beat maps onto sustained CARVING. A hard sustained turn in ONE direction (`|Δheading|/tick ≥ 0.035 rad ≈ 2°/tick`, speed ≥ 175 wu/s, thrust on, grounded) CHARGES a meter (`updateMiniTurbo`, caps at 1100 ms; 480 ms = tier 1, 1100 ms = tier 2). On carve BREAK it FIRES (`releaseMiniTurbo`) a timed `mini-turbo-boost` speedMod (tier 1 `+0.22`/0.9 s, tier 2 `+0.38`/1.3 s) + `event.mini_turbo_fire`. **Anti-farm:** a `MINI_TURBO_COOLDOWN_MS=600` (sim-time) suppresses recharge after a fire, so rhythmic left-right flick-carving can't hold a continuous boost (max snaking cadence ~1080 ms > the 900 ms tier-1 duration). `integrateSurfStep` stays PURE — the boost only raises `targetForwardSpeed`, read once/tick, no double-compound with drag/grip. Bots trigger it from their own turning. |
| **Anti-cheat cap (all boosts)** | `pad-boost` + `mini-turbo-boost` + `rr-turbo-bubble` all fold into the SAME `positiveKineticStack = min(launch+slip+pad+miniTurbo+turbo, KINEMATIC_BOOST_CAP=0.85)` → speedMod ≤ 1.85, and all are in `isPositiveBoostActive` so the 925 wu/s hard cap clamps velocity every tick. Chaining cannot exceed 925 wu/s. |
| **`rr-turbo-bubble` interaction FIX (round-2)** | Turbo was ADDITIVE-folded into the stack (was `Math.max(speedMod, 1.35)`, which discarded any active slow — a whirlpool-slowed victim on turbo kept full speed). Now turbo (+0.35) + whirlpool-slow (−0.35) ⇒ 1.0× — the documented "turbo buys back the hazard" model. |
| **`rr-ink-slick` FIX** | Was set on SELF (slowed the USER) despite the HUD advertising a rival slow. Now `applyInkSlick` sets `rr-ink-slick` on RIVALS within 260 wu and BEHIND the dropper's heading (their `slicked` read → 0.30 speedMod); shielded rivals immune; a later weaker slick **never shortens** a stronger active one (`expiry = max(existing, new)`). Reuses `event.hit`. |
| **`rr-whirlpool` FIX** | The legendary was INERT. Now `applyWhirlpool` is a real AoE: rivals (≤300 wu, unshielded) are PULLED toward the center (`WHIRLPOOL_PULL_IMPULSE=250` × falloff × knockbackResist) and briefly slowed (`hazard-slow`, −0.35, 3 s). The victim's TOTAL speed is **clamped to 925 unconditionally after the pull** (the knockback bypasses delta validation, so two aligned whirlpools can't slingshot an unboosted victim over the cap). Reuses `event.hit`. |
| **Order-independence (round-2)** | Rival-hazard resolution is DETERMINISTIC + order-independent: power-up USE is deferred out of the per-body integrate loop (`pendingPowerUpSlots`) into a post-integrate `resolvePowerUpUses` pass with TWO phases — Phase 1 self-buffs (turbo/shield), Phase 2 offensive — so a shield used the same tick blocks a whirlpool regardless of body-map order, and every rival read sees one consistent post-integrate world. |

**Round-3 hardening (Codex re-review of the committed diff — 3 blockers + 1 nit, all fixed):** (a) **Offensive-hazard order-independence + seeker cap** — the rival-hazard resolution now COLLECTS every effect from the immutable post-integrate snapshot, AGGREGATES per target (impulses sum, slows multiply — both commutative), APPLIES once in a canonical order (impulse then slow), and CLAMPS once to 925 per target. This makes two effects on the same victim in one tick independent of body-map order AND caps every knockback (previously `applySeekerJelly` added an unclamped `0.6×MAX` impulse that only a later whirlpool's clamp contained by roster luck). (b) **Start-line shuttle anti-farm** — seam crossing is now BIDIRECTIONAL: a backward low→high cross is a small NEGATIVE delta that UNDOES a lap (or re-arms the start gun), so wiggling across the start/finish line can no longer farm laps to a FINISH (finish order feeds `activity.match.placed` scoring). A regression test asserts an oscillating body's `lap` stays 0 and never finishes. (c) **Landing height** — the per-body delta now emits explicit numeric `height` (including 0) so a body that lands zeroes on the client instead of keeping a stale airborne height until the next keyframe.

**Segment-time anti-cheat: NOT shipped (round-2 revert).** The dead `validateSegmentTime`/`ReefProgressTracker` were briefly wired then **UNWIRED** — Codex found it false-flagged normal front-row racers on the first start-line crossing, only validated one segment per transition (multi-segment teleports evaded), used a centerline-arc floor that legit inside lines beat, and was wall-clock. And its target exploit is unreachable (the sim takes no client coordinates), so a broken detector that logs false anti-cheat events on legit racers is strictly worse than none. The corridor walls + the 2%-per-tick progress-regression guard remain the anti-shortcut defense.

**Wire additions (protocol.ts, additive; old clients ignore):** `RoomMeta.reefSplineZones { boostPads[], ramps[] }` — world-coord pad+ramp render zones, **room-independent** (`getSplineStaticZones()` builds from the static track so `snapshot.init` during COUNTDOWN returns real zones, no client fallback-to-reconstructed-pads); per-body `EntityDelta.changed.{miniTurboCharge, miniTurboLevel, boosting}` + `WorldState.entities.{height,miniTurboCharge,miniTurboLevel,boosting}` (carried on KEYFRAMES + `snapshot.init` too, so a 1 Hz keyframe / mid-match reconnect doesn't blank the HUD meter/trail — client `hydrateFromWorld` reads them); events `event.boost_pad`, `event.mini_turbo_fire`. No `PROTOCOL_VERSION` bump (activity WS frames). Ramp render zones are now exposed too (were unrendered).

**Status:** built + tests green (reef spline-sim / ramps / integration / anti-cheat / bot suites — 104/104 on the focused run, 224/226 on the full sim glob; the 2 non-passes are pre-existing machine-timing-flaky `closestPointOnSpline` perf microbenchmarks in the UNTOUCHED `reef-race-spline.ts`). `tsc --noEmit` clean (api + web). Codex Rule-E3 physics co-review: round-1 returned BLOCKING (6 findings) → all fixed → round-2 re-verify. NEEDS founder in-browser playtest (pad pop / carve-charge-and-fire feel / ink-slick + whirlpool hit rivals) — NOT "done" (Rule E4). PARITY unchanged: same WS sim + `activity.match.placed` for human + connected/hosted agent. **3-surface sync:** Nori `knowledge[]` reef-mechanics line ADDED same-diff; connection SKILL.md / hosted-runtime manual is a staging-PR follow-up (reef is flag-OFF in prod) — routed to the reef lead.
...
| SPEC 4 | Race mechanics — boost pads (`event.boost_pad`), surf-carve mini-turbo (`event.mini_turbo_fire` + `miniTurboCharge`/`miniTurboLevel`/`boosting` on the delta), `rr-ink-slick`/`rr-whirlpool` item-bug fixes, `reefSplineZones` render zones. See §18d.v7. | 2026-07-10 |
