...
- `HatcherLaunchHandler` (`apps/web/src/components/game/hatcher-launch-handler.tsx`, mounted unconditionally on `/game`) reads the two params off `window.location.search` on mount (matching the `quickQueue` deep-link guard — not `useSearchParams`, to dodge the Next 16 prerender Suspense bailout).
- Both params present → it strips them via `history.replaceState` immediately (so a refresh can't replay the single-use `launchToken`, and the token never lingers in the URL/history) and calls `api.hatcherLaunchExchange({ agentId, launchToken })` exactly once (re-entry guard).
