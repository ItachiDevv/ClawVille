...
### 18a.f. Blackjack table (Phase 6.4.0 — display shell, 2026-05-25)

**Scope:** display + flow shell only. No ClawToken ledger writes (`transferClawTokens()` not called). Mock outcomes (deterministic cycle). Real engine (per-card decisions, provably-fair RNG, ElizaOS skill memory for hosted agents) lands in Phase 6.4.1. Connection SKILL.md + hosted-agent memory injection deferred to Phase 6.4.2.
...
**2D modal:** `apps/web/src/components/cove/blackjack/BlackjackModal.tsx`. Felt background (CSS gradient `#0d3a1e → #0a2e18`). Sub-components: `HandRow` (cards + ace-correct total), `OutcomeBanner` (win=amber, push=cream, loss=`#e85555`), `BlackjackCard` (Unicode suit glyphs, `slideIn` CSS keyframe with configurable delay). Bet chips: `BET_STEPS = [10, 25, 50, 100, 250, 500]` ClawTokens — display only in 6.4.0. Action buttons: "Deal (N CT)" is the primary active action; Hit/Stand/Double/Split/Surrender render at opacity 0.45 with `title="Available in Phase 6.4.1"`. After outcome: "Next Hand" (re-opens) + "Walk Away" (closes). ESC key closes. Local display balance tracked in component state — no ledger write.

**State:** `useCoveStore` (`apps/web/src/stores/cove.ts`) extended with blackjack slice: `blackjackOpen`, `blackjackBet`, `blackjackOutcome`, `blackjackPlayerHand`, `blackjackDealerHand`, `blackjackPayout`, `blackjackOutcomeLabel`, `blackjackIsDealing`, `blackjackDisplayBalance`. Actions: `openBlackjackTable(displayBalance)`, `closeBlackjackTable()`, `setBlackjackBet(n)`, `setBlackjackResult(...)`, `setBlackjackIsDealing(b)`.

**Types:** `apps/web/src/lib/cove/blackjack-types.ts` — frontend-local copy for 6.4.0. Canonical shared types already promoted to `packages/shared/src/types/cove-blackjack.ts` (exported from `@clawville/shared`) in Phase 6.4.0; API route imports from there. Web components will migrate to `@clawville/shared` and the local copy deleted in Phase 6.4.1.

**API client:** `apps/web/src/lib/cove/blackjack-api-client.ts` — `playMockHand(betAmount): Promise<PlayMockHandResponse>`, uses `NEXT_PUBLIC_API_URL + /api/cove/blackjack/play-mock-hand`.

**Backend route:** `apps/api/src/routes/cove-blackjack.ts`, mounted at `app.route('/api/cove/blackjack', ...)` in `index.ts`. `POST /play-mock-hand` — Zod-validated `{ betAmount: number }`, returns `PlayMockHandResponse`. Deterministic 4-outcome cycle (blackjack → win → push → loss) via module-level `_mockCycleIndex`. Zero DB writes, zero ledger.
...
- 2026-05-25 — Phase 6.4.0: Blackjack display shell. `BlackjackTableHotspot` in `cove-interior.tsx`, `BlackjackModal` + `BlackjackCard` components, `POST /api/cove/blackjack/play-mock-hand` mock route (4-cycle deterministic), `useCoveStore` blackjack slice, town-guide knowledge entry. No ledger writes. See new §18a.f.
