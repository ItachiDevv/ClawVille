...
**Last Audited:** 2026-06-10 (NPC gait sync + icon pill row): (1) **NPC walk speed raised to the player's 550 wu/s** (server `moveNpcs` baseStep 44→110 @5Hz; was 220 wu/s) — at 220, the shared walk clip (natural stride = player 550) forced either foot-slide (timeScale 1) or sluggish honest legs (0.4); per user direction "both at normal speed", NPCs now translate at the player's walking pace with legs at natural rate. The client velocity-sync (0.4s-windowed rendered speed ÷ 550, robust to demo-wander steps and 5Hz-interp jitter) stays as a guard: steady-state ≈ 1.0, adapts only when motion actually deviates. (2) **Controls + Language are icon-only circles side by side** under the shortened sidebar (Gamepad2 at right:16, Languages glyph at right:68, bottom+12; current language lives in the hover tooltip; popover opens up-left). Language no longer covers the inventory; Controls no longer floats mid-screen at right:260. Sidebar `maxHeight` calc(100vh−72px) → calc(100vh−140px). Mobile placements unchanged. PARITY: visual/UI only.
