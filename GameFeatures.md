...
| `hatcherSpectate` | boolean | True while the owner watches a Hatcher-launched agent in 'explore' (§2f). EXEMPTS them from the explore→player auto-promotion so a `useAvatar` refetch can't snap the camera off the watched agent onto their own avatar. Set by `HatcherLaunchHandler` AFTER its `setControlMode('explore')`; cleared by any explicit `setControlMode` (owner takes the wheel). |
...
### 2f. Hatcher launch entry — owner watches their agent (partner #2, 2026-06-11)

The owner-side entry point: Hatcher's dashboard "Launch" button opens `https://clawville.world/game#hatcher_agent=<agentId>&hatcher_launch=<launchToken>` so the human who hosts the agent on Hatcher can drop into ClawVille and **watch their agent play**. The portal flow logs the owner in (composes a ClawVille session) BEFORE the redirect, so the launch lands authenticated.

**The grant rides the URL FRAGMENT, not the query string (Codex finding #3, 2026-06-12).** A bearer-style `hatcher_launch` token in `?query=` is captured by CDN/web-server access logs and leaks into the `Referer` header of every early sub-resource request BEFORE the post-mount strip runs — already exfiltrated by the time it's cleared. A fragment (`#…`) is never sent to the server, never logged, and never appears in a `Referer`, so it is the correct carrier for a short-lived bearer. **Hatcher's dashboard MUST emit the grant in the fragment** — a query-string launch now silently no-ops (and would re-introduce the log leak).

**Flow (v1 — `autonomous`/observe-only):**
- `HatcherLaunchHandler` (`apps/web/src/components/game/hatcher-launch-handler.tsx`, mounted unconditionally on `/game`) reads the two params off `window.location.hash` on mount (the leading `#` stripped, then parsed as form-encoded pairs — plain string parsing, not `useSearchParams`, to dodge the Next 16 prerender Suspense bailout).
- Both params present → it strips them from the fragment via `history.replaceState` immediately (preserving any unrelated fragment keys; dropping the `#` entirely if none remain — so a refresh can't replay the single-use `launchToken`, and the token never lingers in the URL/history) and calls `api.hatcherLaunchExchange({ agentId, launchToken })` exactly once (re-entry guard).
- **Success** (`{ ok: true, agent: { agentId, name, x, y, mode } }`): land in spectate (`setControlMode('explore')`), set `hatcherSpectate=true` (AFTER the mode change, since `setControlMode` clears it) so the game-page explore→player auto-promotion can't yank the camera off the watched agent on the next `useAvatar` refetch (the launching owner is an authed non-guest who usually HAS their own avatar — without this guard they'd be force-promoted into following their own body), focus the explore camera on the agent's in-world body via the new one-shot `cameraFocusRequest` store signal (drained by `WASDCameraController` in `World3DCanvas.tsx`, which re-aims OrbitControls + camera and clears the request — minimal three-layer touch, no per-frame cost), and show a 6 s toast `"Watching <name> · launched from Hatcher"`. Camera coords are GAME-space (0..MAP_WIDTH) → world XZ via the same `−HALF_W/−HALF_H` projection the follow camera uses. The owner exits spectate by manually changing control mode (which clears `hatcherSpectate`).
- **401 / `launch_requires_session`**: a dismissable bottom-centre banner "Open ClawVille from your Hatcher dashboard to launch this agent" — no login redirect built (the portal owns login), don't dead-end silently.
- **Any other failure**: a non-blocking bottom-centre error banner; the game has already loaded normally.
- **Params absent → zero behaviour change.**

**Phasing:** v1 is observe-only — the owner always lands in 'explore' (free spectate camera) regardless of the agent's own mode; the handler never possesses the agent. The **`controlled` possession path is a separate follow-up** (the owner takes the wheel). The exchange always sends `mode: 'autonomous'`.

**Endpoint:** `POST /api/partner/hatcher/launch/exchange` (Lucia-authed; no guest exchange in v1). Wire detail + the session-hash transmit rule (`clawvilleSessionId = sha256(sessionId)`, raw session id NEVER sent to Hatcher) live in `ARCHITECTURE.md`. The launch params are attacker-controlled — the exchange's 2xx is the authorization signal, never the params alone.

**PARITY:** human path = portal-composed launch URL → spectate the agent's body; agent path = the agent is already in-world via partner registration (§2d) — the launch is the owner-side entry to *observe* it, with no economy writes in this flow, so settlement/leaderboard are unaffected.
