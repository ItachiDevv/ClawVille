...
All default wanderers are free roamers (`buildingId === ''`), including Milady, Hermes, chibi, and the single crustacean. The server planner keeps them inside the town-commons annulus (900-2400wu from center) and routes every target through the same AABB-aware pathfinder/collider map used by movement collision.
