...
**Last edit:** 2026-05-18 — Concern 6.0.4: 2D slot screen. New frontend modules: `apps/web/src/lib/casino/` (types, mock engine), `apps/web/src/stores/casino.ts`, `apps/web/src/components/casino/` (5 components). No new API routes, no DB changes. Paytable constants in `packages/shared/src/constants/slot-paytables.ts`. Prior: Concern 6.0.3 — SceneTransition fade overlay.
