...
> **Last Audited:** 2026-04-23 (Q2 Activity Portals chunk #10 — bot backfill controllers wired. New service files under `apps/api/src/services/activity/bots/` (controller interface, Bumper Shells heuristic, reservation pool). Matcher in `activity-queue.ts` now reserves bot avatarIds from a 64-avatar pool when a queue is short of `minFill` past `EXTENDED_TIMEOUT_MS=45_000`. `activity-room-manager.setEvictionFn` released bot reservations on terminal transitions. Sim added per-tick `runBotControllers()` that feeds intents through the same anti-cheat-clamped `applyInput()` pipeline humans use. Pre-seed script `scripts/seed-bot-avatars.ts` creates 64 system bot users + avatars — must run after `db:push` + `seed-activities.ts`. Reward issuance carve-out (skip `subject_type='bot'` participants) documented inline in `persistResultsTransition` for chunk #7. Sybil guards deferred. Prior audit same-day W1 — Town Guide generalized to system-agent pattern.)
