...
**Current drift note (2026-05-26):** Frontend-only `/perf` route added for 3D performance diagnosis. It mounts `World3DCanvas` with audit toggles, streams NPC state like `/game`, uses `PerfAudit.ts` to inspect `window.__W3D.scene/gl`, and exposes the current report at `window.__CV_PERF_AUDIT`. No API route or database change.
