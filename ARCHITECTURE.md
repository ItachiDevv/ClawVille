...
**Current drift note (2026-05-26):** Frontend-only `/perf` route corrected for trustworthy visual audits. It mounts `World3DCanvas` with audit toggles and now receives a perf-only camera preset through `perfFlags`, so the diagnostic viewport frames town geometry while reporting `window.__CV_PERF_AUDIT`. Label audit counters now distinguish active labels and idle React render rate. No API route or database change.
...
Frontend (`apps/web`): Next.js 16, React 19, R3F 9, Zustand. Entry: `app/game/page.tsx` → dynamically imports `World3DCanvas` (SSR disabled). Diagnostic route: `app/perf/page.tsx` mounts the same canvas with `WorldPerfFlags` and reports FPS deltas for labels/NPCs/shadows/postprocessing/water-fog-particles/static-world/UI-overlay toggles.
