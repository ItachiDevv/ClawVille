...
class FakeDb {
  ...
  // READ-COMMITTED FIDELITY: the TM's `persistHandAndBusts` updates a winner's
  // chip_stack and a loser's bust-to-0 in ONE transaction. Each `await tx.execute()`
  // yields the microtask queue, so a concurrent status read could otherwise observe a
  // HALF-APPLIED tx (winner credited, loser not yet zeroed) → a transient phantom
  // chip OVER-count that a real Postgres reader (atomic READ COMMITTED) never sees.
  // We serialize: a transaction holds `txTail`; a read (`snapshotReady()`) awaits the
  // in-flight tx so it only ever observes COMMITTED state — matching the live DB.
  private txTail: Promise<unknown> = Promise.resolve();

  async transaction<T>(fn: (tx: FakeDb) => Promise<T>): Promise<T> {
    // Chain after any in-flight tx so transactions serialize (also matches the TM's
    // FOR UPDATE row-lock serialization of concurrent registers/start-triggers).
    const run = this.txTail.then(() => fn(this));
    // Keep the tail alive even if this tx rejects, so the chain never deadlocks.
    this.txTail = run.catch(() => undefined);
    return run;
  }
  /** Await any in-flight transaction so a read sees only committed state. */
  async snapshotReady(): Promise<void> {
    await this.txTail.catch(() => undefined);
  }
...
export interface MockBackend {
  ...
  /**
   * AUTHORITATIVE chip-conservation oracle from the TM ledger (chips BEHIND across all
   * live tables, `getTotalChips`) plus whether any hand is currently in flight. Null
   * once the tournament is no longer running. `behind` ≤ expected ALWAYS (pot ≥ 0);
   * `behind` == expected EXACTLY when `!anyHandInFlight` (all chips back in stacks).
   */
...
export async function createMockBackend(opts: {
  ...
  const backend: MockBackend = {
    ...
    async status(tournamentId: string): Promise<StatusResult> {
      // Read only COMMITTED state — wait out any in-flight settle/bust transaction so
      // the chip-conservation sampler never observes a half-applied tx (real Postgres
      // readers are atomic under READ COMMITTED; the FakeDb mirrors that here).
      await db.snapshotReady();
      const t = db.tournaments.get(tournamentId);
    ...
    authoritativeChips(tournamentId: string): { behind: number; anyHandInFlight: boolean } | null {
      // Use ONE ledger — the TM's `getTotalChips` (chips BEHIND, i.e. each live seat's
      // tournament `chipStack`). Mixing the TM ledger with the sim's `snapshot.pot`
      // (a SEPARATE per-hand ledger) double-counts at the hand boundary where the sim
      // has already awarded the pot into ITS stacks but the TM hasn't yet applied its
      // deltas — so we DON'T add the pot.
      //
      // Soundness: chips behind ≤ startingStack*entrants ALWAYS (the pot holds the
      // rest, never negative), so `behind > expected` is true minting and is asserted
      // continuously. `behind == expected` EXACTLY only BETWEEN hands (no pot), which
      // we detect via the sim: it holds a table ONLY while a hand is live
      // (`stopTable` removes it on hand-complete), so a non-null snapshot ⇒ that table
      // has a hand in flight.
      if (!tm.isRunning(tournamentId)) return null;
      const behind = tm.getTotalChips(tournamentId);
      let anyHandInFlight = false;
      for (const tableId of tm.getAllTableIds(tournamentId)) {
        if (sim.getPublicSnapshot(tableId)) {
          anyHandInFlight = true;
          break;
        }
      }
      return { behind, anyHandInFlight };
    },
