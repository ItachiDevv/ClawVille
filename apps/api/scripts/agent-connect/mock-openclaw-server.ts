/**
 * MOCK-OPENCLAW LOCAL RUNTIME — staging-box-local harness (D-openclaw
 * host-it-for-me, shared-inference onboarding 2026-07-08). The exact mirror of
 * mock-hermes-server.ts.
 *
 * Stands in for a real OpenClaw OpenAI-compat runtime on the HARDCODED
 * host-it-for-me gateway target (`OPENCLAW_LOCAL_GATEWAY_URL` = http://localhost:8643,
 * see apps/api/src/services/agent-session-config.ts) so the 'openclaw-local'
 * cognition seam can be proven end-to-end WITHOUT deploying a real OpenClaw runtime:
 *
 ...
 *   2. set OPENCLAW_LOCAL_GATEWAY_ENABLED=true on that API and restart it;
 *   3. connect a GATEWAY-LESS openclaw-identity agent
 *      (scripts/agent-connect/openclaw-local-e2e.ts) — when the sim pulls its body
 *      into an ambient NPC conversation, the API POSTs here and the reply's
 *      deterministic marker proves the seam.
 *
 * The reply carries exactly one `[ACTION: emote(name=wave)]` tag (mirroring
 * mock-hermes-server.ts). For openclaw-local this tag IS parsed/dispatched by the
...
const PORT = Number.parseInt(arg('--port') ?? process.env.PORT ?? '8643', 10);
 ...
 */
const MOCK_MARKER = 'OPENCLAW_MOCK_REPLY_V1';
...
const server = Bun.serve({
  ...
  async fetch(req) {
    ...
    // Health probe (curl http://localhost:8643/health).
    if (req.method === 'GET' && url.pathname === '/health') {
      return Response.json({ ok: true, marker: MOCK_MARKER, requests: requestCount });
    }

    if (req.method !== 'POST' || url.pathname !== '/v1/chat/completions') {
      console.log(`[openclaw-mock] ${ts} ${req.method} ${url.pathname} → 404 (unhandled)`);
    ...
    console.log(
      `[openclaw-mock] ${ts} chat #${requestCount} → 200 model=${model} lastUser="${lastUser.slice(0, 80)}"`,
    );

    // Fixed OpenAI-shaped completion: deterministic marker + one ACTION tag.
    return Response.json({
      id: `openclaw-mock-${requestCount}`,
      ...
      choices: [
        {
          ...
          message: {
            role: 'assistant',
            content: `Hello from the mock local OpenClaw runtime (${MOCK_MARKER}). [ACTION: emote(name=wave)]`,
          },
          finish_reason: 'stop',
        },
      ],
...
console.log('=== mock-OpenClaw local runtime (staging-box-local harness) ===');
...
console.log('');
console.log('REMINDER: the API only calls this when OPENCLAW_LOCAL_GATEWAY_ENABLED=true');
console.log('AND this runs on the SAME box (target is the hardcoded localhost:8643),');
console.log('AND the connecting openclaw agent has NO gatewayUrl of its own.');
