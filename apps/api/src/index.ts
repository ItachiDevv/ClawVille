...
(async () => {
  ...
  try {
    ...
    // Chunk #10 — return reserved bot petIds to the pool when ANY room
    ...
    // end delivery to the WS hub's `sendToPet`. Done via callback (not
    ...
    // chain into every reward-pipeline test that mocks `@clawville/database`.
    {
      ...
      setMatchEndDeliveryFn((roomId, petId, frame) => {
        activityWsHub.sendToPet(roomId, petId, frame);
      });
    }
    ...
    bumperShellsSim.setIntegrityForfeitFn((roomId, petId) => {
    ...
    // Chunk #10 — hydrate the bot petId pool BEFORE the matcher starts
