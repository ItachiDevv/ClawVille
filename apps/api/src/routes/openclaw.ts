...
import { z } from 'zod';
import { NPC_IDS, BUILDING_OPENCLAW_THEMES } from '@elizapets/shared';
...
openclawRoutes.post('/location-chat', sessionMiddleware, async (c) => {
  ...
  if (petContext?.name) {
    systemParts.push(`The visitor is ${petContext.name}, a ${petContext.species ?? 'pet'}.`);
  }
  if (petContext?.neoTokens !== undefined) {
    systemParts.push(`They have ${petContext.neoTokens} NeoTokens.`);
  }
  if (petContext?.knowledge && petContext.knowledge.length > 0) {
...
openclawRoutes.get('/knowledge-export/:petId', async (c) => {
  ...
  const config = pet.characterConfig;
