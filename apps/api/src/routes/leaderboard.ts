...
// ---------------------------------------------------------------------------
// Composite score weights — tuned so that a high-activity pet across every
...
async function buildSnapshot(cap: number): Promise<LeaderboardSnapshot> {
  ...
  const authoredRows = await db
    ...
    .where(sql`${publishedSkills.authorPetId} is not null`)
    .groupBy(publishedSkills.authorPetId);

  const authoredByPet = new Map<string, number>(
    authoredRows
      .filter((r): r is { petId: string; total: number } => r.petId !== null)
...
interface AgentLeaderboardEntry {
   ...
   * `pet` = pet-only contribution (Player tier). Frontend filter on this
 ...
 *
 *   2. Pet-keyed UNION — Players (pet + no agent) rank alongside Trainers
 *      (pet + agent). Same scoring rubric, separate `subjectType` tag for
 *      the Phase 2 filter chips. Disjoint event sets so no double-counting:
 *      agent path takes events with agent_id IS NOT NULL; pet path takes
 *      events with agent_id IS NULL AND pet_id IS NOT NULL.
 *
 ...
 * pet_id partial) feeding hash aggregates, then a UNION. Index coverage:
 * `idx_events_type_ts`, `idx_events_agent_ts`, `idx_events_pet_ts`.
 *
 * Joining pet / openclaw_bots / wallets happens in memory via batched
...
async function buildAgentSnapshot(
  ...
  const aggRows = await db.execute<{
    ...
    pet_daily AS (
      ...
      WHERE agent_id IS NULL
        AND pet_id IS NOT NULL
        AND ts > now() - ${sql.raw(`interval '${interval}'`)}
      GROUP BY pet_id, date_trunc('day', ts)
    ),
    ...
    pet_scores AS (
      SELECT
        pd.pet_id::text AS subject_id,
        'pet'::text AS subject_type,
      ...
      FROM pet_daily pd
      LEFT JOIN pet_sessions s ON s.pet_id = pd.pet_id
      GROUP BY pd.pet_id
    )
    SELECT * FROM (
  ...
  // Agent rows → openclaw_bots → pets-for-user. Pet rows → pets directly.
  const agentRows = aggRows.filter((r) => r.subject_type === 'agent');
  const petRows = aggRows.filter((r) => r.subject_type === 'pet');

  // Agent path: openclaw_bots → optional bound pet via userId.
  ...
  // Pet path (Q3 plan §2.5 — Player tier groundwork): subject_id IS the pet.id
  // since pet rows come from events with no agent_id. One direct pets lookup.
  ...
  if (petSubjectIds.length > 0) {
    const directPets = await db
      ...
      .from(pets)
      .where(and(inArray(pets.id, petSubjectIds), eq(pets.isActive, true)));
  ...
  const entries: AgentLeaderboardEntry[] = aggRows.slice(0, limit).map((r, idx) => {
    ...
    const pet = boundPet ?? directPet;

    return {
      rank: idx + 1,
      // For agent rows: the openclaw bot's text agent_id. For pet rows: a
      // synthetic `pet:<uuid>` so client-side keys stay unique across the
      // unified board without colliding with real bot agent_ids.
      agentId: isAgent ? r.subject_id : `pet:${r.subject_id}`,
      petId: pet?.id ?? (isAgent ? null : r.subject_id),
 ...
 *   offset  — 0.., default 0
 *   me      — truthy to also include the current user's pet row, even if
...
leaderboardRoutes.get('/', sessionMiddleware, async (c) => {
  ...
  if (wantMe) {
    ...
    if (user) {
      ...
      if (myPet) {
        mePet = sorted.find((e) => e.petId === myPet.id) ?? null;
      }
    }
  }
