 ...
 *
 *   POST /api/cosmetics/:skuId/buy         — debit ClawTokens, insert pet_skins.
...
// ---------------------------------------------------------------------------
//
// Spends ClawTokens to acquire the SKU and inserts a pet_skins row.
// Idempotent: re-buying an already-owned SKU returns 200 with
// `{ alreadyOwned: true }` and does NOT debit again.
//
// Atomicity: token debit + pet_skins insert run inside a single
...
cosmeticsRoutes.post('/:skuId/buy', sessionMiddleware, requireAuth, async (c) => {
  ...
  // Atomic debit + insert.
  let result: { balanceAfter: number; petSkinId: string };
  try {
    result = await db.transaction(async (tx) => {
      ...
      return { balanceAfter: debit.balanceAfter, petSkinId: row.id };
    });
  } catch (err) {
    if (err instanceof InsufficientTokensError) {
      throw new HTTPException(400, {
        message: `Not enough ClawTokens. Need ${sku.priceCt}, have ${pet.clawTokens}.`,
      });
    }
    throw err;
  }
