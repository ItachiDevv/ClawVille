...
async function recalculateSuccessRate(petId: string, tx?: BountyTx): Promise<number> {
  const qb = tx ?? db;
  const attemptCounts = await qb
    ...
    .where(
      and(
        eq(bountyAttempts.hunterId, petId),
...
bountyRoutes.delete('/:id', requireAuth, async (c) => {
  ...
  const { balanceAfter: refundedBalance } = await creditClawTokens({
    petId: pet.id,
    amount: bounty.tokenReward,
