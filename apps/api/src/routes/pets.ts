...
const createPetSchema = z.object({
   ...
   * (optional). Clamped + persisted on `pets.learning_focus`; injected into
...
petRoutes.post('/', async (c) => {
  ...
  if (isAutoProvision) {
    ...
    // pet's fresh wallet secret is disclosed to the user exactly once.
    // Per Phase 5.1: secret shown once per wallet; each pet gets its own
    ...
    // Cloudflare Worker connectivity doesn't block pet creation for
    // existing users).
    try {
      const w = await ensureWalletWithFirstTimeSecret('pet', pet.id);
      pet.walletAddress = w.publicKey;
...
// to the current harness's pool so a user can't promote a self-hosted pet
...
petRoutes.patch('/me/appearance', requireAuth, async (c) => {
  ...
  // Find current pet — need its harness to validate the modelKey swap.
  const current = await db.query.pets.findFirst({
    where: and(eq(pets.userId, user.id), eq(pets.isActive, true)),
  });
  if (!current) {
    throw new HTTPException(404, { message: 'Pet not found' });
  ...
  // Harness-pool guard — a Milady-harness pet can only swap between
  // Milady VRM avatars; a non-Milady pet can only pick non-Milady
