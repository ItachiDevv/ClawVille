...
import { db, users, openclawBots, pets } from '@clawville/database';
...
//
// Resolution order:
//   1. Milady-harnessed pets → always considered connected because
//      ClawVille hosts their Eliza runtime end-to-end. Session handle
//      surfaces as `pet.platformAgentId`; there's no openclaw_bots row
//      to check because the agent IS the pet.
...
authRoutes.get('/me/agent-session', requireAuth, async (c) => {
  ...
  if (pet?.harness === 'milady' && pet.platformAgentId) {
    return c.json({
      connected: true,
      agentId: pet.platformAgentId,
      harness: 'milady',
      // Milady pets have no TTL — ClawVille hosts the runtime, so the
      // session is alive as long as the pet row exists.
      expiresAt: null,
      lastSeenAt: null,
    });
  }
...
// ---------------------------------------------------------------------------
// POST /api/auth/guest — guest pet auto-create (2026-04-23).
//
// Lets an un-authenticated visitor play the Q2 activity games + chat with
// NPCs as a throwaway "Guest Pet" — no email, no signup. Use case:
// "test-drive the game before deciding to make an account."
//
// Behaviour:
//   - Idempotent: if the caller already has a Lucia cookie, return their
//     existing user + pet (handler does NOT create a second guest).
//   - Else: create a (users, pets) pair with is_guest=true,
...
//   - Guest pets do NOT appear on the agent leaderboard
//   - Guest pets do NOT appear on per-activity leaderboards
...
// branch in pets.ts. Each mint creates a (users, pets) pair so the cap
...
/**
 * Try INSERTing the pet up to N times — on a 23505 unique-violation
    ...
    try {
      const [row] = await db
        ...
        .values({
          ...
          // No characterConfig — guests don't run Eliza chat as their pet.
          // characterConfig is hydrated later if/when they convert to a
          // real account. Leaving it null is safe — the pet routes
...
authRoutes.post('/guest', async (c) => {
  ...
  // current user + pet rather than minting a second guest.
  const existingUser = c.get('user');
  if (existingUser) {
    const existingPet = await db.query.pets.findFirst({
      where: and(eq(pets.userId, existingUser.id), eq(pets.isActive, true)),
    });
    return c.json({
      user: {
        id: existingUser.id,
  ...
  // (same pattern as scripts/seed-bot-pets.ts). The placeholder
  ...
  const pet = await insertGuestPet(userId, parsed.data.requestedName);
