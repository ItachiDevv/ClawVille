...
// Species enum still matches the DB pet_species pg_enum (8 land species from a
...
// turtle→lobster_plush). Follow-up: migrate pet_species enum to ocean keys.
...
agentSetupRoutes.post('/create', requireAuth, async (c) => {
  ...
  // and doesn't collect those selections. New pets created via the
  // primary /create-agent flow (POST /api/pets) go through the Phase 2
  // wiring in pets.ts:165-179.
  const [pet] = await db
    .insert(pets)
    .values({
      userId: user.id,
...
agentSetupRoutes.post('/:id/export', requireAuth, async (c) => {
  ...
  const configData: AgentConfigExport = {
    ...
    agentCategory: pet.agentCategory,
    harness: pet.harness,
  };
