...
bazaarRoutes.get('/', async (c) => {
  ...
  const rows = await db
    ...
    .from(bazaarListings)
    .innerJoin(publishedSkills, eq(bazaarListings.skillId, publishedSkills.id))
    .innerJoin(pets, eq(bazaarListings.sellerId, pets.id))
    .where(whereClause)
    .orderBy(orderBy)
    .limit(pageSize)
    .offset(offset);
...
bazaarRoutes.get('/my-listings', requireAuth, async (c) => {
  ...
  const rows = await db
    ...
    .from(bazaarListings)
    .innerJoin(publishedSkills, eq(bazaarListings.skillId, publishedSkills.id))
    .where(eq(bazaarListings.sellerId, pet.id))
    .orderBy(desc(bazaarListings.createdAt));
...
bazaarRoutes.get('/my-purchases', requireAuth, async (c) => {
  ...
  const rows = await db
    ...
    .innerJoin(pets, eq(bazaarTransactions.sellerId, pets.id))
    .where(eq(bazaarTransactions.buyerId, pet.id))
...
bazaarRoutes.get('/skills/:skillId/reviews', async (c) => {
  ...
  const rows = await db
    .select({
      ...
      reviewerPetName: pets.name,
      reviewerSpecies: pets.species,
    })
    .from(bazaarReviews)
    .innerJoin(pets, eq(bazaarReviews.reviewerId, pets.id))
...
bazaarRoutes.post('/list', requireAuth, async (c) => {
  ...
  const existingListing = await db.query.bazaarListings.findFirst({
    where: and(
      ...
      eq(bazaarListings.sellerId, pet.id),
      eq(bazaarListings.status, 'active')
    ),
  });
...
bazaarRoutes.post('/:id/buy', requireAuth, async (c) => {
  ...
  // 2. Find buyer's pet (check clawTokens >= price)
...
bazaarRoutes.post('/:id/review', requireAuth, async (c) => {
  ...
  const [transaction] = await db
    ...
    .where(
      and(
        ...
        eq(bazaarTransactions.buyerId, pet.id)
      )
    )
    .limit(1);
  ...
  const existingReview = await db.query.bazaarReviews.findFirst({
    where: and(
      ...
      eq(bazaarReviews.reviewerId, pet.id)
    ),
  });
