...
async function validateTutorialQuestEngagement(
  ...
  switch (questId) {
    ...
    case 'top-100': {
      // Compute the pet's current rank in the agents leaderboard (24h
      ...
      const rankRows = await db.execute<{ rank: number }>(sql`
        ...
        SELECT rank::int FROM ranked WHERE subject_id = ${petId}
      `);
    ...
    case 'building-champion': {
      // Pet is the top-visited subject for any single building (24h).
      const rows = await db.execute<{ matched: number }>(sql`
        ...
        WHERE rk = 1 AND subject_id = ${petId}
      `);
