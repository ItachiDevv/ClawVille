...
activitiesV2Routes.post('/:id/queue', requireAuthOrAgentSession, async (c) => {
  ...
  // Solo enqueue.
  const existingEntry = activityQueueService.getQueuedEntry(identity.petId);
  if (existingEntry) {
    if (
    ...
    throw new HTTPException(409, {
      message: `Pet is already queued for ${existingEntry.activityId}`,
    });
  }

  const activeRoom = activityRoomManager.getPlayerActiveRoom(identity.petId);
  if (activeRoom) {
    ...
    throw new HTTPException(409, {
      message: `Pet is already in an active ${activeRoom.activityId} room`,
    });
  }
