...
agentGatewayRoutes.post('/connect', async (c) => {
  ...
  //
  // Phase 5.1: the resolver ALSO returns the resolved `{userId, petId}`
  // so we can hang identity-keypair generation + pet-wallet first-time
  ...
  // Event payload — enrich with userId/petId when we resolved them from a
  // connection token. Dashboard funnels join events by userId/petId when
  ...
  void logEventFromContext(c, {
    ...
    petId: pendingForEvent?.petId ?? resolved.petId ?? null,
    agentId: resolvedAgentId,
    sessionId,
    payload: {
      identityType,
...
agentGatewayRoutes.post('/reconnect', async (c) => {
  ...
  return c.json({
    ...
    // `walletAddress` here is the PET wallet (the human-facing economic
 ...
 *
 * Phase 5.1: also returns the resolved `{userId, petId}` so `/connect`
 * can call `generateIdentityKeypairForUser` and
 * `ensureWalletWithFirstTimeSecret` off the same resolution (no double
 * lookup). Returns `{ticket: null, userId: null, petId: null}` only
...
async function mintSessionTicketFromConnect(args: {
  ...
  try {
    ...
    if (!userId) {
      ...
      if (!ident) return { ticket: null, userId: null, petId: null, petName: null };
    ...
    return { ticket, userId, petId, petName };
  } catch (err) {
    console.error('[AgentConnect] ticket mint failed (non-fatal):', err);
    return { ticket: null, userId: null, petId: null, petName: null };
  }
}
...
agentGatewayRoutes.post('/join', async (c) => {
  ...
  // 3. Ensure the pet has a wallet. Phase 5.1 gap-fill: /join previously
  //    created pets without provisioning a wallet row, so returning
  ...
  //    brand-new pets get the plaintext secret returned exactly once —
  ...
  // 4. Mint the magic-link ticket bound to this user+pet.
  ...
  return c.json({
    ...
    // pet wallet was just created; subsequent /join calls omit this.
...
agentGatewayRoutes.post('/:sessionId/visit-building', async (c) => {
  ...
  // quest validator's `(user_id = X OR pet_id = Y)` check (audit-fix
  // 2026-04-29).
