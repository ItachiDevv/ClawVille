...
auctionRoutes.post('/:id/bid', requireAuth, async (c) => {
  ...
  await db
    .update(avatars)
    .set({
      clawTokens: bidderPet.clawTokens - amount,
      updatedAt: new Date(),
    })
  ...
  if (auction.currentBidderId && auction.currentBid) {
    const previousBidder = await db.query.avatars.findFirst({
      where: eq(avatars.id, auction.currentBidderId),
    });
    ...
    if (previousBidder) {
      await db
        .update(avatars)
        .set({
          clawTokens: previousBidder.clawTokens + auction.currentBid,
          updatedAt: new Date(),
        })
        .where(eq(avatars.id, previousBidder.id));
    }
  }
