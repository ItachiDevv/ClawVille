...
function ambiguousOutcome(): ExecutePreparedExactPaymentOutcome {
  return {
    ...
    verifyPassed: true,
    payAi: { attempted: true, providerFailure: false },
    reason: 'post_submit_timeout',
    payer: SENDER_WALLET,
    signature: null,
    result: {
      settled: false, isValid: true, txSignature: null, network: null,
      payer: SENDER_WALLET, failureReason: 'post_submit_timeout', raw: {},
    },
  };
}
...
describe('agent-pay durable x402 machine', () => {
  ...
  it('keeps an unknown signature-less settle rejection countable', async () => {
    ...
    expect([...h.rows.values()][0]).toMatchObject({
      status: 'reconcile',
      failureReason: 'post_submit_timeout',
      capExempt: null,
    });
    expect(h.mintCalls()).toBe(0);
