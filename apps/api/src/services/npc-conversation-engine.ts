...
import { NPC_DEFINITIONS, type NpcDefinition } from '@legacyapp/shared';
...
export async function generateNpcConversation(
  ...
  const arenaContext = arenaMode
    ? '\nYou are in the LegacyApp Arena where NPCs battle each other. You sometimes fight other NPCs. Reference combat, loot, rivalries, and battles naturally in conversation.'
    : '';
...
export async function generateOpenClawConversation(
  ...
  const arenaContext = arenaMode
    ? ' You are in the LegacyApp Arena where NPCs battle each other.'
    : '';
