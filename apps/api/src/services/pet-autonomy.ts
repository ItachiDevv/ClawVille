...
import { NPC_BUILDING_CENTERS, BUILDING_ACTIVITIES, ACTIVITY_EMOJIS, type NpcActivity } from '@elizapets/shared';
    ...
    if (pet.activity === 'walking' && pet.path.length > 0 && pet.pathIndex >= pet.path.length) {
      if (pet.destinationBuildingId) {
        ...
        if (pet.tokensEarned < MAX_TOKENS_PER_SESSION) {
          ...
          this.logActivity(pet.petId, 'visit', `Visited ${pet.destinationBuildingId} and earned 1 NeoToken`, 1).catch(console.error);
        }
