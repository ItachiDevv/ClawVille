...
export function generateElizaOsSkill(opts: SkillGenOpts): { markdown: string; characterJson: string; installPath: string; publishCommand: string } {
  const slug = (opts.customName ?? opts.petName).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  const skillName = opts.customName ?? opts.petName;
  const description = opts.customDescription ?? `OpenClaw knowledge from ${opts.petName}`;
...
export function generateOpenClawSkill(opts: SkillGenOpts): { markdown: string; installPath: string; publishCommand: string } {
  ...
  const description = opts.customDescription ?? `OpenClaw knowledge from ${opts.petName} via ClawVille`;
  const knowledgeEntries = opts.selectedKnowledge ?? opts.knowledge;
  const exportDate = new Date().toISOString().split('T')[0];
  ...
  const publishCommand = `clawhub publish --slug ${skillName} --name "${opts.customName ?? opts.petName}" --version 1.0.0 --tags openclaw,agents,ai`;
