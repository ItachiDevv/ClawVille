...
export class SpecialEventManager {
   ...
   * SOL tx is rejected via the unique (event, avatar) index AND a tx-sig-already-used
   * check (a second avatar replaying the same sig is rejected).
   */
  async signup(
    slug: string,
    subject: SignupSubject,
    ...
    return this.db.transaction(async (tx) => {
      ...
      // signup only (otherwise one payment buys many seats). Reject a sig already
      // recorded on any confirmed signup for this event.
