...
import type { ResearchThoughtEvent, ResearchPhase } from '@elizapets/shared';
...
function makeEvent(
  base: { sessionId: string; petId: string; locationId: string },
  phase: ResearchPhase,
  message: string,
...
export class ResearchService {
  ...
  async research(opts: {
    ...
    const { sessionId, petId, locationId, petName, existingKnowledge, emit, themeOverride } = opts;
    const base = { sessionId, petId, locationId };
    ...
    try {
      ...
      const skillResult = generateSkillMd({
        ...
        customName: `${petName}-${themeName.toLowerCase().replace(/\s+/g, '-')}`,
        customDescription: `${themeName} knowledge researched by ${petName}`,
