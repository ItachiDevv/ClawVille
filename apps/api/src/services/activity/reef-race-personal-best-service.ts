 ...
 * per-pet `Promise.all` is bounded by 8 (the pod's per-room participant
 * cap). Total wall-clock <50 ms even when every pet improves.
 *
 ...
 * Anti-cheat-flagged matches are also skipped (any pet with `flagCount > 0`
...
/**
 * Invalidate the in-memory PB ghost cache for a single pet. Called from
 ...
 *   INSERT ... ON CONFLICT (pet_id, activity_id) DO UPDATE
...
export async function maybeUpdatePersonalBest(
  ...
  const priorRows = await db
    ...
    .where(
      sql`${reefRacePersonalBests.petId} = ${input.petId}::uuid AND ${reefRacePersonalBests.activityId} = ${input.activityId}`,
    )
    .limit(1);
  ...
  if (previousMs === null || input.newBestLapMs < previousMs) {
    ...
    const upsertRows = await db.execute<{ best_lap_ms: number }>(
      sql`
        INSERT INTO reef_race_personal_bests
          (pet_id, activity_id, best_lap_ms, ghost_replay_data, source_room_id)
        VALUES
          (${input.petId}::uuid,
           ${input.activityId},
           ${input.newBestLapMs},
        ...
        ON CONFLICT (pet_id, activity_id) DO UPDATE
          SET best_lap_ms = EXCLUDED.best_lap_ms,
              ghost_replay_data = EXCLUDED.ghost_replay_data,
    ...
    // S4 fix — flush PB ghost cache for this pet so a reconnect within
    ...
    invalidatePbGhostCache(input.petId);
    try {
...
/**
 * Read the latest PB row for a pet. Hot path on snapshot.init for self pet.
 * Returns null when the pet has no PB yet (the common case for fresh pets).
 */
export async function loadPersonalBest(
  ...
  const rows = await db
    ...
    .from(reefRacePersonalBests)
    .where(
      sql`${reefRacePersonalBests.petId} = ${petId}::uuid AND ${reefRacePersonalBests.activityId} = 'reef-race'`,
    )
    .limit(1);
  return rows[0] ?? null;
}
...
/**
 * Read just the captured ghost frames for a pet. 5-min TTL in-memory cache
 * keyed by petId; invalidated by `maybeUpdatePersonalBest` on successful
 * upsert (S4 fix).
 *
 * Returns `undefined` on any error or when no PB exists. Logged via
 * console.warn (NOT alertError — missing PB is common for fresh pets).
 */
export async function loadPersonalBestGhostFrames(
  ...
  const now = Date.now();
  pruneExpired(now);
  const cached = ghostCache.get(petId);
  if (cached && cached.expiresAt > now) {
    return cached.frames;
  }
  let frames: GhostFrame[] | undefined;
  try {
    const row = await loadPersonalBest(petId);
    if (row) {
    ...
    console.warn(
      `[reef-race-personal-best] loadPersonalBestGhostFrames failed for pet ${petId}:`,
      err,
    );
    // Cache the miss briefly so a transient DB error doesn't pile up
    // retries on every WS reconnect.
    ghostCache.set(petId, { frames: undefined, expiresAt: now + 30_000 });
    return undefined;
  }
  ghostCache.set(petId, { frames, expiresAt: now + GHOST_CACHE_TTL_MS });
  return frames;
}
