...
class ActivityQueueService {
   ...
   * strand the lobby with "Pet is already in a queue" when the existing entry
   * is exactly the same activity bucket.
   */
  getQueuedEntry(petId: string): QueueEntry | null {
    for (const queue of this.queues.values()) {
      const entry = queue.find((e) => e.petId === petId);
  ...
  private async matchActivity(activityId: string, queue: QueueEntry[], agentOnly: boolean): Promise<void> {
    ...
    // the seeded pool (`scripts/seed-bot-pets.ts`). Reservation is
    ...
    if (count < minFill) {
      ...
      if (allowBots && selected.length > 0 && allConsentToBots) {
        ...
        botPetIds = botPool.reserve('pending-room', need);
        if (botPetIds.length < need) {
          ...
          if (botPetIds.length > 0) botPool.releaseRoom('pending-room');
          botPetIds = [];
          return;
        }
      } else {
    ...
    try {
      ...
      for (const botPetId of botPetIds) {
        participants.push({
          petId: botPetId,
          userId: null,
          agentId: null,
      ...
      if (botPetIds.length > 0) {
        try {
          botPool.rebindReservation(botPetIds, 'pending-room', room.id);
        } catch (err) {
