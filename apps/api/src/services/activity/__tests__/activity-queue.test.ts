...
mock.module('@clawville/database', () => ({
  ...
  activityParties: { id: 'id', leaderPetId: 'leader_pet_id' },
  ...
  activityRoomParticipants: { roomId: 'room_id', petId: 'pet_id' },
  activityReplays: { id: 'id' },
  // Chunk #10 — bot-pool reads from `users` and `pets` for hydration. Tests
  // pre-load the pool via `__resetForTest`, so these only need to be present
  // for module-level imports to resolve.
  pets: { id: 'id', userId: 'user_id', name: 'name', flags: 'flags' },
...
async function enqueueHuman(petId: string, partyId: string | null = null) {
...
describe('enqueue / leave idempotency', () => {
  it('rejects duplicate enqueue for the same pet', async () => {
    await enqueueHuman(pid(1));
    await expect(enqueueHuman(pid(1))).rejects.toThrow(/already in a queue/);
  });

  it('leaveQueue is idempotent — returns false for never-queued pet', async () => {
...
describe('Party fill atomicity', () => {
  ...
  it('joinParty is idempotent — same pet twice is a no-op', async () => {
    const party = await activityQueueService.createParty(pid(0));
    await activityQueueService.joinParty(party.shortCode, pid(1));
  ...
  it('leaveParty promotes a new leader when leader leaves', async () => {
    ...
    expect(after?.leaderPetId).toBe(pid(1));
  });
});
