...
// Database mock — returns deterministic level rows for the `pets` SELECT
...
const dbMock = {
  ...
  // Outside-tx selects: pets.level fetch for the winrate emitter returns
...
// the per-file pet-profile-loader test (which runs in the same Bun process)
...
function buildReefRoom(participantSpecs: Array<{
  ...
  for (const spec of participantSpecs) {
    participants.set(spec.petId, {
      petId: spec.petId,
      userId: spec.subjectType === 'human' ? `user-${spec.petId}` : null,
      agentId: null,
...
describe('reef_race.bot_winrate.by_level_bucket emission (P3-D1)', () => {
  it('fires exactly once per reef-race room with non-empty issued', async () => {
    mockedLevelRows = [{ id: 'pet-h1', level: 1 }];
    const room = buildReefRoom([
      { petId: 'pet-h1', subjectType: 'human' },
      { petId: 'pet-bot-1', subjectType: 'bot' },
    ]);
    await issueRewardsForRoom({
      room,
      simResults: [
        { petId: 'pet-h1', placement: 1, score: 1, scoreMs: 90_000 },
        { petId: 'pet-bot-1', placement: 2, score: 0, scoreMs: 95_000 },
      ],
    });
  ...
  it('buckets level 1 → "1-10"', async () => {
    mockedLevelRows = [{ id: 'pet-h1', level: 5 }];
    const room = buildReefRoom([
      { petId: 'pet-h1', subjectType: 'human' },
      { petId: 'pet-bot-1', subjectType: 'bot' },
    ]);
    await issueRewardsForRoom({
      room,
      simResults: [
        { petId: 'pet-h1', placement: 1, score: 1 },
        { petId: 'pet-bot-1', placement: 2, score: 0 },
      ],
    });
    await waitForEventsSettled();
  ...
  it('buckets level 15 → "11-25"', async () => {
    mockedLevelRows = [{ id: 'pet-h1', level: 15 }];
    const room = buildReefRoom([
      { petId: 'pet-h1', subjectType: 'human' },
      { petId: 'pet-bot-1', subjectType: 'bot' },
    ]);
  ...
  it('buckets level 35 → "26-49"', async () => {
    mockedLevelRows = [{ id: 'pet-h1', level: 35 }];
    const room = buildReefRoom([
      { petId: 'pet-h1', subjectType: 'human' },
      { petId: 'pet-bot-1', subjectType: 'bot' },
    ]);
  ...
  it('buckets level 50 → "50"', async () => {
    mockedLevelRows = [{ id: 'pet-h1', level: 50 }];
    const room = buildReefRoom([
      { petId: 'pet-h1', subjectType: 'human' },
      { petId: 'pet-bot-1', subjectType: 'bot' },
    ]);
  ...
  it('uses the HIGHEST level when multiple humans race', async () => {
    ...
    const room = buildReefRoom([
      { petId: 'pet-h1', subjectType: 'human' },
      { petId: 'pet-h2', subjectType: 'human' },
      { petId: 'pet-h3', subjectType: 'human' },
      { petId: 'pet-bot-1', subjectType: 'bot' },
    ]);
    await issueRewardsForRoom({
      room,
      simResults: [
        { petId: 'pet-h1', placement: 2, score: 1 },
        { petId: 'pet-h2', placement: 1, score: 2 },
        { petId: 'pet-h3', placement: 3, score: 0 },
        { petId: 'pet-bot-1', placement: 4, score: 0 },
      ],
    });
    await waitForEventsSettled();
    const evt = findWinrateEvent();
  ...
  it('humanFinishedFirst=false + botFinishedAhead=1 when bot wins', async () => {
    ...
    await issueRewardsForRoom({
      room,
      simResults: [
        { petId: 'pet-bot-1', placement: 1, score: 2 },
        { petId: 'pet-h1', placement: 2, score: 1 },
      ],
    });
    await waitForEventsSettled();
  ...
  it('does NOT emit for bot-only rooms (no humans)', async () => {
    ...
    const room = buildReefRoom([
      { petId: 'pet-bot-1', subjectType: 'bot' },
      { petId: 'pet-bot-2', subjectType: 'bot' },
    ]);
    await issueRewardsForRoom({
      room,
      simResults: [
        { petId: 'pet-bot-1', placement: 1, score: 2 },
        { petId: 'pet-bot-2', placement: 2, score: 1 },
      ],
    });
  ...
  it('does NOT emit for non-reef-race activities (e.g. bumper-shells)', async () => {
    ...
    await issueRewardsForRoom({
      room,
      simResults: [
        { petId: 'pet-h1', placement: 1, score: 4 },
        { petId: 'pet-bot-1', placement: 2, score: 2 },
      ],
    });
    await new Promise((r) => setTimeout(r, 10));
    expect(findWinrateEvent()).toBeUndefined();
  ...
  it('payload carries roomId, humanFinished, botCount', async () => {
    mockedLevelRows = [{ id: 'pet-h1', level: 30 }];
    const room = buildReefRoom([
      { petId: 'pet-h1', subjectType: 'human' },
      { petId: 'pet-bot-1', subjectType: 'bot' },
      { petId: 'pet-bot-2', subjectType: 'bot' },
    ...
    await issueRewardsForRoom({
      room,
      simResults: [
        { petId: 'pet-h1', placement: 1, score: 3 },
        { petId: 'pet-bot-1', placement: 2, score: 2 },
        { petId: 'pet-bot-2', placement: 3, score: 1 },
      ],
    });
    await waitForEventsSettled();
    const evt = findWinrateEvent();
