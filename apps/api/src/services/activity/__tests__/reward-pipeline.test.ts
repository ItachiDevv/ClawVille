...
describe('isFocusAligned', () => {
  it('returns true when pet flags carry matching learningFocus', () => {
...
describe('issueRewardsForRoom', () => {
  ...
  it('credits the human participant + writes both result rows + skips credit for bot', async () => {
    ...
    const issued = await issueRewardsForRoom({
      room,
      simResults: [
        { petId: 'pet-human-1', placement: 1, score: 4 },
    ...
    const humanRow = issued.find((r) => r.petId === 'pet-human-1')!;
    ...
    const botRow = issued.find((r) => r.petId === 'pet-bot-1')!;
    ...
    expect(creditCalls[0].petId).toBe('pet-human-1');
