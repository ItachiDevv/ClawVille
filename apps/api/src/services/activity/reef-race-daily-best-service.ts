 ...
 * `reef_race_personal_bests`, joins to avatars + wallets for display, and
 ...
 *   - Guests: filtered via `avatars.is_guest = false`.
 ...
 * a sort-merge against `avatars` (PK) + LEFT JOIN against `wallets`
...
export async function getDailyBestLapSnapshot(
  ...
  // excluded via the avatars.is_guest = false predicate.
  const rows = await db.execute<{
    ...
    JOIN avatars p ON p.id = pb.avatar_id
