 ...
 *        + focus-aligned bonus (+pct% when pet matches activity's
 *          `skillBuildingMatches[]` — looks at `pets.flags.learningFocus`)
 ...
 *      petIds belong to the system user; crediting them inflates the
...
/**
 * Returns true when the pet's stored learning focus aligns with the
 * activity's `skillBuildingMatches[]`. Storage convention (forward-
 * compatible): `pets.flags.learningFocus` (string buildingId). When the
...
interface PetContext {
  ...
  /** Earliest `created_at` for this pet+activity scoped to today (UTC) */
  todayCount: number;
  /** Best (lowest) score_ms recorded for this pet on this activity prior */
  priorBestMs: number | null;
  /**
   * True when this pet belongs to a guest user (un-authed visitor who
...
export async function issueRewardsForRoom(
  ...
  // Pre-fetch per-pet context so the inner transaction is one round-trip
  // for context + writes. Bots skip the context fetch (no flags lookup
  // needed — their pet rows belong to the system user).
  const petIds = simResults.map((r) => r.petId);
  const participants = room.participants;
  const petContexts = await loadPetContexts(petIds, room.activityId, participants);
  ...
  await db.transaction(async (tx) => {
    for (const sim of simResults) {
      const participant = participants.get(sim.petId);
      if (!participant) {
        // Sim returned a petId not in the room — defensive skip + warn.
        console.warn(
          `[reward-pipeline] sim returned unknown petId ${sim.petId} for room ${room.id}`,
        );
        continue;
      }

      const ctx = petContexts.get(sim.petId) ?? {
        flags: null,
        todayCount: 0,
        priorBestMs: null,
        isGuest: false,
...
/**
 * Batch-load per-pet context for the issuance pass. One pass each for:
 *   - `pets.flags` (used by focus-bonus check)
 ...
 * set small. Bot pets are excluded from the flag fetch (their flags are
 * irrelevant — they don't earn rewards).
 */
async function loadPetContexts(
  petIds: string[],
  activityId: string,
  ...
  const nonBotPetIds = petIds.filter((id) => {
  ...
  if (nonBotPetIds.length > 0) {
    const flagRows = await db
      .select({ id: pets.id, flags: pets.flags, isGuest: pets.isGuest })
      .from(pets)
      .where(inArrayWhitelist(pets.id, nonBotPetIds));
  ...
  for (const row of bestRows) {
    bestByPet.set(row.petId, row.best ?? null);
  }
