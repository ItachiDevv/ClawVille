 ...
 * object. Flag accumulation lives on the sim's per-avatar state (which the
...
/**
 * Per-avatar, per-match flag counter. Owned by the sim; this helper just
...
export class BumperFlagCounter {
  ...
  /** Increment the counter for `avatarId`; returns true if forfeit threshold hit */
  bump(avatarId: string): boolean {
    const next = (this.counts.get(avatarId) ?? 0) + 1;
    this.counts.set(avatarId, next);
    return next >= FLAG_FORFEIT_THRESHOLD;
  }

  countFor(avatarId: string): number {
    return this.counts.get(avatarId) ?? 0;
  }

  reset(avatarId: string): void {
    this.counts.delete(avatarId);
  }
