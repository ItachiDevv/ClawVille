 ...
 * (pet, segment) pair on the first tick where the body's t crosses into
 * that segment, and `getEntryMs` to feed `validateSegmentTime`.
 *
 * Mirrors `ReefCheckpointSkipTracker`'s shape: one tracker per room,
 * Map-backed, idempotent recordings, `forget(petId)` on disconnect.
 *
 * NOTE: idempotent — calling `recordEntry` more than once for the same
 * (pet, segment) keeps the FIRST timestamp (so a body that gets knocked
...
export class ReefProgressTracker {
   ...
   */
  recordEntry(petId: string, segmentIndex: number, ms: number): void {
    let perPet = this.entries.get(petId);
    if (!perPet) {
      perPet = new Map();
      this.entries.set(petId, perPet);
    }
   ...
   */
  getEntryMs(petId: string, segmentIndex: number): number | null {
    const perPet = this.entries.get(petId);
    if (!perPet) return null;
    const ms = perPet.get(segmentIndex);
    return ms === undefined ? null : ms;
  }

  /** Drop tracking for a pet on disconnect / match end. */
  forget(petId: string): void {
    this.entries.delete(petId);
  }
