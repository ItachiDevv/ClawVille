 ...
 * carries per-pet state in a Map but exposes pure-style verdicts.
 *
...
/**
 * Per-pet rolling-window input timestamp tracker. The Map lives on the
...
export class InputRateTracker {
  ...
  validateInputRate(petId: string, now: number = Date.now()): ValidationVerdict<void> {
    let times = this.timestamps.get(petId);
    if (!times) {
      times = [];
      this.timestamps.set(petId, times);
    }
