 ...
 * aggregates per pet, joins pet names. Reef Race rows include
...
export async function buildLeaderboardSnapshot(
  ...
  const conditions = [
    ...
    // Guest pet carve-out (2026-04-23) — un-authed visitors play matches
    ...
    // guest petIds is cheap (small index, partial WHERE clause) and lets
    // us keep the existing GROUP BY shape unchanged.
    sql`NOT EXISTS (
      SELECT 1 FROM ${pets} AS gp
      WHERE gp.id = ${activityResults.petId}
  ...
  const rows = await db
    ...
    .groupBy(activityResults.petId, activityResults.agentId);

  // Join pet names — single batch lookup.
  ...
  const leaderboard: ActivityLeaderboardEntry[] = sliced.map((r, i) => ({
    ...
    petId: r.petId,
    agentId: r.agentId,
    displayName: petNamesById.get(r.petId) ?? r.petId.slice(0, 8),
 ...
 */
export async function getLeaderboardForPet(
  activityId: string,
  window: ActivityLeaderboardWindow,
  petId: string,
  context = 5,
  ...
  const myIndex = full.leaderboard.findIndex((e) => e.petId === petId);
  if (myIndex < 0) {
    return {
      snapshot: full,
      myRank: null,
...
/**
 * Tiny helper for the petName batch fetch. Drizzle's `inArray` is the
 * canonical path; this thin wrapper exists so the call site reads
 * cleanly + we can unit-test without a real `pets` table when the test
 * doesn't care about names.
 */
function petInListWhere(petIds: string[]): ReturnType<typeof sql> {
