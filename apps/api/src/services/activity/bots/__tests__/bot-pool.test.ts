...
beforeEach(() => {
  ...
  botPool.__resetForTest([
    { index: 1, slotId: 'bot-001', petId: 'uuid-1' },
    { index: 2, slotId: 'bot-002', petId: 'uuid-2' },
    { index: 3, slotId: 'bot-003', petId: 'uuid-3' },
    { index: 4, slotId: 'bot-004', petId: 'uuid-4' },
  ]);
});

describe('botPool.reserve', () => {
  it('returns N distinct petIds bound to the room', () => {
  ...
  it('respects per-room uniqueness — the same petId is never double-reserved', () => {
...
describe('botPool.rebindReservation', () => {
  ...
  it('throws when the petId is not currently bound to fromRoomId', () => {
...
describe('botPool.isBot', () => {
  it('returns true for seeded petIds and false for arbitrary ones', () => {
