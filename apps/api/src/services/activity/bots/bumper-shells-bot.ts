 ...
 * petId so a single instance can be safely reused if the matcher ever
...
class BumperShellsBot implements BotController {
  ...
  computeInput(roomState: BotRoomView, _dt: number): BotInput {
    ...
    for (const body of roomState.bodies) {
      if (body.petId === this.petId || !body.alive) continue;
...
export function createBumperShellsBot(petId: string): BotController {
  return new BumperShellsBot(petId);
}
