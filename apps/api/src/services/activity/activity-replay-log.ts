 ...
 *   pid: petId (uuid string) — duplicated per frame for filter-by-pet
...
class ActivityReplayLog {
   ...
   * a retry can succeed.
   *
   * `participants` carries the per-pet display snapshot (color/species/
   * model) so the eventual replay viewer (chunk #5) can render without
   * re-reading the live `pets` row, which may have changed.
   */
  async flushToDb(
    roomId: string,
    activityId: string,
