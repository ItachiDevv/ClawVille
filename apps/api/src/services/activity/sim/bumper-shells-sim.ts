 ...
 *     petIds today but a bot's "input" needs a controller that pushes
...
interface BumperBody {
  ...
  /** Per-pet inventory, slot-indexed (length = MAX_POWER_UP_SLOTS) */
...
interface BumperRoomState {
  ...
  ended: boolean;
  /**
   * Eliminated petIds in elimination order — placement is computed by
   * "alive at end" first, then this list reversed (last-eliminated = 4th,
   * first-eliminated = 8th).
   */
  eliminationOrder: string[];
  /**
   * Bot controllers keyed by petId. Populated from the `bots` arg to
   ...
   * `seq`, so we synthesize it server-side. Keyed by petId.
   */
...
interface BumperSnapshot {
  tick: number;
  bodies: Array<{ petId: string; x: number; y: number; vx: number; vy: number; rot: number; alive: boolean }>;
...
class BumperShellsSim {
  ...
  startRoom(
    ...
    opts?: {
      ...
      /**
       * Bot controllers to install for this room. Their petIds MUST be
    ...
    if (opts?.bots) {
      for (const ctrl of opts.bots) {
        if (!participantPetIds.includes(ctrl.petId)) {
          console.warn(
            `[bumper-shells-sim] bot controller for ${ctrl.petId} is not a participant — skipping`,
          );
          continue;
        }
    ...
    if (state.botControllers.size > 0) {
      ...
      for (const [petId, ctrl] of state.botControllers) {
        ...
        try {
          ...
          console.error(`[bumper-shells-sim] bot onSpawn threw for ${petId}:`, err);
        }
      }
    }
   ...
   */
  forfeit(roomId: string, petId: string, reason: 'integrity' | 'timeout' | 'voluntary'): void {
    const state = this.rooms.get(roomId);
    if (!state) return;
    const body = state.bodies.get(petId);
   ...
   */
  computeResults(roomId: string): Array<{ petId: string; placement: number; score: number; alive: boolean }> {
    const state = this.rooms.get(roomId);
    if (!state) return [];
    ...
    survivors.sort((a, b) => a.petId.localeCompare(b.petId));
    const eliminatedReverse = [...state.eliminationOrder].reverse();
    const out: Array<{ petId: string; placement: number; score: number; alive: boolean }> = [];
    let placement = 1;
    for (const b of survivors) {
      out.push({ petId: b.petId, placement: placement++, score: 0, alive: true });
    }
    for (const petId of eliminatedReverse) {
      out.push({ petId, placement: placement++, score: 0, alive: false });
    }
    return out;
  }
   ...
   * room (selfPetId is overwritten per-bot before pass-through). Cheap
  ...
  private runBotControllers(state: BumperRoomState, dt: number, now: number): void {
    ...
    for (const [petId, ctrl] of state.botControllers) {
      ...
      try {
        ...
        console.error(`[bumper-shells-sim] bot ${petId} computeInput threw:`, err);
        continue;
      }
   ...
   */
  private buildBotRoomView(state: BumperRoomState, selfPetId: string): {
    selfPetId: string;
    bodies: Array<{
      petId: string;
      x: number;
    ...
    for (let i = 0; i < bodies.length; i++) {
      for (let j = i + 1; j < bodies.length; j++) {
        ...
        this.broadcastFn(state.roomId, {
          type: 'event.hit',
          srcPetId: a.vx * a.vx + a.vy * a.vy > b.vx * b.vx + b.vy * b.vy ? a.petId : b.petId,
          dstPetId: a.vx * a.vx + a.vy * a.vy > b.vx * b.vx + b.vy * b.vy ? b.petId : a.petId,
  ...
  private resolveStickyBombs(state: BumperRoomState, now: number): void {
    state.bombs = state.bombs.filter((bomb) => {
      ...
      for (const body of state.bodies.values()) {
        if (!body.alive || body.petId === bomb.ownerPetId) continue;
  ...
  private applyKnockbackAura(state: BumperRoomState, src: BumperBody, now: number): void {
    ...
    for (const target of state.bodies.values()) {
      if (target.petId === src.petId || !target.alive) continue;
  ...
  private applyTractorBeam(state: BumperRoomState, src: BumperBody): void {
    ...
    for (const t of state.bodies.values()) {
      if (t.petId === src.petId || !t.alive || t.activeEffects.has('bs-ghost')) continue;
  ...
  private eliminate(
    ...
    state.eliminationOrder.push(body.petId);
    this.broadcastFn(state.roomId, {
  ...
  private endRound(state: BumperRoomState, _reason: 'last_standing' | 'time_expired'): void {
    ...
    const winners = results.map((r) => ({ petId: r.petId, placement: r.placement }));
  ...
  private integrityForfeitFn: ((roomId: string, petId: string) => void) | null = null;
