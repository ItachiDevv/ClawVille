 ...
 *
 * Boots a single room with 1 human pet + 7 bots, ticks the sim for ~90s of
 ...
 *   - The human pet's body moves at least 1000 wu in XZ from spawn
 *
...
describe('ReefRaceSplineSim — full-room integration smoke test', () => {
  ...
  it('1 human + 7 bots → at least one finishes, no progress regression, match_ended fires', () => {
    const events: Array<{ type: string; petId?: string; reason?: string }> = [];
    reefRaceSplineSim.setBroadcastFn((_id, frame) =>
      events.push(frame as { type: string; petId?: string; reason?: string }),
    );
    ...
    // Build bot controllers for the 7 bot pets.
    const bots = BOT_PETS.map((petId) => createReefRaceBot(petId));

    // Start the room. `isBot` returns true for the bot pets so the sim
    // tags them correctly (mirrors how the room manager calls in prod).
    const isBot = (petId: string) =>
      (BOT_PETS as ReadonlyArray<string>).includes(petId);
    ...
    // Track per-pet progress regression — the spline sim's anti-cheat
    ...
    for (const pet of ALL_PETS) {
      maxRegressionByPet.set(pet, 0);
      lastProgressByPet.set(pet, 0);
    }
    ...
    for (let i = 0; i < TOTAL_TICKS; i++) {
      ...
      for (const pet of ALL_PETS) {
        const body = state.bodies.get(pet);
        if (!body) continue;
        const prev = lastProgressByPet.get(pet)!;
        const curr = body.progress;
        const drop = prev - curr;
        if (drop > maxRegressionByPet.get(pet)!) {
          maxRegressionByPet.set(pet, drop);
        }
        lastProgressByPet.set(pet, curr);
      }
    ...
    // Diagnostic: per-pet final progress + finish count, useful when
    ...
    expect(crossedEvents[0].petId).toBeDefined();
    // The first crossed_finish event's petId should match the first body in
    // state.finishOrder (the sim is single-source-of-truth on finish order).
    expect(crossedEvents[0].petId).toBe(state.finishOrder[0]);
    ...
    for (const pet of ALL_PETS) {
      const maxDrop = maxRegressionByPet.get(pet)!;
      expect(maxDrop).toBeLessThanOrEqual(0.02);
    }

    // 5. Human pet moved at least 1000 wu in XZ from spawn.
