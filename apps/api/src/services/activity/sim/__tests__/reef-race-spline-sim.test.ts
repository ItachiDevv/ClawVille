...
describe('ReefRaceSplineSim', () => {
  ...
  describe('finish-line crossing', () => {
    it('emits event.crossed_finish when progress reaches 1', () => {
      const events: Array<{ type: string; petId?: string }> = [];
      reefRaceSplineSim.setBroadcastFn((_id, f) =>
        events.push(f as { type: string; petId?: string }),
      );
      reefRaceSplineSim.startRoom(ROOM_ID, 'reef-race', [PET_A]);
  ...
  describe('wall clamp', () => {
    it('pushes body outside corridor back inside', () => {
      ...
      // Place body far outside corridor at lagoon (halfWidth=50).
      // At z=1500 (CP1), centerline x=0. Put body at x=500.
      body.x = 500;
      body.z = 1500;
      ...
      // After clamp, body should be within the corridor (halfWidth=50 at this segment).
      const closestDist = Math.abs(body.x); // centerline is x=0 here
      // The corridor at lagoon is halfWidth=50 with some tolerance for wall-inset.
      expect(closestDist).toBeLessThan(60);
    });
  });
  ...
  describe('forfeit', () => {
    it('marks body as dnf and emits event.player_left', () => {
      ...
      expect(
        events.some(
          (e) => e.type === 'event.player_left' && e.petId === PET_A,
        ),
      ).toBe(true);
    });
  });
  ...
  describe('getStateSnapshot', () => {
    ...
    it('returns snapshot with bodies', () => {
      ...
      expect(snap!.bodies[0].petId).toBe(PET_A);
    });
  });
});
