...
class ReefRaceSim {
   ...
   *
   * Phase 4 (C3 fix) — for Reef Race rooms, embeds per-pet best lap +
  ...
  computeResults(
    ...
    for (const d of dnfers) {
      out.push({
        ...
        // pet has no anti-cheat flags AND bestLapMs is set.
  ...
  private flag(
    ...
    if (isPhysics) {
      ...
      console.warn(
        `[reef-race anti-cheat] physics-flag (NOT counted toward forfeit) room=${state.roomId} pet=${petId} kind=${kind} detail=${detail ?? '-'}`,
      );
    }
    if (reachedThreshold) {
      const body = state.bodies.get(petId);
      if (body) {
        ...
        console.warn(
          `[reef-race anti-cheat] integrity-forfeit room=${state.roomId} pet=${petId} kind=${kind} detail=${detail ?? '-'} flagCount=${state.flagCounter.countFor(petId)}`,
        );
        this.broadcastFn(state.roomId, {
          type: 'event.player_left',
          petId,
...
/**
 * Phase 4 (C3 fix) — per-pet Reef Race extension on `SimResultRow`.
