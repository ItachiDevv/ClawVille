...
export class ReefRaceSplineSim {
    ...
    if (opts?.bots) {
      for (const ctrl of opts.bots) {
        if (!participantPetIds.includes(ctrl.petId)) {
          console.warn(
            `[spline-sim] bot controller for ${ctrl.petId} not a participant — skipping`,
          );
          continue;
        }
    ...
    if (state.botControllers.size > 0) {
      ...
      for (const [petId, ctrl] of state.botControllers) {
        ...
        try {
          ...
          console.error(`[spline-sim] bot onSpawn threw for ${petId}:`, err);
        }
      }
    }
  ...
  getFlagCount(roomId: string, petId: string): number {
    const state = this.rooms.get(roomId);
    if (!state) return 0;
    return state.flagCounter.countFor(petId);
  }
  ...
  private applyIntentForTick(
    ...
    if (deltaMag > maxLegitDelta) {
      ...
      this.flag(state, body.petId, 'overaccel',
    ...
    if (posDelta > maxPosDelta) {
      ...
      this.flag(state, body.petId, 'overspeed',
  ...
  private resolveProgress(state: SplineRoomState, now: number): void {
    for (const body of state.bodies.values()) {
      ...
      if (!progressIsMonotonic(newProgress, body.prevProgress)) {
        this.flag(state, body.petId, 'checkpoint_skip',
  ...
  private computeLivePlacements(state: SplineRoomState): Map<string, number> {
    ...
    for (const b of state.bodies.values()) {
      if (b.dnf || b.forfeited) {
        ...
        racing.push({ petId: b.petId, progress: b.progress, finishedAt: null, dnf: false });
      }
    }
    racing.sort((a, b) => {
  ...
  private flag(
    ...
    const reachedThreshold = isPhysics ? false : state.flagCounter.bump(petId);
    ...
    if (isPhysics) {
      console.warn(
        `[spline-sim anti-cheat] physics-flag (NOT forfeit) room=${state.roomId} pet=${petId} kind=${kind}`,
      );
    }
    ...
    if (reachedThreshold) {
      ...
      if (body) {
        ...
        body.alive = false;
        console.warn(
          `[spline-sim anti-cheat] integrity-forfeit room=${state.roomId} pet=${petId}`,
        );
        this.broadcastFn(state.roomId, {
          type: 'event.player_left',
          petId,
  ...
  private runBotControllers(
    ...
    for (const [petId, ctrl] of state.botControllers) {
      ...
      try {
        ...
        console.error(`[spline-sim] bot ${petId} computeInput threw:`, err);
        continue;
      }
