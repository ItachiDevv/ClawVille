...
class ActivityWsHub {
  /** SINGLE-POD: per-room → petId → WS connection */
  private rooms = new Map<string, Map<string, HubWs>>();

  /** Reconnect grace timers keyed by `${roomId}:${petId}` */
  private graceTimers = new Map<string, ReturnType<typeof setTimeout>>();

  /** Rate tracker shared across all rooms (stateless per-pet) */
   ...
   * the WS with 4001) or if the pet isn't in the room participants.
   */
  async registerConnection(ws: HubWs, firstFrame: ClientFrame): Promise<boolean> {
    ...
    const participant = room.participants.get(identity.petId);
    if (!participant) {
    ...
    this.clearGraceTimer(roomId, identity.petId);
    ...
    // If the same pet has an existing connection (reconnect collision),
    // close the old one first.
    const existing = roomMap.get(identity.petId);
    if (existing && existing !== ws) {
      this.safeClose(existing, ACTIVITY_WS_CLOSE_CODES.UNAUTHORIZED, 'superseded by new connection');
    }
    roomMap.set(identity.petId, ws);
  ...
  unregisterConnection(ws: HubWs): void {
    ...
    if (data.internalCloseCode === ACTIVITY_WS_CLOSE_CODES.INTEGRITY) {
      this.notifyForfeit(room, petId, 'integrity');
      return;
    }
    ...
    if (data.internalCloseCode === 1000) {
      this.notifyForfeit(room, petId, 'voluntary');
      return;
    }

    // Otherwise set a 10s grace timer.
    const key = `${data.roomId}:${petId}`;
    const timer = setTimeout(() => {
      this.graceTimers.delete(key);
      const stillRoom = activityRoomManager.getRoom(data.roomId);
      if (!stillRoom) return;
      const p = stillRoom.participants.get(petId);
      if (!p) return;
      if (p.connected) return; // reconnected in time
      this.notifyForfeit(stillRoom, petId, 'timeout');
    }, RECONNECT_GRACE_MS);
  ...
  /**
   * Send a frame to a single pet. Used by the queue's match.found
   * delivery (chunk #2 registered this callback) and by the sim for
   * per-pet events.
   */
  sendToPet(roomId: string, petId: string, frame: ServerFrame): void {
    const ws = this.rooms.get(roomId)?.get(petId);
    if (!ws) return;
    this.safeSend(ws, frame);
  }

  /** Broadcast a frame to every connected pet in the room */
  ...
  private handleInput(ws: HubWs, frame: Extract<ClientFrame, { type: 'input' }>): void {
    ...
    const rate = this.rateTracker.validateInputRate(identity.petId);
    if (!rate.ok) {
  ...
  private notifyForfeit(room: Room, petId: string, reason: 'voluntary' | 'timeout' | 'integrity'): void {
  ...
  private clearGraceTimer(roomId: string, petId: string): void {
