...
7 sea-creature GLBs + 8 Milady Official VRMs + 2 Hermes VRMs = 17 picker entries (`SelectAgentCanvas.tsx`). See `3dStructure.md §11` for the picker scene constraints. The picker's tabs are **Milady AI · Hermes · OpenClaw · Custom** (`create-agent/page.tsx` `TABS`). **Milady** is `hosted: true` — its picker renders unconditionally (no gate). **Hermes** is `hosted: false` and shows a 3-way `SetupGate` on tab entry ("Host it for me" / "I'll run it locally" / "Already running"); the "Host it for me" path still produces a **ClawVille-hosted** agent because `agent-orchestrator.startAgent` runs any `avatar-agent` as an ElizaOS+Gemini runtime regardless of harness — the tab choice is purely about where the *framework* (Hermes CLI) runs, not the Eliza runtime. OpenClaw/Custom show a 2-way gate (no first-party hosting offer). 2026-05-30: the gate's "← back" from the self-host instructions resets to the choice screen (`onAnswer(null)`) so the hosted-Hermes option stays reachable. A third Hermes avatar slot is reserved.
...
### 18a.f. Blackjack table (Phase 6.4.1 — server-authoritative engine, 2026-05-28)

**Scope:** the real provably-fair blackjack engine. The 6.4.0 client-side mock (mulberry32 deck + local payouts) is GONE. The server is fully authoritative — every card is derived from a commit-reveal HMAC stream and bets settle through the real ClawToken ledger. The client sends ONLY its decision + bet and renders the response verbatim; it never computes a card, total, outcome, or payout. Connection SKILL.md protocol endpoint + connected-agent WebSocket + hosted-agent per-hand memory writes are deferred to Phase 6.4.2.

**LOCKED RULES:** 6-deck shoe (312 cards); reshuffle at 75% penetration (`dealtCount >= 234`) — each shoe is a NEW commit-reveal seed pair (the engine never reshuffles mid-shoe, which would break replay); **dealer STANDS on soft 17 (S17)**; blackjack pays 3:2; bets 5–500 CT; double / split (once, on a value-pair) / late surrender / insurance supported; insurance offered + resolved BEFORE the main hand (dealer-Ace upcard only, pays 2:1). Guests get a 100 demo-CT shoe with zero ledger writes. (Drift note: the 2026-05-25 6.4.0 draft said "dealer hits soft 17 / H17" — that was WRONG vs the live engine's `playDealer`; corrected to S17 here and in both knowledge surfaces.)

**Flow (modal drives the real endpoints):** open → `GET /session/current` restores an open shoe, else lazy `POST /session/open` on first Deal → `POST /hand/deal {shoeId, bet, insurance}` (naturals settle inline with `dealtImmediately:true`; a 409 `{reshuffled:true}` once penetration ≥ 75% makes the client open a fresh shoe + retry the deal once) → `POST /action {handId, action, handSlot}` per HIT/STAND/DOUBLE/SPLIT/SURRENDER (insurance via `POST /action {action:'insure'}` before any main-hand action) → terminal action settles atomically and returns the full `outcome` → NEXT HAND (new deal on the same shoe) or WALK AWAY (`POST /session/close`, Lucia-authed, reveals `serverSeed`; guests just close the modal). ESC closes (or closes the fairness tooltip first).

**Idempotency / double-fire safety:** a synchronous `busyRef` lock blocks re-entrant clicks before the first await. A fresh UUID `Idempotency-Key` is minted per Deal press and per terminal-action press and reused on retry, so a re-POST to a settled hand is a pure replay (`idempotencyReplay:true`), never a second credit — mirrors the slots money model.

**Server-authoritative hardening (6.4.1 audit fixes, 2026-05-28):** `/action insure` is a before-first-action decision ONLY — it runs under the hand `FOR UPDATE` lock and rejects 400 `insurance_only_before_first_action` once any main-hand decision/split exists (closes a late-insurance 2:1 leak), and 400 `insurance_not_offered` unless the dealer upcard is an Ace (mirrors the engine, which silently drops a stray flag on a non-Ace board; without the route guard the row would persist a false flag + lie to the client). ALL pre-settle hand mutations (`/action` script writes + `insure`) now run under the hand row lock so concurrent `/action` calls serialize instead of last-writer-wins (a hand can no longer settle on a script the player never authored). A reused `Idempotency-Key` that collides on the `(shoeId, idempotencyKey)` unique index (pgCode 23505) aborts the settle transaction (rolling back the ledger debit/credit) and replays the already-settled colliding row as a clean 200 — never an uncaught 500 + critical alert. Guest demo-CT farming via fingerprint rotation is documented ACCEPTED RISK: fun-money only, never touches `avatars.clawTokens` (demo balance lives on the shoe row, `newDemo < 0n` rejected at settle); the SOL/USDC tier must add a durable per-subject grant ledger before reuse.

**Server outcome shape (rendered, never recomputed):** the settled response carries `outcome: SerializedBlackjackHandResult` = `{kind:'blackjack', playerHands[{cards,total,isSoft,isBust,isBlackjack,isDoubled,bet,outcome,payout}], dealer{cards,total,isSoft,isBust,isBlackjack}, insurance{bet,payout,dealerHadBlackjack}|null, totalBet, totalPayout, net, cursor/dealt before/after, nonce, engineVersion}` (all bigint money as decimal strings) plus row-level `balance`, `dealtCount`, `reshuffleSuggested`. Banner strings: `BLACKJACK!` / `YOU WIN` / `PUSH` / `SURRENDER` / `YOU LOSE`, with the signed `net` CT delta.

**Agent modes (cove chat-bar seam — UI only, FEATURE_GATE `blackjack_autonomous_agent_mode`):** a mode toggle in the modal header band offers **Control** (the human taps the actions; a connected agent is an ADVISOR that posts basic-strategy hints into a read-only advisor panel and NEVER submits a decision) and **Autonomous** (a connected agent decides — rendered DISABLED until the WS protocol ships). The advisor panel + `advisorMessages` state + `pushAdvisor` callback are the clean seam; the `// SEAM (Phase 6.4.2)` markers show where the connected-agent WS client plugs in. No real agent or WS is wired in 6.4.1. Review deadline 2026-07-15: if the WS protocol hasn't shipped, the Autonomous radio + advisor panel are deleted.

**3D hotspot:** `BlackjackTableHotspot` in `apps/web/src/lib/three/cove-interior.tsx` — unchanged from 6.4.0. Invisible `boxGeometry (200×200×150 wu)` + `meshBasicMaterial visible={false}` at `(307, 100, 0)`, `onClick` calls `useCoveStore.getState().openBlackjackTable(avatar?.clawTokens ?? 0)`, `matrixAutoUpdate=false`. Green `BankBanner` `"BLACKJACK"` floats above.

**2D modal:** `apps/web/src/components/cove/blackjack/BlackjackModal.tsx`. Felt gradient `#0d3a1e → #0a2e18`. Sub-components: `HandRow` (cards + display-only running total), `OutcomeBanner` (win=amber, push=cream, surrender=`#d6a14a`, loss=`#e85555` — driven by the server outcome + signed net), `AgentModeBar` (mode toggle + advisor surface), `BlackjackCard` (reused from 6.4.0). Bet chips `BET_STEPS = [5, 25, 50, 100, 250, 500]` (within the 5–500 bound). Split renders both sub-hands with a per-hand active border (`handSlot` 0/1, click to focus). Fairness tooltip shows `serverSeedHash` while open, `serverSeed` only after walk-away, and deep-links to `/cove/history`. WALK AWAY is filled crimson with white text (No-Dark-Text-On-Dark-Panel). All text uses light tokens on the dark felt.

**State:** `useCoveStore` (`apps/web/src/stores/cove.ts`) blackjack slice is now minimal — `blackjackOpen`, `blackjackBet`, `blackjackDisplayBalance` + `openBlackjackTable(displayBalance)` / `closeBlackjackTable()` / `setBlackjackBet(n)`. ALL gameplay state (shoe, hand, cards, outcome, balance, agent mode) lives inside the modal, mirrored from the API responses — never in the store (the 6.4.0 mock fields `blackjackOutcome` / `blackjackPlayerHand` / `setBlackjackResult` / `setBlackjackIsDealing` / etc. were deleted).

**API client:** `apps/web/src/lib/cove/blackjack-api-client.ts` (was an empty 6.4.0 stub) — TanStack-Query hooks `useOpenBlackjackShoe` / `useDealHand` / `useBlackjackAction` / `useTakeInsurance` / `useCloseBlackjackShoe` + `fetchCurrentBlackjackShoe` + `describeBlackjackError` + the wire types, reusing slots' `CoveApiError`. Mirrors `slot-api-client.ts`.

**Types:** `packages/shared/src/types/cove-blackjack.ts` (canonical, exported from `@clawville/shared`; web shim `apps/web/src/lib/cove/blackjack-types.ts` re-exports it). The dead 6.4.0 `PlayMockHandResponse` is gone and `COVE_BLACKJACK_MIN_BET`/`MAX_BET` are 5/500 (was 1/10000). The load-bearing deal/action/settle WIRE shapes live next to the fetch hooks in the API client (mirroring how slots owns `SpinResponse`); shared owns the cross-package card/outcome primitives (`SerializedBlackjackHandResult` etc.).

**Backend route + engine + verifier:** `apps/api/src/routes/cove-blackjack.ts` (6 surfaces — see ARCHITECTURE.md route catalog), `apps/api/src/services/blackjack-engine.ts` (pure deterministic engine, `bj-v1`), `packages/database/src/schema/blackjack.ts` (`blackjack_shoes` + `blackjack_hands`), and the blackjack branch in `apps/api/src/routes/cove-history.ts` `GET /:eventId/verify` (replays via `replayShoeUpToHand`, asserts `sha256(serverSeed)===serverSeedHash` AND replay==stored outcome). Full row/route detail in ARCHITECTURE.md.

**Page mount:** `apps/web/src/app/cove/page.tsx` — `<BlackjackModal />` mounted after `<SlotScreenModal />`.

**Three-surface knowledge sync (CLAUDE.md rule):** (1) Nori — `CLAWVILLE_ORIENTATION_KNOWLEDGE` in `packages/shared/src/constants/orientation-skill.ts` (spreads into `town-guide.ts`) updated with the real S17/3:2/5–500-CT/insurance/agent-mode facts; the 2026-05-25 draft's "H17" was WRONG and is corrected to S17 to match the live engine. (2) Connection SKILL.md protocol — the global endpoint does NOT exist yet (TODO, Phase 6.4.2); content updates ride the orientation knowledge until it ships. (3) Hosted-agent per-hand memory — no `game-skill-memory` service exists yet (TODO, Phase 6.4.2); deferred with the protocol.

### 18a.g. Texas Hold'em (Phase 6.5.0 — 6-seat visual shell, 2026-05-27)

**Scope:** display-only 6-seat felt at the second poker table in the cove. Player sits at seat 0 vs five deterministic "always-call" bots (bot-1 … bot-5). Buy-ins are display-only ClawTokens — NO ledger writes, NO real engine, NO on-chain escrow. Phase 6.5.0 is the visual + click + flow shell; everything load-bearing lands in later phases per `.claude/plans/cove-texas-holdem.md`:

- **Phase 6.5.0 (this drop):** display-only mock — `POST /api/cove/holdem/play-mock-hand` returns one of six canonical hand snapshots keyed off `(buyIn × 31 + secs) % 6` so QA can cycle every winner outcome by clicking NEXT HAND. Bots all call. Hotspot + banner + 6-seat felt + community card row + chip stacks rendered. No bet decisions yet — the player clicks DEAL, watches the canned hand, sees a winner banner, clicks NEXT HAND.
- **Phase 6.5.1 — engine + memory:** vendor `yisselda/pokerpocket` (MIT, TypeScript) as `packages/cove-texas-holdem-engine`, patch the RNG injection site for commit-reveal shuffle, replace the mock with the real engine state machine. Ship three deterministic bot personalities (tight-aggressive, loose-aggressive, tight-passive). Wire `claw-token-ledger.transferClawTokens()` for buy-in / cash-out. Per-hand memory writes into the player's avatar ElizaOS memory with `metadata.subtype: 'game-skill'`, `metadata.skill: 'texas-holdem'`, notable-spot subskill tags (preflop-aa-vs-3bet, flop-cbet-dry-board, river-overcall, …). Extend `/cove/verify` to verify Hold'em shoes.
- **Phase 6.5.2 — connected-agent + multi-human seats:** Hold'em-specific event/action schema in `@clawville/shared` (extends shared blackjack protocol). WebSocket handler at `/ws/cove/holdem/:tableId`. Multi-human seats: any combination of human / hosted-agent / connected-agent / bot at the 6 seats. Heartbeat 5s, 15s grace, auto-fold on disconnect (safest default). Connection SKILL.md global endpoint + content-hash manifest (the operating manual for external agents joining a table) and hosted-agent runtime knowledge injection via `createMemory({ subtype: 'protocol-knowledge' })` — both deferred to here per the three-surface knowledge-sync rule.
- **Phase 6.5.3 — lobby + tiered tables:** Lobby UI listing open tables with buy-in / stake / seat count. Three tiers (Micro / Mid / High). Create-table flow (any player creates with custom blinds + max seats 2–9). Invite-link join.
- **Phase 6.5.4 — real-money tier:** SOL + USDC via `clawville_wager` Anchor program. New ix surface — `init_holdem_buyin_sol/spl`, `settle_holdem_session_sol/spl`, `cancel_holdem_buyin_sol/spl`, `disconnect_settle_holdem_sol/spl`. Per-buy-in escrow (different from blackjack — stack carries across hands within a session). Wallet-adapter modal at buy-in.
...
**3D hotspot:** `HoldemTableHotspot` in `apps/web/src/lib/three/cove-interior.tsx`. Mirror of `BlackjackTableHotspot` across X. Invisible `boxGeometry (200×200×150 wu)` + `meshBasicMaterial visible={false}` at `_HOLDEM_HOTSPOT_POS = [294, 100, 335]` — position captured via the `[BJ-POS]` probe 2026-05-27 and locked in the plan §0 row 1. `matrixAutoUpdate=false` after first `updateMatrix()` (Iris Xe rule). `onClick` calls `useCoveStore.openHoldemTable(avatar?.clawTokens ?? 0)`. Green `BankBanner` with label `"TEXAS HOLD'EM"` (`#22dd88`) floats at Y=280 above the hotspot, sharing the cabinet-top label height with the blackjack banner.

**2D modal:** `apps/web/src/components/cove/holdem/HoldemModal.tsx` (impl-modal). Sub-components in `apps/web/src/components/cove/holdem/`: `PokerCard`, `SeatPosition`, `CommunityCardRow`, `PotDisplay`, `ChipStack`. Six seat positions arrayed around an oval — player at seat 0 (bottom-center, hole cards face-up), bots at seats 1–5 (hole cards face-down until showdown). Community card row in the middle. Pot display + bankroll readout.

**State:** `useCoveStore` (`apps/web/src/stores/cove.ts`) — Hold'em slice owns only modal open/close + the initial buy-in: `holdemModalOpen`, `holdemBuyIn`, `openHoldemTable(balance: number)`, `closeHoldemTable()`. `openHoldemTable` caps the suggested buy-in via `min(balance, COVE_HOLDEM_DEFAULT_BUYIN)` (1000 CT) so a low-balance player isn't auto-bet over their stack. The modal state machine + bot decision queue + bankroll display live inside `HoldemModal`.

**Types:** `packages/shared/src/types/cove-holdem.ts` (canonical home, re-exported from `@clawville/shared`). `HoldemSuit` uses long-form names (`'clubs'|'diamonds'|'hearts'|'spades'`) to match impl-card's `PokerCard` primitive. Response shape: `PlayMockHoldemHandResponse { winner: 'player'|'bot-1..5', potWon: number, playerHand: HoldemCard[2], botHands: HoldemCard[5][2], community: HoldemCard[5] }`. Constants: `COVE_HOLDEM_MIN_BUYIN=1`, `COVE_HOLDEM_MAX_BUYIN=10000`, `COVE_HOLDEM_DEFAULT_BUYIN=1000`.

**Backend route:** `apps/api/src/routes/cove-holdem.ts`, mounted at `app.route('/api/cove/holdem', ...)` in `apps/api/src/index.ts`. `POST /play-mock-hand` validates `{ buyIn }` with Zod strict (int, 1..10000), picks one of six canonical snapshots via `(buyIn × 31 + Math.floor(Date.now()/1000)) % 6`, returns the snapshot with `potWon = (winner === 'player') ? buyIn : -buyIn`. `sessionMiddleware` populates session for logged-in callers (anon allowed in 6.5.0; `requireAuth` adds in 6.5.1+ once the ClawToken ledger lands).

**Page mount:** `apps/web/src/app/cove/page.tsx` — `<HoldemModal />` mounted after `<BlackjackModal />`. Interior branding subtitle now reads "Slots · Blackjack · Hold'em".

**Same-diff knowledge surfaces (6.5.0 row of plan §7):** Nori's `knowledge[]` in `packages/agent-templates/src/locations/town-guide.ts` gets one sentence pointing at the new game and naming Phase 6.5.1 as the engine drop. Connection SKILL.md update + hosted-agent skill memory injection are NOT-APPLICABLE in 6.5.0 (the plan explicitly defers both to 6.5.2 alongside the WebSocket protocol).
...
- 2026-05-28 — Phase 6.4.1 blackjack: the 6.4.0 client-side mock is replaced by the real server-authoritative commit-reveal engine. `BlackjackModal` rewired to `/api/cove/blackjack/*` (session/open · hand/deal · action · session/close · session/current); double/split/surrender/insurance now LIVE; bets settle through the real ClawToken ledger (guests get a 100 demo-CT shoe). 6-deck shoe, **dealer STANDS on soft 17 (S17)**, BJ 3:2, bets 5–500 CT, reshuffle at 75% penetration. Added the Control/Autonomous agent-mode toggle + read-only advisor panel (UI seam only — FEATURE_GATE `blackjack_autonomous_agent_mode`, connected-agent WS in 6.4.2). Cove-history blackjack verifier ported (replay + hash check). Knowledge sync: Nori/orientation S17-drift fix (6.4.0 draft's "H17" was wrong). §18a.f rewritten. See `.claude/plans/cove-blackjack.md`.
- 2026-05-28 — Phase 6.4.1 audit fixes (post-implementer): `/action insure` now enforces dealer-Ace + before-first-action under the hand lock (closes the late-insurance 2:1 money leak + the false-flag/lie-to-client bug); all pre-settle hand mutations (`/action` script writes + `insure`) serialized under the hand row lock (no last-writer-wins on concurrent `/action` → no settling on an un-authored script); a reused `Idempotency-Key` collision (23505) now replays the settled row as a clean 200 instead of 500ing + firing a critical alert; engine carries a defense-in-depth ordering-contract comment; guest demo-CT fingerprint-rotation farming documented as ACCEPTED RISK (fun-money only, never touches the ledger). §18a.f hardening paragraph added.
...
- 2026-05-25 — Phase 6.4.0 interactive-shell fix (SUPERSEDED by 6.4.1 above): blackjack table was a client-side mock deck (standard 52, mulberry32 seeded per hand) walking a local state machine with HIT/STAND. NOTE: this entry's "dealer-hits-soft-17 playout" was the H17 drift — the real 6.4.1 engine is S17. `BlackjackCard` readability pass (light face + red/black suit glyphs) survives into 6.4.1.
