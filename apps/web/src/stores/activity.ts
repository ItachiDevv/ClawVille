...
export interface ActivityState {
  ...
  /**
   * SPEC 3 — last `event.ramp_launch` received (any pet, not just self).
   * `ReefRacePlayer` subscribes to drive extended nose-up tilt, particle
   * burst, and screen shake for the self pet. null until first ramp launch.
   */
  lastRampLaunchEvent: { petId: string; rampId: string; at: number } | null;
   ...
   * pet. Driven by `EntityDelta.changed.streak` (per-tick mirror).
  ...
  /**
   * v2 — true once the LOCAL pet has crossed the finish line for this match.
   * Set by `event.crossed_finish` when `petId === selfPetId`. Cleared on
   ...
   * is still `live` AND the local pet has finished.
   */
  selfFinished: boolean;
  /**
   * v2 — local pet's final placement (1 = first), set by `event.crossed_finish`
   * when the petId matches self. null until self finishes.
   */
  selfPlacement: number | null;
  /**
   * v2 — local pet's authoritative total race time in ms, set by
   ...
   */
  finishedRacers: Array<{ petId: string; placement: number; totalMs: number }>;
...
> {
  return {
    ...
    finishedRacers: [],
    // SPEC 1 — per-pet GLB species metadata
    reefParticipantMeta: {},
  };
}
    ...
    applyServerFrame: (frame) => {
      ...
      switch (frame.type) {
        ...
        case 'snapshot.init': {
          ...
          // Phase 4 — populate the self pet's PB ghost path from the
          // snapshot.init payload (per-recipient, server already gated
          // on identity). Empty/null when the pet has no PB row yet.
          const selfGhost =
          ...
          if (Object.keys(participantMeta).length > 0) {
            ...
            injected.forEach((e, petId) => {
              ...
              if (meta) {
                injected.set(petId, { ...e, species: meta.modelKey });
              }
            });
        ...
        case 'snapshot.delta': {
          ...
          // positional change. `applyEntityDelta` has no selfPetId access.
        ...
        case 'event.match_ended': {
          ...
          // pet AND the match brought new ghost frames, swap in the new
        ...
        case 'event.player_left': {
          ...
          if (frame.reason === 'integrity' && state.selfPetId === frame.petId) {
        ...
        // SPEC 3 — ramp launch. Stored for ALL pets so ReefRacePlayer can
        // apply extended tilt to other visible riders, and so the self-pet
        // handler can fire particles + screen shake.
        case 'event.ramp_launch': {
          set({ lastRampLaunchEvent: { petId: frame.petId, rampId: frame.rampId, at: Date.now() } });
          break;
        }
