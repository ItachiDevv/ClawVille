...
import { NPC_BUILDING_CENTERS } from '@legacyapp/shared';
  ...
  tick: () => {
    ...
    switch (state.tickState) {
      ...
      case 'planning': {
        ...
        const path = findPath(game.petPosition.x, game.petPosition.y, center.x, center.y);
        if (path.length === 0) {
        ...
        // Set the click path — player-pet.tsx will follow it
      ...
      // ---------------------------------------------------------------
      // TRAVELING — wait for pet to reach building proximity
      // ---------------------------------------------------------------
      case 'traveling': {
        ...
        if (!game.clickPath) {
          ...
          const center = NPC_BUILDING_CENTERS[goal.targetBuildingId];
          if (center) {
            const dx = game.petPosition.x - center.x;
            const dy = game.petPosition.y - center.y;
          ...
          if (center2) {
            const path = findPath(game.petPosition.x, game.petPosition.y, center2.x, center2.y);
            if (path.length > 0) {
              game.setClickPath(path, goal.targetBuildingId);
            } else {
...
function planNextGoal(
  state: AutonomyState,
  game: { petPosition: { x: number; y: number }; visitedBuildings: Set<string> },
): AgentGoal | null {
