...
export interface GameState {
  ...
  // Pet appearance (species + color for sprite rendering; modelKey for 3D GLB)
  petSpecies: string;
  petColor: string;
  petName: string;
  /** Phase 2: stable model key from AGENT_MODELS registry — drives GLB in player-pet.tsx */
  petModelKey: string;
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  setControlMode: (mode) => {
    ...
    // Reset jump state on every mode transition — prevents pet being stranded airborne
    ...
    // Guest-pet auto-create (2026-04-23) — entering NPC mode without a
    // pet means the visitor is "test-driving" before signup. Mint a
    // throwaway guest pet in the background so the activity portals,
    ...
    // GuestPetBootstrap component (mounted at /game and /activity) does
    // the actual API call + cache invalidation + welcome toast.
    if (mode === 'npc' && prev !== 'npc' && typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('clawville:ensure-guest-pet'));
    }
  ...
  setHasAgent: (v) => {
    // Reset jump state before mode change — prevents pet being airborne on agent connect/disconnect
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { resetJump } = require('@/lib/three/jump-state') as typeof import('@/lib/three/jump-state');
    resetJump();
  ...
  // and never-customized pets render as Miladys (matches the canonical default
  ...
  setAgentConnection: (sessionId) => {
    ...
    // Reset jump state on agent connect/disconnect — prevents pet being stranded airborne
