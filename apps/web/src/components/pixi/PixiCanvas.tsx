...
import { getSpeciesConfig, blendColors, COLOR_TINT_MAP } from '@/lib/pixi/avatar-sprites';
import type { AvatarColor } from '@legacyapp/shared';
import {
  MAP_WIDTH,
  MAP_HEIGHT,
  TILE_SIZE,
...
export default function PixiCanvas() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Avatar container (procedural sprite)
      const avatarContainer = new Container();
      world.addChild(avatarContainer);

      // Avatar body graphics (will be redrawn each frame)
      const petBody = new Graphics();
      avatarContainer.addChild(petBody);
      ...
      function drawPetSprite(
        ...
        const tint = COLOR_TINT_MAP[color as AvatarColor] ?? 0xffeb3b;
      ...
      app.ticker.add((ticker) => {
        ...
        // Redraw avatar procedural sprite each frame for smooth animation
        if (petGraphicRef.current) {
          ...
          lastSpecies = state.avatarSpecies;
          lastColor = state.avatarColor;
        }

        // Update avatar container position
        avatarContainer.x = state.avatarPosition.x;
        avatarContainer.y = state.avatarPosition.y;
