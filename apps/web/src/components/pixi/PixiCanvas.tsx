...
import type { PetColor, PetSpecies } from '@legacyapp/shared';
import {
  MAP_WIDTH,
  MAP_HEIGHT,
  TILE_SIZE,
...
function createNpcSprite(npc: NpcSpriteState): NpcSpriteInfo {
  ...
  const targetH = 56; // NPCs slightly smaller than player pet
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!isSpectator) {
        ...
        petContainerRef.current = petContainer;
        ...
        bubbleContainer.visible = false;
        petContainer.addChild(bubbleContainer);

        footstepContainer = new Container();
        world.addChild(footstepContainer);

        // Pet sprite itself is created in the texturesReady callback above
      }
      ...
      texturesReady.then(() => {
        spritesLoaded = true;
        // Create pet sprite once textures are available
        if (!isSpectator && petContainer && !petSpriteRef.current) {
      ...
      let elapsedTime = 0;
      let lastSpecies: string = (useGameStore.getState().petSpecies || 'cat');
      ...
      app.ticker.add((ticker) => {
        ...
        // ---- Pet rendering (non-spectator only) ----
        if (!isSpectator && petContainer && petSpriteRef.current) {
          // Update pet sprite if species changed
          if (st.petSpecies !== lastSpecies) {
          ...
          const petIsMoving = st.movementDirection !== 'idle';
          ...
          if (footstepContainer) {
            if (petIsMoving) {
              ...
              if (footstepTimer > 0.12) {
                ...
                const px = st.petPosition.x + (Math.random() - 0.5) * 10;
                const py = st.petPosition.y + 4 + Math.random() * 4;
