...
export default function HudMiniLeaderboard({ entries, selfId, max = 5 }: HudMiniLeaderboardProps) {
  ...
  const selfIncluded = visible.find((e) => e.avatarId === selfId);
  const selfEntry =
    selfId && !selfIncluded ? entries.find((e) => e.avatarId === selfId) ?? null : null;
  ...
  return (
    ...
    >
      ...
      {visible.map((e, i) => (
        <Row key={e.avatarId} rank={i + 1} entry={e} isSelf={e.avatarId === selfId} />
      ))}
      {selfEntry && (
        <>
          <div
          ...
          />
          <Row
            rank={entries.findIndex((x) => x.avatarId === selfEntry.avatarId) + 1}
            entry={selfEntry}
            isSelf
          />
        </>
