 ...
 * button. Two filters: pets (search by name) and agents (list connected
 * agents).
 *
 * Q2 reality:
...
export type InviteFilter = 'pets' | 'agents';
...
export default function InviteSearchPopover({
  ...
  const title = filter === 'pets' ? 'Invite a Friend' : 'Invite an Agent';
  const placeholder =
    filter === 'pets'
      ? 'Search pets by name…'
