...
export interface EliminatedOverlayProps {
  ...
  /** Self avatar id — used for leaderboard self-row + chat self-tint. */
...
export default function EliminatedOverlay(props: EliminatedOverlayProps) {
  ...
  // Derive the spectated-avatar display name for the header row.
  ...
  // Resolver for chat names — falls back to short avatarId tail.
  const resolveName = (avatarId: string) => resolveDisplayName(avatarId, scoreLookup, aliveEntities);
