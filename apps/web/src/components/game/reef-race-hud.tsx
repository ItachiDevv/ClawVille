 ...
 *     has no laps; we show the local pet's `entity.progress` (0..1 fraction
 ...
 *   - <WaitAtFinishOverlay> is rendered once the local pet crosses the
 ...
 */
function ProgressBar({ selfPetId }: { selfPetId: string | null }) {
  const progress = useActivityStore((s) => {
    if (!selfPetId) return 0;
    const e = s.entities.get(selfPetId) as any;
...
/**
 * v2 — wait-at-finish centered overlay shown when the local pet has crossed
          ...
          <div style={{ marginTop: 16 }}>
            ...
            >
              {finishedRacers
                ...
                .map((r) => {
                  const isSelf = selfPetId && r.petId === selfPetId;
                  const name =
                    scores.get(r.petId)?.displayName ??
                    (r.petId.length > 8 ? `…${r.petId.slice(-6)}` : r.petId);
                  return (
                    <div
                      key={r.petId}
                      style={{
...
export default function ReefRaceHud({
  ...
  return (
    <div style={baseStyle}>
      ...
      >
        {USE_SPLINE ? (
          <ProgressBar selfPetId={selfPetId} />
        ) : (
      ...
      {/* v2 — wait-at-finish overlay. Internal gates ensure it only renders
          when the local pet has crossed the finish line AND the match is
