...
export default function AvatarChatBar() {
  ...
  const species = avatar.species as AvatarSpecies;
  const speciesData = AVATAR_SPECIES.find((s) => s.id === species);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const errorMsg: AvatarMessage = {
        id: crypto.randomUUID(),
        role: 'assistant',
        content: `(Error: ${err.message || 'Could not reach avatar agent'})`,
      };
      ...
      {expanded && (
        <div className="w-full mb-2 bg-black/80 backdrop-blur-sm border border-yellow-500/40 rounded-xl overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200">
          ...
          <div className="flex items-center gap-2 px-3 py-2 bg-gradient-to-r from-yellow-600/80 to-yellow-500/80 border-b border-yellow-500/30">
            ...
            />
            <span className="text-black font-bold text-sm">{avatar.name}</span>
