...
/**
 * EditAppearanceSection — in-game Layer 1 edit panel for pet-settings-modal.
 *
 ...
 * Calls `PATCH /api/pets/me/appearance` via `useEditPetAppearance`.
 *
...
import { useEditPetAppearance } from '@/hooks/use-pet';
...
export function EditAppearanceSection({ pet }: EditAppearanceSectionProps) {
  const editMutation = useEditPetAppearance();
  const addToast = useGameStore((s) => s.addToast);

  const currentModelKey = (pet.modelKey ?? DEFAULT_AGENT_MODEL_KEY) as ModelKey;
  const currentColor = (pet.color ?? 'green') as PickerColorId;
  const currentGender = (pet.gender ?? 'male') as 'male' | 'female';

  // Audit fix — `pet.harness` is the server's source of truth for the
  ...
  const currentIsMilady = pet.harness
    ? pet.harness === 'milady'
    : currentModelCategory === 'milady';

  // Draft state (only diffs vs. current pet get sent).
  ...
  // Audit fix — sync drafts when the pet prop changes (after a successful
