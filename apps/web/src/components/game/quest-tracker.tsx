...
function QuestPanel({
  ...
  return (
    <div className={mobile ? 'w-96' : 'w-80'}>
      ...
      <button
        onClick={onToggle}
        className={`w-full legacytheme-panel !p-4 !rounded-xl flex items-center gap-3 hover:brightness-105 transition-all group ${
          isNew ? 'animate-pulse ring-4 ring-legacytheme-green ring-offset-2' : ''
        }`}
      >
        {allDone ? (
        ...
        <div className="flex-1 min-w-0 text-left">
          ...
          {!allDone && activeQuest && (
            <div className="h-2.5 w-full bg-black/15 rounded-full mt-2 overflow-hidden border border-black/5">
              <div
                className="h-full bg-legacytheme-green rounded-full transition-all duration-500"
      ...
      {/* Expanded quest list */}
      {expanded && (
        <div className="mt-2 legacytheme-panel !p-3 !rounded-xl space-y-2 max-h-96 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-150 shadow-xl">
          {/* Active quest highlight */}
          {activeQuest && (
            <div className="bg-white/50 rounded-lg px-3 py-3 mb-2 border-2 border-legacytheme-green/40 shadow-sm">
