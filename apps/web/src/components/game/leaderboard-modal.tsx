...
function StatsBar() {
  ...
  return (
    ...
    >
      <StatPill icon="🐾" label="Pets" value={data.totalPets.toLocaleString()} />
      <StatPill
        icon="💰"
        label="Gold in circ"
...
export default function LeaderboardModal() {
  ...
  return (
    <RpgModal
      ...
      title="Leaderboard"
      subtitle="Single source of truth for every ClawVille pet"
      tier="legendary"
      glow="strong"
      ...
      tokenBadge={
        myPet ? (
          <StatusChip
            tone="warning"
            size="sm"
            label={`${myPet.clawTokens?.toLocaleString() ?? 0} NT`}
          />
        ) : undefined
      }
    >
      ...
      {!isLoading && !isError && entries.length === 0 && (
        ...
        >
          No pets ranked yet — be the first to earn a spot by buying, selling,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {entries.map((entry) => (
            <EntryRow
              ...
              metric={tabMeta.metric}
              unit={tabMeta.unit}
              highlight={entry.petId === me?.petId}
            />
          ))}

          {/* "You are here" fallback row when user is outside the top N */}
          {me && !entries.some((e) => e.petId === me.petId) && (
            <>
              <div
                style={{
                  textAlign: 'center',
      ...
      {data && (
        ...
        >
          {data.rankedCount} of {data.totalPets} pets · refreshed every 30s
        </div>
      )}
    </RpgModal>
  );
