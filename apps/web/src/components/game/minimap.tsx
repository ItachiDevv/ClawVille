...
export default function Minimap() {
  ...
  const handleClick = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    const path = findPath(petPosition.x, petPosition.y, mapX, mapY);
    ...
    >
      ...
      <div className="relative rounded-xl overflow-hidden border border-cyan-400/30 shadow-[0_0_30px_rgba(0,229,255,0.22)] bg-[#04111e]/95 backdrop-blur-md">
        ...
        <div className="flex items-center justify-between px-3 pt-2 pb-1.5 border-b border-cyan-500/15">
          ...
          <span className="font-mono text-[8px] text-white/30">{Math.round(petPosition.x)},{Math.round(petPosition.y)}</span>
        </div>
        ...
        >
          ...
          {/* Pet position — pulsing sonar blip */}
          <circle cx={petX} cy={petY} r={7} fill="url(#pet-glow)" />
          <circle cx={petX} cy={petY} r={3.5} fill="#ffd700" stroke="#fff" strokeWidth={0.8}>
            <animate attributeName="r" values="3;4;3" dur="1.6s" repeatCount="indefinite" />
          </circle>
          <circle cx={petX} cy={petY} r={1} fill="#fff" />
