...
export default function SkillBuilderModal() {
  ...
  // Prefill from pet data
  useEffect(() => {
    if (skillBuilderOpen && pet) {
      const config = pet.characterConfig as any;
      const knowledge: string[] = config?.knowledge ?? [];
      setCustomName(pet.name ?? '');
      setCustomDescription(`OpenClaw knowledge from ${pet.name}`);
  ...
  const handleDownloadSkillMd = () => {
    const slug = (customName || pet?.name || 'skill').toLowerCase().replace(/[^a-z0-9]+/g, '-');
  ...
  const handleDownloadMemory = async () => {
    ...
    try {
      const data = await api.exportMemory(pet.id);
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${pet.name}-memory-pack.zip`;
      a.click();
      URL.revokeObjectURL(url);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg max-h-[85vh] flex flex-col">
        <div className="neopets-panel flex flex-col overflow-hidden">
          {/* Header */}
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <button
                    ...
                    disabled={downloadingMemory || !pet?.id}
                ...
                {/* Publish to In-Game Marketplace */}
                {previewMd && (
                  <div className="bg-orange-50 rounded-lg border border-orange-200 px-3 py-3">
                    <p className="text-sm font-bold text-orange-900 mb-2">Publish to Marketplace</p>
                    <p className="text-xs text-orange-700 mb-2">
                      Share your skill with other players for free.
                    </p>
                    <button
                      onClick={async () => {
                        setPublishing(true);
                        try {
                          await api.publishSkill({
                            name: customName || pet?.name || 'Unnamed Skill',
                            description: customDescription || 'A skill from ClawVille',
                            skillMd: previewMd,
                          });
                          setPublished(true);
                          addToast('🛒', 'Skill published to marketplace!');
                        } catch (err: any) {
                          addToast('!', err.message || 'Publish failed');
                        }
                        setPublishing(false);
                      }}
                      ...
                      className={`w-full text-sm font-bold py-2 rounded-lg transition-colors ${
                        published
                          ...
                          : 'color-btn bg-orange-500 hover:bg-orange-600 disabled:opacity-50'
                      }`}
                    >
                      {published ? 'Published!' : publishing ? 'Publishing...' : 'Publish (Free)'}
                    </button>
                  </div>
                )}
              </>
