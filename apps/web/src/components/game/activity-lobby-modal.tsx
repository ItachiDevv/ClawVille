  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      >
        ...
        {/* Slot 1 — self (or sign-in CTA when there's no pet) */}
        <PartySlot
          member={selfMember ?? undefined}
          ctaLabel="Sign in to join a party"
          ctaDisabled
          ctaDisabledReason="Connect a pet first"
        />
        ...
        {Array.from({ length: PARTY_CAP - 1 }).map((_, i) => {
          ...
          const filter: InviteFilter = i === 1 ? 'agents' : 'pets';
          const label =
...
export default function ActivityLobbyModal({
  ...
  const handleQueue = useCallback(async () => {
    ...
    try {
      ...
      // Guest pet auto-create — visitors who hit /queue without going
      // through NPC mode (e.g. deep-link, quick-queue) get an automatic
      // guest pet here. This mirrors the NPC-mode bootstrap so the
  ...
  useEffect(() => {
    ...
    if (!activity || !pet) return;
    autoQueueFiredRef.current = true;
    onAutoQueueConsumed?.();
    void handleQueue();
  }, [autoQueue, activity, pet, handleQueue, onAutoQueueConsumed]);

  if (!activity) return null;

  const selfMember: PartySlotMember | null = pet
    ? {
        petId: pet.id,
        displayName: pet.name || 'You',
        ready: true,
        isSelf: true,
      }
    : null;
