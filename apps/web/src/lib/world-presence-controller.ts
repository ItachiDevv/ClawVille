...
export function resolveUplinkUrl(
  apiBaseUrl: string,
  roomId: string,
): string {
  const url = new URL(
...
export class WorldPresenceController {
  ...
  start(): void {
    ...
    this.#removePageLifecycleListener =
      this.#env.addPageLifecycleListener((event) =>
        this.#handlePageLifecycle(event),
      );
  }
      ...
      this.#dispatch(
        {
          ...
          canUpload:
            controlMode !== 'explore' && controlMode !== 'autonomous',
          hasFrozenPosition: this.#frozenPosition !== null,
          recoveryInFlight: this.#recoveryInFlight,
          poseChanged: activePose
            ? this.#activePoseChanged(activePose)
            : false,
  ...
  #dispatch(
    ...
    if (
      !previousState.httpFallbackTripped &&
      this.#state.httpFallbackTripped
    ) {
  ...
  async #join(recovery: boolean): Promise<JoinResult> {
  ...
  async #bootstrap(): Promise<void> {
    const result = await this.#join(false);
    if (this.#stopped) return;
  ...
  async #recoverWithTicket(): Promise<string | null> {
    ...
    try {
      const result = await this.#join(true);
  ...
  #openStream(roomId: string): void {
    ...
    source.addEventListener('snapshot', (event) => {
      if (source !== this.#eventSource || typeof event.data !== 'string') {
        return;
      }
      try {
    ...
    source.onerror = () => {
      ...
      const shouldEscalate =
        canRejoin && this.#lastStreamAttemptWasBareReopen;
      this.#streamRetryTimer = this.#env.setTimeout(() => {
        ...
        if (shouldEscalate) {
          ...
          void this.#recoverWithTicket().then((rejoinedRoomId) => {
            if (!this.#stopped && rejoinedRoomId === null) {
