...
export class TownScene extends Phaser.Scene {
  private pet!: Phaser.GameObjects.Rectangle & { body: Phaser.Physics.Arcade.Body };
  ...
  create(): void {
    ...
    // Create pet sprite as colored rectangle
    const { x, y } = store.petPosition;
    const rect = this.add.rectangle(x, y, 32, 32, 0xff6600);
    this.physics.add.existing(rect);
    this.pet = rect as typeof this.pet;
  ...
  update(): void {
    ...
    this.pet.body.setVelocity(vx, vy);

    // Update position in store
    store.setPetPosition(
      Math.round(this.pet.x),
      Math.round(this.pet.y)
    );
