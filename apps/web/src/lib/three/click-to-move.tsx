...
const SPEED = 200; // px/s — matches player-pet.tsx
...
// Building pixel zones for proximity detection (same as player-pet)
...
function ClickPlane() {
  ...
  const handleClick = useCallback(
    (e: THREE.Event & { stopPropagation?: () => void }) => {
      ...
      const { petPosition } = store;
      const path = findPath(petPosition.x, petPosition.y, pixel.x, pixel.y);
