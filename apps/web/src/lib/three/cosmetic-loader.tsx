 ...
 * existing pet/avatar mesh. Categories handled:
 *
...
export interface CosmeticLoaderProps {
  /** The pet's ID — used as the React Query cache key */
  petId: string;
  /**
...
/**
 * Subscribes to the pet's equipped cosmetics via React Query.
...
export function useEquippedCosmetics(
  ...
  // mutations propagate to the live render instantly. petId is kept on the
  // signature for forward-compat with multi-pet rendering (NPC cosmetics)
...
/**
 * Renders all equipped cosmetics for a pet onto its avatar.
 *
...
export function CosmeticLoader({
  ...
  const equipped = useEquippedCosmetics(petId, context, rigType);
   ...
   * register their cleanup function. The id is the pet_skins.id (unique per
