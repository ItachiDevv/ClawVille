 ...
 * FPSFollowCamera, ArenaNpcs, NpcController, and PlayerPet.
 *
 * R3F runs useFrame hooks in mount order. Hoisting the tick here guarantees
 * every consumer (camera, NPC mesh, player pet) reads the current frame's
...
export default function JumpTicker() {
  ...
  useFrame((_, delta) => {
    ...
    // so we can early-return here without leaving the pet stranded airborne.
