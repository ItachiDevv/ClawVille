 ...
 * PLAYER_GROUP_MAP module-scope Map<petId, THREE.Group>.
 */
...
function BumperShellsPlayerInner({
  ...
  useEffect(() => {
    ...
    PLAYER_GROUP_MAP.set(petId, group as any);

    return () => {
      PLAYER_GROUP_MAP.delete(petId);
    };
  }, [entity.petId]); // re-run if petId ever changes (shouldn't, but safe)
