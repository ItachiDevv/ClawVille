...
interface SpectatorCameraProps {
  ...
  /** petId of the entity to follow in 'follow' mode. null = auto-pick closest alive. */
...
function SpectatorCamera({ mode, targetPetId, entities }: SpectatorCameraProps) {
  ...
  useFrame((_, delta) => {
    ...
    if (mode === 'follow') {
      // 'follow': explicit target petId, or first alive entity as fallback.
      if (targetPetId) {
    ...
    if (hits) {
      ...
      if (len > _hitCheckScratch.lastHitCount) {
        for (let i = _hitCheckScratch.lastHitCount; i < len; i++) {
          ...
          // srcPetId = higher-velocity body (dealt the hit) → 'attack'
          // dstPetId = lower-velocity body (received the hit) → 'hurt'
          // Both fire alongside the squash/stretch VFX (triggerHit stays on
          // the player's own wasHit path; this is purely animator-side).
          if (h.srcPetId) {
            const srcGroup = PLAYER_GROUP_MAP.get(h.srcPetId);
            srcGroup?.triggerCombatAction?.('attack');
          }
          if (h.dstPetId) {
            const dstGroup = PLAYER_GROUP_MAP.get(h.dstPetId);
    ...
    if (elims) {
      ...
      if (len > _elimCheckScratch.lastElimCount) {
        for (let i = _elimCheckScratch.lastElimCount; i < len; i++) {
          ...
          const elimGroup = PLAYER_GROUP_MAP.get(e.petId);
...
export interface BumperShellsSceneProps {
  ...
  spectatorCamMode?: SpectatorCamMode;
  /**
   * PetId to follow in 'follow' mode. Ignored for 'free' and 'action'.
