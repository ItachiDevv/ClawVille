...
export const RIG_HEAD_OVERRIDE: Record<string, RigHeadOverride> = {
  ...
  // chibi: APPROXIMATE — its 'head' bone is DISPLACED ~35wu ABOVE the rendered
  // head mesh (the rig is broken), so no bone-anchored value looks right:
  // raising the hat floats it, lowering buries it in the black hair. NEEDS A
  // BLENDER RE-RIG (fix the head bone → head mesh association) before cosmetics
  // sit correctly. These values keep it near the head as a stopgap.
  chibi: { boneAnchored: true, headTopAboveBoneM: 0.0, headHeightM: 0.3, headWidthM: 0.6 },
};
