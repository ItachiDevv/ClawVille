  ...
  useFrame((state, delta) => {
    ...
    if (store.controlMode === 'player') {
      ...
      if (inputFwd !== 0 || inputRight !== 0) {
        ...
        // only when airborne (grounded walking stays pure XZ so the pet doesn't drift
  ...
  useFrame((state, delta) => {
    ...
    if (store.controlMode === 'player') {
      ...
      if (inputFwd !== 0 || inputRight !== 0) {
        ...
        // playerAltitude, but only when the pet is airborne (grounded walking
        // stays pure XZ so the pet doesn't drift upward just because the camera
        // is tilted).
        const camForwardY = _playerCamForward.y;
        _playerCamForward.y = 0;
        const fwdLen = _playerCamForward.length();
