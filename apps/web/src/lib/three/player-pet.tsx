...
function attachKeyListeners() {
  ...
  const onKeyDown = (e: KeyboardEvent) => {
    ...
    // Fixes pre-existing bug: typing W/A/S/D in pet chat moved the pet.
...
function PlayerPetVRMInner({ reg }: { reg: ModelRegistryEntry }) {
  ...
  // Load a fresh VRM instance for the player. Stable instanceId 'player-pet'
  // since only one player pet ever exists at a time. Per-instance loading
  ...
  const vrm = useVRMInstance(reg.path, 'player-pet');

  // Dispose this player-pet's instance when the avatar path changes or unmounts.
  useEffect(() => {
    return () => disposeVRMInstance(reg.path, 'player-pet');
  }, [reg.path]);
  ...
  useFrame((state, delta) => {
    ...
    if (store.controlMode === 'player') {
      ...
      // they ALSO drive altitude here when the pet is airborne.
      const airborne =
...
function PlayerPetGLBInner() {
  ...
  useFrame((state, delta) => {
    ...
    if (store.controlMode === 'player') {
      ...
      // pet moves in the direction the camera is facing. This mirrors
