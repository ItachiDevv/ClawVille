...
interface ProceduralState {
  /** Accumulated time in seconds, per petId. */
  t: number;
...
/** Module-scope state map — one entry per petId. No per-frame allocations. */
const _state = new Map<string, ProceduralState>();

function getState(petId: string): ProceduralState {
  let s = _state.get(petId);
  if (!s) {
...
/**
 * Probe whether `root` has any isBone nodes. Cached after first call per petId
 ...
 * @param petId      Stable id used to phase each entity independently.
...
/**
 * Reset the procedural state for a given petId. Call when the entity is
 * removed / remounted so the new clone starts from t=0 with fresh probe state.
 */
export function resetTransformSwimState(petId: string): void {
  _state.delete(petId);
}
