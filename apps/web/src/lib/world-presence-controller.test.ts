...
function createTestEnvironment(options: TestEnvironmentOptions = {}) {
  ...
  const lifecycleListeners = new Set<(event: PageLifecycleEvent) => void>();
  ...
  const fetchImpl = ((input: URL | RequestInfo, init?: RequestInit) => {
    ...
    } else if (url.endsWith("/api/world/position") && !options.holdPositions) {
...
function createHarness(options: TestEnvironmentOptions = {}) {
  ...
  return {
    ...
    setDownlinkEnabled(value: boolean) {
...
describe("world presence controller", () => {
  ...
  test("an in-flight POST held across promotion is discarded as 200 and 409", async () => {
    ...
    harness.avatar.x += 1;
    harness.advance(400);
    const held = callsEndingWith(harness.fetches, "/api/world/position").slice(
      0,
      2,
    );
  ...
  test("a control frame and its close dedupe into one rejoin", async () => {
    ...
    const recoveryBody = JSON.parse(
      String(
        callsEndingWith(harness.fetches, "/api/world/join")[1]!.init?.body,
      ),
    );
  ...
  test("stop during an in-flight join aborts and leaves", async () => {
    ...
    await flush();
    expect(callsEndingWith(harness.fetches, "/api/world/leave")).toHaveLength(
      1,
    );
  });
  ...
  test("cold activity keeps both downlink and uplink dark", async () => {
    const harness = createHarness();
    harness.setPolicy("remote");
    harness.setRemoteActivity("at-activity");
  ...
  test("activity dwell closes SSE while the WS uplink keeps at-activity cadence", async () => {
    const harness = createHarness();
    const socket = await startJoined(harness);
    ready(socket!);
    ...
    expect(callsEndingWith(harness.fetches, "/api/world/join")).toHaveLength(1);
    expect(
      socket!.sent.filter((raw) => JSON.parse(raw).activity === "at-activity")
        .length,
    ).toBeGreaterThanOrEqual(2);

    harness.setDownlinkEnabled(true);
    harness.advance(200);
    expect(harness.eventSources).toHaveLength(2);
  ...
  test("pending SSE reopen is cancelled after the failed source was nulled", async () => {
    const harness = createHarness();
    await startJoined(harness);
    const source = harness.eventSources[0]!;
    source.onerror?.();
  ...
  test("callbacks from a replaced SSE epoch cannot mutate or reopen", async () => {
    const harness = createHarness();
    await startJoined(harness);
    const stale = harness.eventSources[0]!;
  ...
  test("a hung recovery settles at 15s and releases the lease for retry", async () => {
    const harness = createHarness({
      wsAdvertised: false,
      holdJoins: true,
      holdPositions: true,
    ...
    harness.controller.start();
    harness.advance(200);
    const bootstrap = callsEndingWith(harness.fetches, "/api/world/join")[0]!;
    bootstrap.resolve(200, {
      roomId: "AB2C",
      id: "session-1",
      roomTicket: "ticket-1",
    });
    ...
    const position = callsEndingWith(
      harness.fetches,
      "/api/world/position",
    )[0]!;
    position.resolve(409);
    await flush();
    harness.advance(200);
    const recovery = callsEndingWith(harness.fetches, "/api/world/join")[1]!;
    expect(recovery).toBeDefined();

    harness.advance(15_000);
    await flush();
    expect(recovery.init?.signal?.aborted).toBe(true);
    harness.advance(20_000);
    expect(callsEndingWith(harness.fetches, "/api/world/join")).toHaveLength(3);
    harness.controller.stop();
  });
  ...
  test("policy is read at tick time", async () => {
    ...
    expect(
      socket.sent.some((raw) => JSON.parse(raw).activity === "at-kelp"),
    ).toBe(true);
    harness.controller.stop();
  });
});
