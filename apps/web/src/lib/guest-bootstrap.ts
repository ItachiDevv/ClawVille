/**
 * Guest pet auto-create bootstrap helper (2026-04-23).
 *
 * Two entry points trigger this:
 *   A) `setControlMode('npc')` in stores/game.ts — visitor switches into
 *      NPC mode but has no pet yet, so we silently mint a guest pet so
 ...
 * we never accidentally mint two guest pets for the same visitor (e.g.
 ...
 *
 * The pet provider invalidation is the caller's responsibility (because
...
/**
 * Ensure the visitor has a Lucia session + pet. If they already do
 * (cookie present), the server returns the existing pair and `reused`
 * is true. If not, mints a guest user + guest pet and sets a session
 * cookie.
 *
