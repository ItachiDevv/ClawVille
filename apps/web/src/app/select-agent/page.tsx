    ...
    >
      ...
      >
        ...
        <span className="select-none">
          {AVATAR_SPECIES.find((s) => s.id === agent.species)?.emoji ?? '?'}
        </span>
      </div>
...
function AgentCreationForm({
  ...
  const [species, setSpecies] = useState(AVATAR_SPECIES[0]!.id);
  const [color, setColor] = useState(AVATAR_COLORS[0]!.id);
  const [gender, setGender] = useState('male');
  const [archetype, setArchetype] = useState(AVATAR_ARCHETYPES[0]!.id);
            ...
            {selectedAgent ? (
              <>
                ...
                <div className="relative">
                  ...
                  >
                    {AVATAR_SPECIES.find((s) => s.id === selectedAgent.species)?.emoji ?? '?'}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 mt-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/80 flex items-center gap-1.5">
                      ...
                      />
                      {AVATAR_COLORS.find((c) => c.id === selectedAgent.color)?.name ?? selectedAgent.color}
                    </p>
                  </div>
                  <div>
