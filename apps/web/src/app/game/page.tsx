...
export default function GamePage() {
  ...
  // queue with a default lobster pet auto-provisioned by the queue route,
  ...
  return (
    <div className="game-container">
      ...
      {/* World UI that's useful for ALL pet-bearing visitors — including
          ...
          guest pets (isGuest carve-out in chat.ts). */}
      {hasPet && (
        <>
          <LocationHUD />
          <TutorialOverlay />
      ...
      {/* Player-mode (agent-connected) UI — hidden in NPC/Explore mode.
          ...
          agent-only UI. (Chat moved up into the hasPet block.) */}
      {agentConnected && controlMode !== 'npc' && controlMode !== 'explore' && (
        <>
          ...
          <PetSettingsModal />
          <LocationConfigModal />
          {/* PetChatBar — restored to agent-connected-only gate (Phase 6.2).
              The Phase 6.1 move into hasPet leaked it into Explore mode
              and created Eliza state for the user's pet during NPC mode,
