 ...
 *   - REMOTE: POST /api/casino/slots/verify — pure-compute replay on the
...
import { runSpinLocal, sha256Hex } from '@/lib/casino/verifier';
...
export default function CasinoVerifyPage() {
  ...
  const handleVerify = useCallback(async () => {
    ...
    try {
      ...
      const message =
        err instanceof CasinoApiError ? describeCasinoError(err) : (err as Error).message;
      setErrorMsg(message);
    }
  }, [form, remote]);
    ...
    >
      <div style={{ maxWidth: 920, margin: '0 auto' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: 13, lineHeight: 1.6, marginTop: 12 }}>
          ...
          no rounding gap. This is the proof that the casino can't bias outcomes.
        </p>
        ...
        >
          ...
          />
          <Link
            href={`/casino/verify/${deeplink.trim()}`}
            style={{
              padding: '8px 14px',
              borderRadius: 8,
