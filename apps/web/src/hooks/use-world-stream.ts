...
export function useWorldStream(
  ...
  const updateNpcsFromSnapshot = useNpcStore(
    ...
    const controller = new WorldPresenceController({
      ...
      callbacks: {
        ...
        addToast: (icon, message, durationMs) => {
          try {
            useGameStore.getState().addToast(icon, message, durationMs);
          } catch {
            // Best effort.
          }
        },
        ...
        readDownlinkEnabled: () => downlinkEnabledRef.current,
        readControlMode: () => useGameStore.getState().controlMode,
        readAvatarPosition: () => avatarPositionRef,
      },
    });
    const probeWindow = window as typeof window & {
      __WORLD_PRESENCE_CONTROLLER__?: WorldPresenceController;
    };
    if (process.env.NEXT_PUBLIC_ENABLE_STAGE_PROBE === "1") {
      probeWindow.__WORLD_PRESENCE_CONTROLLER__ = controller;
    }
    controller.start();
    return () => {
      if (probeWindow.__WORLD_PRESENCE_CONTROLLER__ === controller) {
