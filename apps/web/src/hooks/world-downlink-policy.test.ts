...
import { createWorldStreamMachineState, decide } from "./world-stream-machine";

const hookSource = await Bun.file(
  `${import.meta.dir}/use-world-stream.ts`,
).text();
const controllerSource = await Bun.file(
  `${import.meta.dir}/../lib/world-presence-controller.ts`,
).text();
// The frozen suite predates the staging extraction from an effect closure into
// a class. Normalize only syntax/field spelling so its structural assertions
// continue to pin the same ownership semantics in the controller that now owns
// them; the pure policy behavior assertions below remain unchanged.
const streamSource = controllerSource
  .replaceAll(
    "this.#callbacks.readDownlinkEnabled()",
    "downlinkEnabledRef.current",
  )
  .replaceAll("this.#env.setTimeout", "setTimeout")
  .replaceAll("this.#env.clearTimeout", "clearTimeout")
  .replaceAll("this.#env.now()", "Date.now()")
  .replace(/^  async #(\w+)\(/gm, "  async function $1(")
  .replace(/^  #(\w+)\(/gm, "  function $1(")
  .replaceAll("this.#", "")
  .replaceAll("#", "")
  .replaceAll("retryTokenSequence", "retryTokenSeq")
  .replaceAll("recoveryLeaseSequence", "recoveryLeaseSeq")
  .replaceAll("eventSource", "es")
  .replaceAll("streamRetryTimer", "retryTimeout")
  .replaceAll('"', "'");
...
describe("decideWorldDownlink", () => {
  test("closes an open source when the downlink is not wanted", () => {
    expect(decideWorldDownlink({ ...connected, wanted: false })).toBe("CLOSE");
  });

  test("opens a connected room when the downlink is wanted", () => {
    expect(decideWorldDownlink({ ...connected, open: false })).toBe("OPEN");
  });
...
describe("world downlink integration contract", () => {
  ...
  test("ordinary enabled error arms one delayed replacement", () => {
    expect(streamSource).toMatch(
      /source\.onerror[\s\S]*?dropFailedSource\(source\)[\s\S]*?armRetry\(roomId, delay, shouldEscalate, null, epoch\)/,
    );
  });
  ...
  test.each([
    ...
    [
      "rejoin replacement",
      /function settleRecovery[\s\S]*?invalidateStream\(\)/,
    ],
    ["supersession", /function handleSuperseded[\s\S]*?invalidateStream\(\)/],
    [
      "teardown",
      /stop\(\): void[\s\S]*?stopped = true[\s\S]*?invalidateStream\(\)/,
    ],
  ...
  test("disable then re-enable success leaves recovery as sole opener", () => {
    expect(streamSource).toMatch(
      /const joined = outcome\.value;[\s\S]*?if \(downlinkEnabledRef\.current\) \{\s*openStream\(joined\.roomId\)/,
    );
    expect(
      decideWorldDownlink({
        ...connected,
  ...
  test("armed escalation cannot bare-open beneath a 409 recovery", () => {
    expect(streamSource).toMatch(
      /if \(recoveryInFlight\) \{[\s\S]*?armRetry\([\s\S]*?roomId,[\s\S]*?delayMs,[\s\S]*?shouldEscalate,[\s\S]*?since,[\s\S]*?lineageEpoch,[\s\S]*?\);[\s\S]*?return;/,
    );
  });
  ...
  test("recovery success owns exactly one open site", () => {
    expect(streamSource).toMatch(
      /settleRecovery[\s\S]*?invalidateStream\(\);[\s\S]*?openStream\(joined\.roomId\)/,
    );
  });

  test("recovery failure permits exactly one fallback path", () => {
    const continuation = streamSource.slice(
      streamSource.indexOf("void recoverWithTicket().then"),
      streamSource.indexOf("function dropFailedSource"),
    );
    expect(continuation).toContain("rejoinedRoomId !== null) return;");
    expect(continuation.split("lastStreamAttemptWasBareReopen = true;").length - 1).toBe(1);
    expect(continuation.split("openStream(roomId);").length - 1).toBe(1);
  });
  ...
  test("deadline does not depend on the fetch honoring abort", () => {
    ...
    expect(streamSource).toContain("abort.abort()");
  ...
  test("a refused late success cannot reach source construction", () => {
    ...
    const open = streamSource.indexOf("openStream(joined.roomId)", guard);
  ...
  test("operation-first settlement clears its deadline timer", () => {
    expect(streamSource.match(/clearTimeout\(deadlineTimer\)/g)).toHaveLength(
      2,
    );
  });
  ...
  test("join deadline is strictly below retry retirement ceiling", () => {
    const join = Number(
      streamSource.match(/JOIN_TIMEOUT_MS = ([\d_]+)/)?.[1].replace("_", ""),
    );
    const ceiling = Number(
      streamSource
        .match(/RECOVERY_WAIT_CEILING_MS = ([\d_]+)/)?.[1]
        .replace("_", ""),
    );
  ...
  test.each([
    [
      "hung bootstrap aborts and returns failure",
      /joinBounded[\s\S]*?abort\.abort\(\);\s*return \{ kind: 'failed' \}/,
    ],
    [
      "late bootstrap result loses the completed race",
      /Promise\.race\(\[[\s\S]*?operation\.then/,
    ],
  ...
  test("bootstrap while disabled still writes membership without opening", () => {
    expect(streamSource).toMatch(
      /sessionId = joined\.id;[\s\S]*?callbacks\.setRoomId\(joined\.roomId\);[\s\S]*?if \(downlinkEnabledRef\.current\)/,
    );
  });

  test("rejoin while disabled repairs membership without opening", () => {
    expect(streamSource).toMatch(
      /const joined = outcome\.value;[\s\S]*?sessionId = joined\.id;[\s\S]*?invalidateStream\(\);[\s\S]*?if \(downlinkEnabledRef\.current\)/,
    );
  });

  test("OPEN edge invalidates land exactly at resume", () => {
    expect(streamSource).toMatch(
      /downlinkAction === 'OPEN'[\s\S]*?openStream\(roomId!\);[\s\S]*?callbacks\.invalidateLandQuery\(\)/,
    );
  });

  test.each([
    [
      "persisted pageshow resets membership",
      /if \(event\.persisted && pageHiddenPersisted\)[\s\S]*?sessionId = null/,
    ],
    [
      "non-persisted pageshow preserves membership and only reopens",
      /function handlePageLifecycle[\s\S]*?if \(event\.persisted && pageHiddenPersisted\)[\s\S]*?if \([\s\S]*?sessionId &&[\s\S]*?roomId &&[\s\S]*?!es[\s\S]*?openStream\(roomId\)/,
    ],
  ...
  test("unmount while suspended follows one normal cleanup path", () => {
    const cleanupBlocks = hookSource.match(
      /return \(\) => \{[\s\S]*?controller\.stop\(\);[\s\S]*?\};/g,
    );
    expect(cleanupBlocks).toHaveLength(1);
    expect(cleanupBlocks?.[0].match(/controller\.stop\(\)/g)).toHaveLength(1);
    expect(cleanupBlocks?.[0]).toContain(
      "delete probeWindow.__WORLD_PRESENCE_CONTROLLER__",
    );
    expect(streamSource).toMatch(/stop\(\): void[\s\S]*?sendLeave\(\)/);
  });
