...
SSH `ssh -i ~/.ssh/clawville_deploy root@87.99.142.34` · container restart `docker restart <name>` (find via `docker ps`) · Coolify UI `https://coolify.clawville.world` · logs `docker logs --tail 200 <name>` · DB query `docker exec coolify-db psql -U coolify -d coolify -c "<sql>"` · full playbook `docs/DEPLOY-HETZNER.md`.
