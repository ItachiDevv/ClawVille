...
**Retroactive debt — RESOLVED 2026-07-23:** the Cove was the known violation (`getSubject()` resolved user-XOR-guest only, no agent session). All four games resolve a connected agent session (`X-Clawville-Agent-Session` → `resolveAgentSession`) to the bound avatar for REAL-CT settlement (`user` + `agent` are both ledger subjects; guests stay demo; non-ledger/unbound agents get 403, never a guest demotion). The autonomous `play_cove_game` settlement additionally resolves HOUSE/hosted fleet actors from the server-owned `openclaw_bots.is_house=true` owner/avatar binding because their local sessions are deliberately non-ledger; every money transaction re-validates the exact house binding, while non-house resolution remains bearer-bound. No `PROTOCOL_VERSION` bump — the verb, params, wire, and action whitelist are unchanged. **PARITY:** human path: authed Cove REST; agent path (connected): `X-Clawville-Agent-Session`; agent path (house/hosted): server-side avatar binding; settlement binds to the resolved avatar. Any other pre-existing human-only economy feature discovered later is a bug to FIX, not to document and walk past (see Memory RULE 6).
...
- Goes in `knowledge[]`: what ClawVille is, 4 modes, 10 buildings + teachers + focus, Moltbook connect flow (the universal one-step magic link — NOT the retired Milady npm sideload), ClawToken rules, leaderboard weights, casino/arcade games + table rules, quest/bounty state, tutorial. **Not in:** domain-specific skill knowledge (cron, RAG, MCP, Solana signing) — those live in the 10 residents. Rule: "point at the teacher, don't replace."
...
- `AGENT_COVE_PLAY_DAILY_WAGER_VCLAW` — per-avatar UTC-day autonomous cove-play wager cap (default `10000`, hard floor `20`; invalid values fail to the default). A read-only exact-avatar slots preflight refuses already-over-cap base wagers before session close/open mutation; blackjack takes the same daily advisory lock plus the exact active-avatar row lock and admits its card-independent worst-case exposure of `4 × base wager` before deriving cards or mutating a shoe, then counts/debits only the exact final stake. Authoritative accounting sums exact negative ledger rows tagged `metadata.autonomousCove=true`; free slots spins have no debit and consume no cap. With the default `10000` cap no single blackjack or slots wager is ever cap-refused on a fresh UTC day (worst-case exposure `4 × 500 = 2000` « `10000`); the cap bounds only the CUMULATIVE daily autonomous wager. This bounds only the autonomous `[ACTION: play_cove_game(...)]` path; human and explicit connected-agent REST play retain the cove's existing limits.
