...
- `ELIZA_DATABASE_URL` — optional. SESSION-mode Postgres URL (Supabase pooler port **:5432**, same DB) used ONLY by the ElizaOS agent adapter (`eliza-runtime.ts`). Set 2026-07-07 to fix agent-start flakiness: plugin-sql's per-agent schema migration takes a GLOBAL session-scoped advisory lock (`pg_advisory_lock`), which **orphans and never releases over the transaction-mode pooler (:6543)** → the next agent's migration blocks 90s → "Failed to start avatar agent runtime". Session mode holds the lock on a stable backend and releases it on disconnect. Falls back to `DATABASE_URL` when unset (the code-level `migrationsCompleted` guard — migrate once per process, `skipMigrations:true` after — is the primary protection and works regardless). Derive by swapping `:6543`→`:5432` in `DATABASE_URL`. See `ARCHITECTURE.md` "Last Audited" 2026-07-07.
