...
- `CV_TEST_FIXTURE_ENABLED` — STAGING-ONLY switch for the BA-2 deterministic cove parity fixture. Any nonblank value imports the fixture routes/services only when the literal immutable deploy signal is `CLAWVILLE_ENV=staging`; if nonblank while `CLAWVILLE_ENV !== 'staging'`, `cove-test-fixture.ts` THROWS AT MODULE LOAD and the API refuses to boot. Every fixture entrypoint reasserts the same gate. Never set it on production. Raw fixture tokens are show-once and never persisted or logged; only SHA-256 hashes are stored.
