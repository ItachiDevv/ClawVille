...
|---|---|---|
| **`GameFeatures.md`** | Gameplay functionality | Game modes, agent connect flow, marketplace, skill economy, quests, bounties, daily login, avatar system, tutorial, UI components, control-mode toggle, NPC simulation, talk-to-character, Phase 5 magic-link, Phase 6 memory isolation, landing-page surfaces |
...
- `avatars` (one per user) — key columns:
...
- `POST /api/agent/export-character` — **Phase 3** emits a Milady-installable bundle for a avatar the caller owns: `{character, skillPack, miladyInstallPayload, installCommand, exportedAt, summary}`. Accepts `{avatarId, targetHarness?, miladyBaseUrl?}`. `character.knowledge` is intentionally empty (ElizaOS v2 normalizes knowledge strings as filesystem paths, so the skill pack is the authoritative RAG carrier). Phase 4a UI wraps this with the one-click install button.
