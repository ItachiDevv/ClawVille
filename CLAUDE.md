...
- Dynamic context via `processMessage(dynamicContext)` prepended to prompt. Avatar chat injects token balance + knowledge count + NPC world state. Location chat injects visitor info + shop items + OpenClaw theme; awards +1 token per message.
...
- **Archetypes** — 14 in `avatar-archetypes.ts` (id, label, description, tone, bio[], lore[], knowledge[], topics[], adjectives[], style, messageExamples, greeting, rules[]). `avatars.archetype` varchar; `characterConfig` JSONB stores resolved.
...
- **Outbound** — `POST /api/portal/scape` (Lucia-authed). Signs `sha256(canonical-JSON body)` with service issuer SK, POSTs to `SCAPE_HOSTED_SESSION_URL` with `X-Clawville-Issuer-Pubkey` + `X-Clawville-Signature`. First crossing auto-provisions 'scape account + character: `principalId = principal:clawville:<user.id>`, `worldCharacterId = cv-<avatar.id>`, display `<avatar.name>-cv`.
