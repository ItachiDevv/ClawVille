...
- **Phaser ↔ React bridge**: Zustand store (`stores/game.ts`). Phaser writes `avatarPosition` and `nearLocation`; React reads `currentLocation` and renders ChatPanel overlay.
...
- `avatars` (one per user, species/color/gender/personality/stats/position)
