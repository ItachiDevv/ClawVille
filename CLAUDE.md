...
- `avatars` (one per user) — identity: `species`/`color`/`gender`/`archetype`/`personality`/`stats`. Phase 2 framework fields: `model_key` (3D GLB key, default `lobster`), `agent_category` (`openclaw`/`hermes`/`milady`/`other`, default `openclaw`), `harness` (`openclaw`/`hermes`/`milady`/`custom`, default `milady`) — all NOT NULL with DEFAULTs so existing rows backfill automatically. Also: `position_x/y`, `claw_tokens`, `login_streak`, `level`, `xp`, `total_xp`, `equipped_skills`, `wallet_address`, `avatar_type`/`avatar_url`/`vrm_metadata`, `platform_agent_id`
