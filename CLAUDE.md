...
- **Tokenomics env vars (added 2026-07-07/08, economy v1 — full detail `.env.example` + `ARCHITECTURE.md` Tokenomics entries):**
  ...
  - `CLV_SWAP_EXECUTE` — **MUST NEVER be 'true'** (Codex-gated seam): live CLV swap execution refuses at MODULE LOAD (a flagged box won't boot, crash-loud like `FINGERPRINT_SECRET`). The swap executor is DRY-RUN only. The GoLive-executors slice (2026-07-07) added the FULLY-PLUMBED-BUT-DARK live path (`clv-swap-custody.ts` + `clv-swap-live.ts`: funding sweep merchant→swap-wallet, atomic claim, per-clip Jupiter USDC→CLV) — every live entrypoint re-asserts this gate default-OFF AND a mainnet/real-facilitator network guard (devnet/mock can NEVER reach a real send); nothing is wired into index.ts boot. Opening the seam = remove the module-load throw (one-line Codex-reviewed change).
  - `CLV_SWAP_SLIPPAGE_BPS` (default 100, floor 1, cap 1000) / `CLV_SWAP_JUPITER_BASE_URL` (default keyless lite-api.jup.ag) — DARK live-path knobs, read only behind the `CLV_SWAP_EXECUTE` seam; slippage drives BOTH the Jupiter quote AND the oracle-derived on-chain min-out floor.
