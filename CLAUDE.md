...
- `AGENT_COVE_PLAY_DAILY_WAGER_VCLAW` — per-avatar UTC-day autonomous cove-play wager cap (default `1000`, hard floor `20`; invalid values fail to the default). A read-only exact-avatar preflight refuses already-over-cap base wagers before session close/open mutation; authoritative admission remains race-safe under a transaction advisory lock and counts exact negative ledger rows tagged `metadata.autonomousCove=true`. Free spins have no debit and consume no cap. This bounds only the autonomous `[ACTION: play_cove_game(...)]` path; human and explicit connected-agent REST play retain the cove's existing limits.
