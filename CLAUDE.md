...
- `USDC_BOUNTY_REWARD_MIN` — USDC-rail bounty reward FLOOR (whole USDC). Default 10; clamped to integer ≥ 1 (`resolveUsdcBountyRewardMin` in `routes/bounties.ts`, enforced per-rail in `createBountySchema`'s superRefine). Set `1` on the mainnet smoke box so a funded 1-USDC bounty runs without an 11-USDC top-up. USDC rail ONLY — the CT rail floor stays 10.
