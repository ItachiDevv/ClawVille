...
- **Double-queued builds = duplicate GitHub webhook (RULE, 2026-06-10):** if every push creates TWO `ApplicationDeploymentQueue` rows per app (~10s apart, same commit), check `gh api repos/ItachiDevv/ClawVille/hooks` — the repo had the same Coolify webhook registered twice (every push delivered 2×, every deploy built 2×). Keep EXACTLY ONE hook per Coolify endpoint; delete extras via `gh api -X DELETE repos/ItachiDevv/ClawVille/hooks/<id>`. Check this FIRST whenever builds double-queue.
