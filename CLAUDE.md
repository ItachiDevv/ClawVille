...
- `AGENT_COVE_PLAY_DAILY_WAGER_VCLAW` — per-avatar UTC-day autonomous cove-play wager cap (default `1000`, hard floor `20`; invalid values fail to the default). A read-only exact-avatar slots preflight refuses already-over-cap base wagers before session close/open mutation; blackjack takes the same daily advisory lock plus the exact active-avatar row lock and admits its card-independent worst-case exposure of `4 × base wager` before deriving cards or mutating a shoe, then counts/debits only the exact final stake. Authoritative accounting sums exact negative ledger rows tagged `metadata.autonomousCove=true`; free slots spins have no debit and consume no cap. With the default cap, an autonomous blackjack base wager above 250 is conservatively refused even though the per-game bound remains 500. This bounds only the autonomous `[ACTION: play_cove_game(...)]` path; human and explicit connected-agent REST play retain the cove's existing limits.
