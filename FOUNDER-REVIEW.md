...
### Slot machine screens no longer show a black triangle (on prod via #292, cache fix following)
- **What:** every slot machine screen in the cove had a big black triangle over
  ...
  same fix.
- **Where:** clawville.world → /cove → look at the slot rows from the door and
  walk along them. If you still see triangles, hard-refresh once (a cache fix
  for returning visitors ships right after #292).
