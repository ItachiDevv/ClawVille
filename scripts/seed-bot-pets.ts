/**
 * Q2 Activity Portals — seed the reserved bot-pet pool used by the
 * activity matchmaker for backfill (chunk #10).
 *
 * Why pre-seeded rows?
 *   `activity_room_participants.pet_id` is `uuid NOT NULL REFERENCES
 *   pets(id)` — the schema explicitly says "pet_id MUST exist in pets
 *   even for bots". Rather than relax the FK or add a parallel `bot_id`
 *   column, we pre-create N pet rows owned by N system bot users and
 ...
 *   4. `bun run scripts/seed-bot-pets.ts`   ← this script
 *
...
function botPetName(index: number): string {
  return `${BOT_PET_NAME_PREFIX}${String(index).padStart(3, '0')}`;
}

async function seed(): Promise<void> {
  console.log(`Seeding ${BOT_POOL_CAPACITY} bot users + pets…`);
  ...
  for (let i = 1; i <= BOT_POOL_CAPACITY; i++) {
    ...
    // 2. Upsert the bot pet (one per user — pets.userId is unique).
    ...
    await db.insert(pets).values({
      ...
      // only exist as DB rows so `activity_room_participants.pet_id` FK
  ...
  console.log(
    `Done! Users: ${usersCreated} created, ${usersExisting} existed. Pets: ${petsCreated} created, ${petsExisting} existed.`,
  );
...
seed().catch((err) => {
  console.error('Bot pet seed failed:', err);
  process.exit(1);
});
