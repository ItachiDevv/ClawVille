/**
 * Phase 6.1 — Monte Carlo RTP simulator (slice 4; bonus support 6.1.5).
 *
 * Drives `runSpin()` from the apps/api slot engine + `CLASSIC_PAYTABLE`
 * (or `CLASSIC_BONUS_PAYTABLE` via `--paytable`) for N spins and reports:
 *
 ...
 *   - Wall-clock time
 *
 * For the bonus paytable, base spins consume real predict and award free
 * spins on a 3+ scatter trigger. The simulator drains the free-spin
 * budget after each base spin (with retriggers compounding capped at
 * `FREE_SPIN_RULES.CAP_REMAINING`). Free-spin wins are NOT charged to
 * `totalWagered` (FS is free) — combined RTP can exceed 100% in
 * principle; the CI gate band [96.5%, 99.5%] keeps it within sanity.
 *
 ...
 *
 * Used by:
 *   - `.github/workflows/rtp-gate.yml` — 100k-spin CI gate (BOTH paytables).
 *   - Local 1M-spin tuning runs while iterating reel strip density.
 *   - One-shot acceptance check in the slice-4 / 6.1.5 ship reports.
 *
 * CLI:
 *   bun scripts/casino/rtp-sim.ts \
 *     [--paytable classic-3x5|classic-3x5-bonus] \
...
function printHelpAndExit(): never {
  console.log(`Monte Carlo RTP simulator for slot paytables.
...
interface SimResult {
  ...
  // tracks payout earned WHILE in FS mode; freeSpinPseudoWagered tracks the
...
function runSimulation(opts: CliOptions, serverSeed: string): SimResult {
  ...
  for (let i = 0; i < opts.spins; i++) {
    ...
    accumulateSpin(spin, opts.predict, predictFloat, result);
    ...
    while (freeSpinsRemaining > 0) {
      ...
      result.freeSpinPseudoWagered += opts.predict;
      ...
      accumulateSpin(fs, opts.predict, predictFloat, result);
      freeSpinsRemaining--;
...
function formatReport(opts: CliOptions, serverSeed: string, r: SimResult): string {
  ...
  // 95% CI half-width — see approximateRtpCi() for derivation.
...
function main(): void {
  ...
  process.stderr.write(
    `Starting Monte Carlo: ${opts.spins.toLocaleString()} BASE spins, predict=${opts.predict}, paytable=${opts.paytable}\n`,
  );
