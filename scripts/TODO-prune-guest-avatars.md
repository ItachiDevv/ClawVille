# TODO — `scripts/prune-guest-avatars.ts`

Deferred from the guest-avatar auto-create PR (2026-04-23) so the play-now
...
`avatars.userId` has `ON DELETE CASCADE` so a single `DELETE FROM users
