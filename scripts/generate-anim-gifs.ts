...
async function main() {
  const sheetPath = path.join(process.cwd(), `apps/web/public/sprites/avatars/${SPECIES}-sheet.png`);
  const outDir = path.join(process.cwd(), 'apps/web/public/sprites/avatars/gifs/anims');
...
${ANIMS.map(a => `<div class="card"><img src="${a.name}.gif"><div class="name">${a.name}</div><div class="desc">${a.label}</div></div>`).join('\n')}
  ...
  console.log(`\nDone! ${ANIMS.length} GIFs saved to apps/web/public/sprites/avatars/gifs/anims/`);
}
