/**
 * One-time backfill: generate a custodial Solana wallet for every pet AND
 ...
 *   bun run scripts/backfill-wallets.ts --apply --only=pet    # pets only
...
async function main() {
  ...
  const doPets = !onlyArg || onlyArg === 'pet';
  ...
  console.log(`[backfill] mode=${apply ? 'APPLY' : 'DRY-RUN'}  pets=${doPets}  agents=${doAgents}`);
  console.log('');
  ...
  if (doPets) {
    ...
    console.log(`[backfill] Pets without a wallet: ${petTargets.length}`);
  }
  ...
  for (const p of petTargets) {
    try {
      const result = await ensureWallet('pet', p.id);
      if (result.alreadyExisted) alreadyHad += 1;
      else {
        generated += 1;
        console.log(`  ✓ pet   ${p.id}  ${p.name}  → ${result.publicKey}`);
      }
    } catch (err) {
      failed += 1;
      console.error(`  ✗ pet   ${p.id}  ${p.name}:`, err instanceof Error ? err.message : err);
    }
  }
