/**
 * One-time backfill: generate a custodial Solana wallet for every avatar that
 * doesn't have one yet. Safe to run multiple times — ensurePetWallet() is
 * idempotent per avatar_id.
 *
 * Usage:
 *   bun run scripts/backfill-avatar-wallets.ts         # dry run, shows count
 *   bun run scripts/backfill-avatar-wallets.ts --apply # actually generates
 *
...
import { ensurePetWallet } from '../apps/api/src/services/avatar-wallet-service';
...
async function main() {
  ...
  console.log(`[backfill] Found ${targets.length} avatars without a wallet.`);
