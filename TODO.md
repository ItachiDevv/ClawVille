...
### ⭐ FOUNDER QUEUE — decisions + sign-offs blocking "done" (keep at top; P0+P1+P2+P3s1+P3s2+P3s4 all live-proven on staging (P3s3 built+approved, cognition legs blocked on the OpenAI quota incident), Rule E4 gates open)
...
- [ ] **🚨 INCIDENT 2026-07-05: OpenAI account OUT OF QUOTA — PROD cognition failing** (31 errs/hr on prod api: teacher chats, Coralia autonomy, embeddings; staging identical). Billing top-up at platform.openai.com is a FOUNDER action. Also blocks the P3 slice-3 live gate (cognition legs). Context: tonight's 3-slice testing + Coralia's continuous decide loop + the D8 embed-cost tradeoff all draw on this key — consider a usage alert/limit split per env.
...
- [ ] **Guest-account economy semantics** — model doc §2 says guest economy is "DEMO — tokens not real", but is_guest accounts earn real soft CT by shipped deliberate design (activities "the dopamine works", chat +1 CT/turn, daily-login, 100 CT/mint). Sanction as logged-in-lite (amend doc) or gate the earn paths (quests.ts:1229 template). Cannot be defaulted.
...
- [ ] **Chibi harness-pool asymmetry (product call)** — create accepts chibi for milady-harness; the /me/appearance swap guard forbids returning to it. Relax or document.
