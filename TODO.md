...
- Server: Hetzner CCX13 at 87.99.142.34, orchestrated by Coolify at https://coolify.clawville.world
...
Coolify VPS (87.99.142.34) hit 100% disk on 2026-04-16 from accumulated Docker images + build cache during rapid iteration. Pruned to 71% used / 22 GB free.
...
- [ ] **Weekly prune cron** (Sunday 3 AM):
  ```bash
  echo '0 3 * * 0 docker system prune -af --filter "until=168h" >/dev/null 2>&1' | ssh root@87.99.142.34 "tee -a /etc/cron.d/clawville-prune"
  ```
...
- [ ] **Verify DOM element IDs** used by the plugin's viewer bootstrap script (`#landing-overlay`, `#auth-modal`, `#login-overlay`, `#create-pet-overlay`) match what ClawVille's frontend actually renders — the bootstrap was written defensively without verifying.
