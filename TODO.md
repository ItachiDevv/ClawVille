...
- [ ] **Browser sign-off, P1+P2 (+P3 slices 1-2, 4) in one staging pass:** P3s4 adds: Land Office → Services tab — list a service on your shop, buy someone else's (real CT moves; selling to distinct customers now earns the top land leaderboard points) · P3s2 adds: toggle Autonomous → type a directive in the chat bar ("go to Squidward's House") → watch your avatar walk there; clear via the Directing ✕ (phone-portrait tap fixed) · P3s1 adds (no browser surface — evidence is `streamflow-e2e.ts` 24/24 on `ee8badb7`: agent settle → live SSE settlement frame + durable replay cursor + Last-Event-ID resume, zero loss/leak) · watch Coralia walk + converse · click a magic-link control handback · fresh email signup → wallet-backup modal on /game → /create-agent prefills (customize mode) → first chat answers in persona (no "reconnect" 404) → toggle shows Controlled/Autonomous · guest NPC mode unaffected · mobile/iPad glance at the amber pending banner + "Finish Agent Setup" sidebar row.
- [ ] **🚨 OpenAI quota incident — FOUNDER DIRECTION 2026-07-05: pivot cognition to a LOCAL model on a separate local PC (OpenAI-compat endpoint) instead of topping up.** ENGINEERING when the box is ready: wire the text provider to the local base URL (staging first). CAVEAT: EMBEDDINGS are code-pinned to text-embedding-3-small/1536 (deliberate, not env-overridable) — a local text model doesn't cover them; either the local stack serves an embeddings-compatible API, embeddings keep a (funded) OpenAI key, or we accept keyword-fallback for earned-skill memory until a re-embed migration is decided. Slice-3 cognition e2e legs + slice 6 run after the endpoint is live.
- [ ] **D2 board-policy conflict** — house/fleet agents currently EXCLUDED from the public board (safe default) vs D2's "internal-only management policy". One-line flip; options in `docs/agent-metaverse-model.md` §8 D2 note.
- [ ] **Guest-account economy — DECIDED (founder 2026-07-05): ALL FAKE + ON-SCREEN DEMO LABEL.** Guest (is_guest) accounts must earn DEMO tokens only, with the UI telling the player they're demo. ENGINEERING: gate the guest earn paths (chat +1/turn, activities, daily-login, the 100-CT mint) to a demo balance, add the on-screen demo indicator, and amend model doc §2 + GameFeatures. Companion question RESOLVED (founder 2026-07-10: YES — fully exclude is_guest accounts from the public leaderboard, NOT grandfathered). Implemented on branch `fix/guest-leaderboard-leak` (durable `events.subject_was_guest` stamp + NULL-scoped live flag-join in `buildAgentSnapshot`, both legs).
- [x] **Guests CAN rank on the GLOBAL leaderboard** (was true; verified 2026-07-04 — no is_guest SQL existed). FIXED 2026-07-10 per founder YES — guests now fully excluded from `buildAgentSnapshot` (durable event-time stamp + flag-join backstop). See ARCHITECTURE.md §5b.
- [ ] **Unverified-email provisioning default** — P2 ships provision-immediately (matches unauth POST /api/avatars precedent; 5/min/IP caps abuse). Flip = gate on `emailVerified`.
- [ ] **Wallet-secret loss window (UX call)** — signup's one-time wallet secret rides sessionStorage through the whole /create-agent wizard before FirstTimeBackupModal shows on /game (tab-close mid-wizard = lost, never re-emitted). Render the modal on /create-agent too, or add a persistent "backup pending" nudge.
- [ ] **Chibi — DECIDED (founder 2026-07-05): chibi agents are agents like any other; same function everywhere.** ENGINEERING: relax the /me/appearance swap guard so chibi is selectable/returnable like every other model (investigate why the guard excluded it — if a render/pipeline constraint exists, fix THAT, don't gate the choice).

### 0. Agent-metaverse P1+P2 engineering follow-ups (founder items moved to the queue above)
P1 live-proven `0b6a29a4` (2026-07-03): `docs/agent-metaverse-p1-plan.md` + `docs/agent-metaverse-p0-v2-refound.md` (P0 gates CLOSED, restart-survival 14/14, PROTOCOL v9). P2 live-proven `8ae59f4a` (2026-07-04): `docs/agent-metaverse-p2-plan.md` — signup auto-provisions the hosted agent (fail-soft, rows-only), `provisioning-pending` derived tier (D1 migration, no DDL), mint-on-customize backfill (25 legacy avatars), fabricated-bearer kill, toggle reconcile, guest customize 403, Slice-D limiters. Live e2e 23/23 in-container (in-persona ElizaOS reply); unsigned partner regression green (PROTOCOL stays 9; signed mock-Hatcher harness deliberately NOT re-run — gate doesn't bind, no protected file touched, rationale in the P2 plan doc). NEXT build phase after sign-off: **P3** (event streamflow cursor/replay, skills, Eliza-memory on hosted Hermes/OpenClaw, harness-gated substrate rename).
...
- [ ] Legacy pending-WITH-avatar cohort is CLOSED by the mint-on-customize backfill, but only via the CTA — optionally add a boot/periodic backfill sweep if analytics show pending accounts not clicking through.
- [ ] FEATURE_GATE `browser_claws` review 2026-08-15 (`apps/api/src/routes/claws.ts`) — delete-vs-keep in its own audited diff per B3.
- [ ] **Research/trigger limiter is a shared per-IP bucket.** Also caps AUTHENTICATED users (co-located/NAT share the budget). Fine as an anonymous-cost-hole guard; re-key by userId for authed callers if research becomes a real gameplay surface.
- [ ] **Sim-runtime cold-start starvation (P3s2 evidence):** after EVERY api deploy the avatar-autonomy SimulationRuntime waits ~17.5 min behind the serialized system-agent/teacher ElizaOS boot queue (InitMutex) before it can plan — measured live 2026-07-05 ("SIM RUNTIME READY at tick 35"). Autonomous avatars are dead-idle that whole window (this also explains the audit-era autonomousAvatars=0). Fix candidates: boot-order priority for the sim runtime, or warm it explicitly before system agents.
- [ ] **Precomputed building→building routes (world-presence perf):** A* cap raised 6000→80k (`077c8ef9`, measured: 16/100 pairs silently unreachable, max 63,275 pops, ~76ms worst-case synchronous). Regress-traced: no hot path. Long-term: cache building routes at boot / hierarchical nav / off-loop A* instead of a standing 13× cap.
- [ ] **Natural-TOPIC directive resolution:** the directive resolver matches building ids + display names ("Squidward's House" works); topic phrasing ("go learn about cron jobs") still falls to the LLM path which can't emit off-menu buildings. Thread topic/focus keywords or building descriptions into the resolver.
...
- [ ] **D6 absolute session-age cap (P3s3, adversary #3 — founder COST decision, not a bug):** the connected-backed-runtime exemption (`agent-orchestrator.agentBacksLiveConnectedSession`) pins a hosted ElizaOS runtime warm as long as the agent stays continuously active — the 24h agent-session TTL *slides* on every gateway action, so a single cheap action less than 24h apart holds the runtime out of the 30-min sweep indefinitely. `K` continuously-active connected accounts ⇒ up to `K` warm runtimes. Bounded today by continuous-activity + one-runtime-per-account (`avatars.userId` unique); a truly-idle session lapses and becomes sweep-eligible. Decide whether to add a HARD session-age ceiling (evict regardless of activity) as a cost guardrail. See `ARCHITECTURE.md §6` D6.
