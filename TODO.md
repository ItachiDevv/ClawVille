...
### ⭐ FOUNDER QUEUE — decisions + sign-offs blocking "done" (keep at top; P0+P1+P2+P3s1+P3s2 all live-proven on staging, Rule E4 gates open)
- [ ] **Browser sign-off, P1+P2 (+P3 slices 1-2) in one staging pass:** P3s2 adds: toggle Autonomous → type a directive in the chat bar ("go to Squidward's House") → watch your avatar walk there; clear via the Directing ✕ (phone-portrait tap fixed) · P3s1 adds (no browser surface — evidence is `streamflow-e2e.ts` 24/24 on `ee8badb7`: agent settle → live SSE settlement frame + durable replay cursor + Last-Event-ID resume, zero loss/leak) · watch Coralia walk + converse · click a magic-link control handback · fresh email signup → wallet-backup modal on /game → /create-agent prefills (customize mode) → first chat answers in persona (no "reconnect" 404) → toggle shows Controlled/Autonomous · guest NPC mode unaffected · mobile/iPad glance at the amber pending banner + "Finish Agent Setup" sidebar row.
