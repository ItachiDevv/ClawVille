...
- [ ] **Guest-account economy — DECIDED (founder 2026-07-05): ALL FAKE + ON-SCREEN DEMO LABEL.** Guest (is_guest) accounts must earn DEMO tokens only, with the UI telling the player they're demo. ENGINEERING: gate the guest earn paths (chat +1/turn, activities, daily-login, the 100-CT mint) to a demo balance, add the on-screen demo indicator, and amend model doc §2 + GameFeatures. Companion question OPEN: also remove is_guest accounts from the public leaderboard (they can rank today)? — presumably yes under this ruling, confirm below.
- [ ] **Guests CAN rank on the GLOBAL leaderboard today** (verified 2026-07-04 — no is_guest SQL exists; two stale comments claiming otherwise were corrected). Adding the exclusion changes public board composition — ratify together with D2.
