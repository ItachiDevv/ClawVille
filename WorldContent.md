...
Draw calls, MEASURED with `renderer.info.render.drawCalls` on the WebGPU backend: **24 per frame**, of which **9 are the room** — walls, ceiling, floor slab, trim glow, holo dais, monitor kiosk, the instanced desk row, the instanced chair row and the big board. The other 15 are 12 Milady VRM submeshes plus overhead; the shared player avatar draws more than the whole room by itself, and reducing that is an avatar-pipeline change, not an interior one. Adding desks or chairs costs nothing: each row is one draw call at any count.
