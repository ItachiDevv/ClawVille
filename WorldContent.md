...
| Trade tape | **12 emissive slabs**, each **160 × 90**, in TWO lanes at x **±890** (slot 0 left, slot 1 right), y **520**, drifting z **−900 → +720** over **18 s** then wrapping. One slab per recent verified trade: GREEN for a sell that realised a gain, RED for a loss, CYAN for a buy, SLATE where we have no figure. Face carries the token name (or the venue — never a mint) and the signed USD figure. Scene-owned, live data, same `useHouseTraders` query as the board — no route and no second poller. ONE mesh, ONE atlas, **1 draw call**; a newly arrived trade enters at the board wall with a grow-and-rise, while the tape a visitor walks in on is already mid-flight. Lane x is fixed by the desk face (985) on one side and by keeping the board unoccluded from the spawn on the other, both computed by test | `trading-floor-trade-tape-mesh.tsx` · rules in `trading-floor-trade-tape.ts` |
