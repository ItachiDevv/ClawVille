---
title: PET_SCALE must be calibrated against actual GLB bbox — never assume from comments
category: gotcha
tags: [scale, PET_SCALE, lobster, GLB, bbox, calibration]
date: 2026-04-17
confidence: high
...
`PET_SCALE` in `player-pet.tsx` is a flat multiplier applied to the lobster/crayfish GLB.
If the GLB is ever updated, the native height changes and `PET_SCALE` becomes miscalibrated.
...
- `PET_SCALE=20` was set during pass 2 scale-down with comment "targets ~48 wu"
...
- Player pet appeared 2× smaller than NPC lobsters on screen

**Fix:** `PET_SCALE = 40` → `40 × 1.12 = 44.8 wu ≈ TARGET_NPC_HEIGHT = 45 wu`.
...
auto-measures the GLB and is immune to this — but `PET_SCALE` is hardcoded and needs
