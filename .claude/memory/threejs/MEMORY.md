...
## Gotchas
- [SkinnedMesh frustum culling causes disappear-at-close-range](gotchas/skinned-mesh-frustum-cull-close-range.md) — VRM + GLB SkinnedMesh bounding spheres come from bind pose (T-pose); animated geometry extends outside them; Three.js culls the mesh when camera is close/angled. Fix: `scene.traverse(o => { o.frustumCulled = false; })` in vrm-loader.ts after rotateVRM0 AND in GLBNpcMesh useMemo after scene.clone(true). (2026-04-21)
