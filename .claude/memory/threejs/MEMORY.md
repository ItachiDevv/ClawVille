...
## Gotchas
- [three-vrm v3: AnimationMixer must target normalized bones not raw bones](gotchas/vrm-threejs-v3-normalized-bones-for-animation.md) — `getRawBoneNode()` produces permanent T-pose because `vrm.update()` overwrites raw bones every frame; use `getNormalizedBoneNode()` so mixer drives `Normalized_*` nodes; `vrm.scene.getObjectByName` finds them via VRMHumanoidRig child traversal. (2026-04-23)
