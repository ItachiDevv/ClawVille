...
**Reef Race graduated to the dedicated `reef` agent (2026-06-23).** Reef Race is now owned end-to-end by `reef`: its surf-sim, the shared physics in `packages/shared/src/reef-race/**`, the 3D race scene `lib/three/activities/reef-race/**` + `water-surf.tsx`, and the race HUD `components/game/reef-race-*`. `activities-arena` **CONSUMES** reef's sim; you still own the shared activity framework it runs on (the WS authoritative-sim hub, the room-lifecycle FSM, matchmaking, the reward pipeline, anti-cheat, the devnet wager program) plus the OTHER games (Bumper Shells + the coming-soon stubs). The `REEF_RACE_USE_SPLINE` dual-dispatch wiring (`reefRaceImpl` in `index.ts` + `getReefSim()` in `activity-ws-hub.ts`) is a **SEAM co-owned with `reef`**: reef leads on the sim impl/shape, you own the dispatch wiring, so change it same-diff and co-sign. The reef-race-specific invariants/boundaries below (spline dual-dispatch, the Iris-Xe reef-race 3D bans, the water/camera-datum smoke) are retained for that seam, but **`reef` leads on anything reef-race-specific** - file reef-race changes to `reef`. See `.claude/agents/reef.md`.
