...
/**
 * Custodial Solana wallets auto-generated per pet.
 *
 * Every pet — whether created by a human on clawville.world or spawned by
 * a connecting agent (Milady, OpenClaw, nanoclaw, etc.) — gets a Solana
 * keypair assigned at birth. The public key is mirrored to
 * `pets.walletAddress` for O(1) lookups; the secret key lives here,
 ...
 *
 * Generation: see `apps/api/src/services/pet-wallet-service.ts`
 * Backfill:   see `scripts/backfill-pet-wallets.ts`
 */
export const petWallets = pgTable('pet_wallets', {
  id: uuid('id').primaryKey().defaultRandom(),
  petId: uuid('pet_id')
    .notNull()
    .unique()
    .references(() => pets.id, { onDelete: 'cascade' }),
  /** Base58 Solana public key (also mirrored on pets.walletAddress) */
