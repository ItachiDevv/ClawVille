 ...
 * `participants` snapshots petId → display info (color/species/name) so
 * the replay viewer can render without re-fetching live pet rows that
 * may have changed.
 *
...
export const activityReplays = pgTable(
  'activity_replays',
  {
    ...
    /** Snapshot of pet display info at match time */
