 ...
 *   3. pet_skins            — ownership ledger. Records which pet owns
...
export const petSkins = pgTable(
  'pet_skins',
  {
    ...
    skuId: uuid('sku_id')
      ...
      // Retire SKUs via `availableUntil`; pet_skins rows survive even if
  ...
  (t) => ({
    // Each pet owns each SKU exactly once. Re-purchases are a no-op (idempotent).
    uniqPetSku: uniqueIndex('uniq_pet_skin_pet_sku').on(t.petId, t.skuId),
    // Hot path: load all equipped skins for a pet.
    idxPetEquipped: index('idx_pet_skin_pet_equipped').on(t.petId, t.equipped),
  }),
);
