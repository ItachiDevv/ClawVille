 ...
 *
 * One row per (pet, activity) holding the FASTEST single lap that pet
 ...
 * `INSERT ... ON CONFLICT (pet_id, activity_id) DO UPDATE WHERE
 * EXCLUDED.best_lap_ms < reef_race_personal_bests.best_lap_ms`.
 *
 * Reads:
 *   - Snapshot.init carries the self pet's ghost frames (RoomMeta.selfBestLapGhost).
 ...
 * reach this table (sim discards them); pets with anti-cheat flags this
