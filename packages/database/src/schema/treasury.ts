...
// --- NeoToken audit ledger ---

/**
 * Source of a NeoToken transaction — used for observability and
 * replay-to-chain migrations.
 */
export const neoTokenSourceEnum = pgEnum('neo_token_source', [
...
/**
 * Append-only audit ledger for every NeoToken credit/debit.
 *
 * pets.neoTokens remains the authoritative balance column; this table
 * is the auditable history. Every write to pets.neoTokens MUST go
 * through creditNeoTokens() / debitNeoTokens() in
 ...
 *
 * When we eventually tokenize NeoTokens (Phase 5+), we replay this
 * ledger to establish opening on-chain balances.
 */
export const neoTokenTransactions = pgTable(
  'neo_token_transactions',
  {
    ...
    /** Snapshot of pets.neoTokens AFTER this transaction applied */
    balanceAfter: integer('balance_after').notNull(),
    /** Short human-readable reason: 'buy_book', 'autonomous_visit', etc. */
    reason: text('reason').notNull(),
    /** High-level source category */
    source: neoTokenSourceEnum('source').notNull(),
