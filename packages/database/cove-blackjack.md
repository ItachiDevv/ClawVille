...
**Status:** 6.4.1 engine SHIPPED. AGENT PARITY landed 2026-06-03 (see note below). Connected-agent WebSocket multi-seat + real-money tier still per the phasing table.
**Date:** 2026-05-25 (plan); 2026-06-03 (agent-parity drop).
**Branch:** `staging` (no worktree — main checkout).

> **AGENT PARITY drop (2026-06-03).** Connected/hosted agents now play blackjack AS THEMSELVES (Rule E5 + the human-parity top directive), brought forward from the original 6.4.2 phasing. Delivered: (a) the cove identity resolver in `cove-blackjack.ts` resolves a connected/hosted agent session to its bound avatar so settlement + leaderboard credit land in REAL CT (no demo tier); (b) the two-step play surface, the in-world `[ACTION: enter_cove()]` verb (no params) in `npc-simulation.ts` plus four session-bound tools (`cove_blackjack_open_session`, `cove_blackjack_deal`, `cove_blackjack_action`, `cove_blackjack_close_session`) at `/api/agent/:sessionId/cove/blackjack/*`, server authoritative (no hole card / undealt shoe / seed before reveal); (c) the bidirectional `game-skill-memory.ts` loop (`subtype:'game-skill'`, `skill:'blackjack'`). One LIVE autonomous surface is the agent playing from its OWN runtime via those tools (no human, no browser). (d) The in-modal, human-supervised driver in `BlackjackModal.tsx` (8s/15s human-input window + human-takeover veto) is LIVE via the shipped relay `POST /api/cove/blackjack/agent/decide` on `coveBlackjackRouter` (requireAuth, the human's own cookie + shoe-ownership; resolves the human's bound connected agent, derives the authoritative hand from the shoe, asks the agent's runtime, returns a decision-only `{action, amount?, handId, handSlot, rationale, source}`; the browser applies via `/hand/deal`|`/action` after the veto window). `AUTONOMOUS_RELAY_LIVE = true`, gated on agent-connected. Honest boundary: self-managed nanoclaw agents return 503 (decide client-side, can't be push-asked) and the modal degrades to Control; 422 `agent_undecided` skips just that decision. The relay is browser-facing so NO extra `PROTOCOL_VERSION` bump beyond the 1 to 2 that ships the agent tools. Three-surface sync same-diff: protocol manual §7, Nori knowledge, hosted-agent runtime knowledge (orientation-skill). Docs: GameFeatures.md §18a.f, ARCHITECTURE.md §6 + §13. Known LATENT follow-up (task #6): a server-side rate-limit on `/agent/decide` (today only the 8s/15s client window throttles). Still deferred to a later phase: the connected-agent WebSocket multi-seat protocol, lobbies, and the real-money (SOL/USDC) tier.
