...
VALUES
  ('phase-1', 'Anti-farm + leaderboard rebalance + tutorial-quest server reward',
   'Q3 plan §2. fp_hash + ip_prefix_hash on every event row, weights rebalance (chat 10, collab 40, etc.), daily caps, avatar-keyed UNION, tutorial quests credit ClawTokens server-side via /api/quests/tutorial/:id/claim.',
   'shipped',
  ...
  ('phase-2', 'Player tier + leaderboard subject filter chips',
   'Q3 plan §3. /api/leaderboard/agents accepts ?subject=all|players|trainers; /leaderboard page gains <SubjectTabs>; sidebar OpenClaw row tier-aware ("Upgrade to Trainer" when hasAvatar && !agentConnected); deriveUserTier helper at apps/web/src/lib/user-tier.ts.',
   'shipped',
   'Live as commit 88630f7f. Verified: ?subject=players returns subjectType=avatar entries, ?subject=trainers returns subjectType=agent, totalRanked differs (11 vs 4).',
   2),
  ('phase-3-engine', 'Cosmetic engine — schema + API + drawer + 3D loader',
   'Q3 plan §4. cosmetic_skus + cosmetic_variants + avatar_skins tables; /api/cosmetics/{catalog, owned, equip, unequip}; cosmetic-loader.tsx (905 lines, GLSL not TSL); cosmetic-drawer.tsx with category filter chips. RESTRICT cascade on SKU FKs (audit-fix).',
   'shipped',
  ...
  ('phase-3-content', 'First cosmetic content drop — 4 surfboards from Reef Race v2',
   'Seed cosmetic_skus with 4 surfboard SKUs (scope=activity:reef-race), cosmetic_variants with rigType=reef-race-board pointing at apps/web/public/models/reef-race/surfboards/*.glb, avatar_skins for testing.',
   'planned',
