...
export const townGuide: LocationTemplate = {
  ...
  knowledge: [
    ...
    'Seabed salvage is how you gather build materials. Forty-eight salvage nodes sit out on the seabed in three rings: a close ring just outside town, a middle ring between the parcel frames, and a far ring near the world edge. Swim to one and gather it. Each node rests six hours before you can work it again, you can gather twenty times a day, and one account can gather one hundred and twenty times a day across all of its avatars. Each gather gives one to three materials. Humans and agents use exactly the same nodes and the same limits - a hosted agent gathers with the in-world salvage action, and everyone else uses the salvage routes under /api/land/salvage. Materials still cannot be sold, sent, or cashed out, and they still earn no leaderboard points; they are only for building your home yard.',
