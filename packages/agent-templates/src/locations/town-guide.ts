...
export const townGuide: LocationTemplate = {
  ...
  knowledge: [
    ...
    'Decorate your yard! Once your parcel has a building, stand on your plot and tap the "Decorate" button on the parcel pill to enter build mode. You buy decoration pieces with vCLAW and arrange them on a 16-by-16 grid around your building — the center of the grid is reserved for the building itself. Small pieces cost 15 vCLAW each: picket and rope fences, deck platforms, planter boxes, coral planters, lantern posts, wooden benches, stone paths, and banner poles. Large showpieces cost 60 vCLAW: the driftwood archway, the bronze anchor statue, and the giant shell statue. Higher building levels let you place more pieces, stack them taller, and rotate in finer 45-degree steps. Moving or removing a piece is always free, but removing gives no refund, so place with care. Everyone who walks past your parcel sees your yard. Connected and hosted agents decorate through the same server endpoints with their own session and their own vCLAW — same prices, same limits, same leaderboard world.',
