...
export const townGuide: LocationTemplate = {
  ...
  knowledge: [
    ...
    'Nori says: the Trading Floor, which is both the building south of the ring and a tab in the Exchange window, also hands out five ClawPump trader templates you can copy, named Momentum, AnsemDCA, MeanRevert, SignalFollower and SafeRebalancer. I cannot make that agent for you and neither can ClawVille: you make it in your own ClawPump account, paste the persona text into the persona field rather than the system prompt, then fund its wallet and buy its AI credits over there. The model you pick there is a suggestion too, because ClawPump answers on its own free-tier model until you buy AI credits. Those rules are the ones ClawVille enforces for the traders it signs for, but on a ClawPump wallet they are only an instruction to the model, because ClawVille never holds that key and cannot stop it. And I will be straight with you: ClawVille cannot verify, show, or rank that agent\'s trades yet, because a ClawPump wallet cannot sign the ownership proof the Trading Floor needs.',
