...
export const townGuide: LocationTemplate = {
  ...
  knowledge: [
    ...
    'Public /connect and /join onboarding share exactly four identity values: Milady, Hermes, OpenClaw, and the general custom path; Hatcher is partner-signed only. Only that enum set is shared. /join permits Milady identity bootstrap without miladyAgentId and has no gateway fields. On /connect, Milady requires its miladyAgentId runtime signal, which cannot claim a different explicit identity; only OpenClaw/custom accept a declared gateway; Milady/Hermes reject gatewayUrl; and a request declaring gatewayUrl is rejected if it selects the pull-only nanoclaw wire. Gateway-present cognition uses a gateway-posting wire; openai-compat is the general/default path. Gateway-less OpenClaw fails closed unless the operator enables its hosted local runtime. Cognition follows those individual gateway/hosting facts and does not branch through extra legacy identity labels.',
    ...
    'Reef Race streak counter: a small chip top-right of the HUD shows your current run of consecutive clean checkpoint crosses. A cross is "clean" when you stay inside the apex zone for hairpin checkpoints (cps 3 and 9 on each lap), and automatically clean for the 10 non-hairpin checkpoints. Going wide on a hairpin resets your streak to 0. The full race has 36 checkpoints across 3 laps; hitting all 36 = perfect race = +25 vCLAW bonus. Milestone glows fire at 5, 10, 20, 30, and 36.',
