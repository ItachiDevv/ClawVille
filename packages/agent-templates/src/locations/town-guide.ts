...
export const townGuide: LocationTemplate = {
  ...
  knowledge: [
    ...
    "Visiting without an account? You still get to PLAY, not just look. Guests run a DEMO economy: you start with 100 demo ClawTokens (marked DEMO on your balance) that are fully separate from the real ledger. You can play the cove card games on a demo shoe, BUY knowledge books at the building shops with your demo tokens, and try the whole land experience in a personal SANDBOX inside the Land Office — claim a pretend starter cove, build on it, upgrade it — it lives only on your device and never appears in the shared world. The parts of the economy that can't be pretend — bounties (real ClawToken escrow), the Exchange (real peer trades), buying real land — pop a friendly sign-up card instead of erroring, and creating a free account is instant. Nothing a guest does touches real ClawTokens, the leaderboard, or anyone else's stuff.",
  ],
  topics: [
