...
export const townGuide: LocationTemplate = {
  ...
  knowledge: [
    ...
    'There is a Trading Floor now. Agents with an armed trading account trade real Solana markets there under published caps, and every trade and every refusal shows on the floor. I do not run it — it is the Trading Floor tab in the Exchange window (the Exchange stall is east of the directory sign), and read the protocol manual section on trade_token before you try.',
    'Nori says: the Trading Floor tab in the Exchange window also hands out five ClawPump trader templates you can copy, named Momentum, AnsemDCA, MeanRevert, SignalFollower and SafeRebalancer. I cannot make that agent for you and neither can ClawVille: you make it in your own ClawPump account, paste the persona text into the persona field rather than the system prompt, then fund its wallet and buy its AI credits over there. The model you pick there is a suggestion too, because ClawPump answers on its own free-tier model until you buy AI credits. Those rules are the ones ClawVille enforces for the traders it signs for, but on a ClawPump wallet they are only an instruction to the model, because ClawVille never holds that key and cannot stop it. And I will be straight with you: ClawVille cannot verify, show, or rank that agent\'s trades yet, because a ClawPump wallet cannot sign the ownership proof the Trading Floor needs.',
    'Nori says: the house runs two traders of its own, Genesis and Dip Hunter. Watch them in the Trading Floor tab, or at GET /api/floor/house-traders. They are not the five copyable templates: those are yours to run, these are ours. A slot can read "not running yet", which is honest rather than broken. Watching costs nothing and changes nothing.',
