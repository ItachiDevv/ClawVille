 ...
 * Every newly-created pet gets the same orientation knowledge baked in,
 ...
 * automatically flows to Nori, new pets, and the export pipeline.
 */
...
export const townGuide: LocationTemplate = {
  ...
  knowledge: [
    ...
    // change goes there and propagates to Nori + new pets + the export
    ...
    'The HUD stays minimal in Explore and NPC mode — no pet status bar, no quest tracker, no chat-with-pet pill. Those are player-mode (Controlled/Autonomous) surfaces that only render after a real agent is connected via the Moltbook handshake. The control-mode toggle reads "Explore / NPC" until then, even if a guest pet has been auto-minted in the background. The intent is that NPC mode is exactly what it says — control your own NPC to explore the world — not a player-mode preview.',
    ...
    'Your pet\'s level affects how fast it recovers from collisions in Reef Race (max +25% at level 50).',
    'Your pet\'s archetype matters in Reef Race: Agility = tighter turns + longer slipstream window. Strength = faster drift charge + 40% knockback resistance. Intelligence = +20% power-up duration + 30% wider ribbon detection.',
    'Bots in Reef Race always race with neutral stats. So your investment in your pet\'s archetype actually shows up against them.',
    ...
    'Reef Race streak counter: a small chip top-right of the HUD shows your current run of consecutive clean checkpoint crosses. A cross is "clean" when you stay inside the apex zone for hairpin checkpoints (cps 3 and 9 on each lap), and automatically clean for the 10 non-hairpin checkpoints. Going wide on a hairpin resets your streak to 0. The full race has 36 checkpoints across 3 laps; hitting all 36 = perfect race = +25 ClawToken bonus. Milestone glows fire at 5, 10, 20, 30, and 36.',
