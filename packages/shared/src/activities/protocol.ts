...
export interface PbDelta {
  ...
  newMs: number;
  /** Previous best in ms; `null` when this is the pet's first PB ever. */
  oldMs: number | null;
  /**
   ...
   * gating this field on `ws.data.identity.petId === <pb-setter-petId>`.
   */
...
export interface RoomMeta {
  ...
  >;
  /**
   * Reef Race Phase 4 — self pet's PB ghost replay frames. Sent ONCE per
   * snapshot.init for the SELF pet only (not other racers — too crowded
   * per spec §2). Skipped for non-Reef-Race rooms, guests, bots, and pets
   * without a PB row.
   *
