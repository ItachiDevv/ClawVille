...
export const BUILDING_OPENCLAW_THEMES: Record<string, { label: string; focus: string; category: string }> = {
  ...
  'canvas-studio': { label: 'Pineapple House', focus: 'AI image (Nano Banana Pro, FLUX.2, GPT Image 2), AI video (Veo 3.1, Kling 3.0, Seedance 2.0), AI 3D (Hunyuan 3D, Tripo, Rodin), agentic pipelines (fal.ai, Replicate, ComfyUI), real-time interactive visuals in TouchDesigner, Adobe Photoshop / After Effects / Premiere Pro deep controls + UXP / ExtendScript / aerender / AME automation, and Blender (Geometry Nodes, Cycles + EEVEE Next, rigging, sculpting, Python bpy, headless rendering)', category: 'Visual Creation' },
