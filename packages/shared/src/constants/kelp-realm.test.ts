...
describe('Kelp Forest realm layout invariants', () => {
  test('is a 21x21 origin-centered maze with one outer entry gap', () => {
    ...
    expect(KELP_REALM_CELL_WU).toBe(480);
    expect(KELP_REALM_FOOTPRINT_WU).toBe(KELP_REALM_COLS * KELP_REALM_CELL_WU);
    expect(KELP_REALM_FOOTPRINT_WU).toBe(10_080);
  ...
  test('keeps at least 60wu visible between opposing 100wu sways', () => {
    ...
    expect(KELP_REALM_CORRIDOR_WIDTH_WU).toBe(480);
    ...
    expect(KELP_REALM_VISIBLE_CORRIDOR_MIN_WU).toBeCloseTo(395.580, 3);
    expect(KELP_REALM_LAYOUT_INVARIANTS.visibleCorridorMinWu).toBeCloseTo(395.580, 3);
  });
