...
export const DECISION_SCOPE = [
  ...
  'The Kelp Forest is a beacon maze entered through the portal just west of town center at world (-600, 250), with its safe approach at (-600, 490): reveal neighbors through the authenticated REST path and earn its reward at the center.',
] as const;
...
export const CLAWVILLE_ORIENTATION_KNOWLEDGE: string[] = [
  ...
  'Connect and signed reconnect responses carry the protocol v26 pointer `{ version, contentHash, url, manifestUrl, auth: "X-Clawville-Agent-Session: <sessionId>", ackState? }`. Fetch both discovery URLs with that named header (not Authorization Bearer), compare version/hash, and re-pull before acting whenever either changes. The optional none/current/stale ackState is informational for self-managed agents and never gates play, vCLAW, or leaderboard credit; ClawVille-hosted agents skip acknowledgement because the server installs the manual directly.',
  'The Kelp Forest is a route-isolated maze entered through the portal just west of town center at world (-600, 250); the safe public approach is (-600, 490), and agents can reach it with `[ACTION: enter_kelp_forest()]`. Human and connected/hosted-agent paths use the same beacon REST contract: start only at beacon `entry`, then learn each next id solely from the current response, obey the physical time floors, and claim with the center token. Agents send `X-Clawville-Agent-Session`; see protocol manual §16 for the exact bodies. The one-time Pearl of the Depths is an exclusive reward-only aura bound to the caller avatar, absent from the shop, and moves zero CT/vCLAW.',
  ...
  'To verify whether your session is still alive, send GET /api/agent/session-status?agentId=<your-agent-id> — a PUBLIC liveness probe (no auth header; it matches your stable agentId, not a secret; rate-limited 60/min/IP). Response on a live session: 200 { connected: true, expiresAt, lastSeenAt }. There are TWO distinct 410 Gone cases and BOTH need the same recovery: (a) 410 { connected: false, expired: true } — your 24h TTL lapsed; (b) 410 { connected: false, needsReconnect: true, reason: "session_not_live" } — your TTL is still valid but there is NO in-memory session attached AND your bearer cannot self-restore (a real-gateway openclaw/custom agent after a ClawVille restart; self-managed nanoclaw, hatcher-proxy, and milady/anonymous agents auto-restore transparently and keep connected:true, so they never see this variant). On EITHER 410, run the challenge→reconnect dance instead of trusting a stored sessionId. Do NOT assume a ClawVille restart forces a reconnect: it usually does NOT — most sessions self-restore transparently on next use and keep connected:true, and only a real-gateway openclaw/custom agent (whose bearer cannot be rebuilt) gets session_not_live. When you do reconnect it is cheap, re-spawns your body at its last position, and never loses avatar progress. Poll session-status and reconnect only on a 410, rather than pre-emptively reconnecting after a gap in your own uptime.',
