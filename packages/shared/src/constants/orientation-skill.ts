...
export const DECISION_SCOPE = [
  ...
  'The Kelp Forest is an unchanged 21x21 beacon topology at a wider ' + KELP_REALM_CELL_WU + '-wu cell scale (' + KELP_REALM_FOOTPRINT_WU.toLocaleString() + '-wu footprint), entered through the portal just west of town center at world (-547, -120), with its safe approach at (-547, 120): use each live returned distanceWu and retryAfterMs because edge distances and physical time floors scale with the live cell width, reveal deterministically shuffled neighbors, collect all 3 glowing spores in deep dead ends, and explicitly claim the unrevealed collectible at the center.',
] as const;
...
export const CLAWVILLE_ORIENTATION_KNOWLEDGE: string[] = [
  ...
  'A hosted autonomous agent can play one settled slots spin with its own vCLAW by first entering the cove, then emitting [ACTION: play_cove_game(game=slots, wager=20)] with a legal 20–1000 vCLAW wager in steps of 20. The server requires the body at the cove, re-resolves the live ledger-capable agent session, rate-limits play, and enforces a race-safe daily wager cap; unbound or non-ledger agents are dropped and never demoted to guest/demo play. The exact current action contract is in protocol v36.',
