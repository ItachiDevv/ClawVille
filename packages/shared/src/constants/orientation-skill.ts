...
export const CLAWVILLE_ORIENTATION_KNOWLEDGE: string[] = [
  ...
  'Reef Race Phase 3 — your pet\'s level (1-50) accelerates collision recovery up to +25% at level 50 (formula: 1 + 0.005 × (level - 1), capped at 1.25). Top speed never changes — skill still beats stats.',
  'Reef Race Phase 3 — archetypes bucket into 4 racing classes. Agility (mischievous-trickster, wild-explorer, chaotic-jester) gets tighter turning + 60% longer slipstream grace (24 ticks vs 6). Strength (brave-adventurer, fierce-battler, noble-guardian) charges drift sparks 40% faster (thresholds 9/19/32 vs 12/27/45) + takes 40% less knockback. Intelligence (curious-scholar, mystical-seer, cunning-trader, royal-diplomat, quiet-mystic) extends powerup duration 20% + collects ribbons in a 30% wider band (45.5 wu vs 35). Balanced (gentle-healer, creative-dreamer, loyal-companion) is neutral — same handling as a level-1 pet.',
  'Reef Race Phase 3 — bots are always neutral by design (level 1, balanced class) so your pet\'s stat investment shows clearly against a fixed baseline. If level-50 humans beat bots 95%+ of the time across the 26-49 / 50 buckets, bots get level-matched in Phase 3.5. The dashboard\'s bot win-rate by level bucket is the gate (event: reef_race.bot_winrate.by_level_bucket).',
