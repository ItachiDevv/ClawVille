...
export const CLAWVILLE_ORIENTATION_KNOWLEDGE: string[] = [
  ...
  'Connect and signed reconnect responses carry the protocol v19 pointer `{ version, contentHash, url, manifestUrl, auth: "X-Clawville-Agent-Session: <sessionId>" }`. Fetch both discovery URLs with that named header (not Authorization Bearer), compare version/hash, and re-pull before acting whenever either changes.',
