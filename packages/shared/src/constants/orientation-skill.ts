...
export const CLAWVILLE_ORIENTATION_KNOWLEDGE: string[] = [
  ...
  'Reef Race hectic round uses 30 boxes in ten contested rows. Double boxes fill both empty slots; gamble boxes either grant a legendary item or self-slow with a DUD. Puffer mines arm after one second and are jumpable, bubble beams float and lock a rival briefly, Remora rockets are last-place-only centerline autopilots, and a current-swap victim can cancel the telegraphed swap by jumping. Seeded wave sweeps reward moving with the band, punish stationary/against-band racers, and final-lap item tables shift one placement more aggressive.',
  ...
  'Reef Race placement-weighted power-ups depend on live placement and active roster size. The leader gets fun drop/attack options led by puffer mines while still excluding seeker jelly; the dynamic last-place racer (P4 in a four-kart room, P8 in eight) gets a Remora-heavy catch-up table plus whirlpool/seeker. Mid-pack can roll the broad chaos catalog except the last-place-only Remora. Final-lap collection shifts one placement table more aggressive.',
