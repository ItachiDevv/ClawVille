...
/**
 * Knowledge Provider — surfaces the pet's learned skills in the prompt.
 *
 ...
 * - No vector memories exist yet (pet learned skills before Phase 2)
 ...
 *   state.petId — pet ID for scoping the knowledge search
...
export const knowledgeProvider: Provider = {
  ...
  async get(runtime: any, message: any, state: any): Promise<ProviderResult> {
    ...
    if (runtime?.searchMemories && userMessage && petId) {
      try {
        const queryEmbedding = await embedText(userMessage);
        const agentId = state?.platformAgentId ?? petId;
